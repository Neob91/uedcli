...
// Debug-only detector-stage counters (UEDCTL_OPTGEOM_DEBUG): [scans, already_vert, band_reached,
...
fn dbg_on() -> bool {
    ...
    *ON.get_or_init(|| std::env::var("UEDCTL_OPTGEOM_DEBUG").is_ok())
}
#[inline]
fn dbg_bump(i: usize) {
    if dbg_on() {
...
fn merge_near_points(model: &mut Model) {
    ...
    if std::env::var("UEDCTL_OPTGEOM_DEBUG").is_ok() {
...
pub fn eliminate_tjunctions(model: &mut Model) -> usize {
    ...
    let dbg = std::env::var("UEDCTL_OPTGEOM_DEBUG").is_ok();
