...
enum Emit {
    ...
    /// (The leading `usize` is the owner node index, for the UEDCTL_PASSD_DUMP diagnostic only.)
...
pub fn assign_leaves_and_zones(model: &mut Model) -> Vec<usize> {
    ...
    let dump_emit = std::env::var("UEDCTL_PASSD_DUMP").is_ok();
    for e in emissions {
        match e {
    ...
    // TEMP diagnostic (UEDCTL_PASSD_DUMP): full vert-pool layout right after Pass-D re-emit —
    ...
    if std::env::var("UEDCTL_PASSD_DUMP").is_ok() {
