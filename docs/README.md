...
Skim **[usage.md](usage.md)** for the verb families (generators → `actor add -`, per-surface `brush
poly`, per-actor `actor prop set` / `mover key`), then work through **[leveldesign/](leveldesign/README.md)**
— start in `general/` for engine-generic craft (geometry, zones, lighting) and drop into `deusex/`
when you need a Deus Ex class name, dimension, or the immersive-sim approach.
