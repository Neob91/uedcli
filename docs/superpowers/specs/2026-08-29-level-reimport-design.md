# `level reimport` — reimporting a hand-edited `.dx`/`.unr` into an existing trunk
...
- **Write goes through the existing delta path unchanged** — `TrunkLevelSource.save` /
  ...
  `deleted=` set, and the right order overrides before calling it that already exists. No new write
  primitive.
