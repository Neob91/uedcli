...
- `--highlight BRUSH:idx` emphasises the exact face you're about to retexture or align (a bare actor
  ...
  world box `X0,Y0,Z0,X1,Y1,Z1`. Under `textured` a highlight re-colours what is visible; when it is
  not visible at all a stderr note names it (under `--layout quad`, that means no pane showed it).
...
- Every preview carries **locator cells** (on by default): a gutter of column letters (top) and row
  ...
  density; `--no-locator-cells` turns the whole thing off; `--json` emits the cell map for a script.
