...
Get the geometry clean and on-grid and most other problems never appear; get it wrong and you fight
invisible holes for hours.
...
A builder brush (the editor's red "cookie cutter") is shaped, then committed as a Subtract or an Add
...
At build time the compiler walks brushes in order and, where two operations overlap the same region,
the last one wins. Ordering is the geometry, not cosmetic.
...
- Semisolids are fully walkable: same reliable collision as a solid — you can walk and stand on a
  semisolid floor, ramp, or ledge. The only difference from solid is that a semisolid does not cut
  the world BSP (it splits only itself). 📖
...
- A semisolid must not touch another semisolid, a nonsolid, or a zone portal. This reliably wrecks
  the local BSP (invisible polys / HOM / merged zones). 📖 To make two touching semisolids safe,
  weld them into one brush with `brush intersect` (see below). 📖 This is the narrow case where
...
single builder looks per-brush. But because the bit is really per-face, one brush can carry a mix of
...
The community myth is that "off-grid coordinates cause a floating-point overflow / the maths gives
up." That is false. The real cause is a handful of discrete tolerance bands in the split/merge tests
(a ~0.25 uu plane-alignment band; a ~1e-4 uu vertex-merge band). Off-grid coordinates land inside
those bands, so faces get mis-classified, collapsed, or thrown away. The fix follows: stay out of the
bands — build on clean coordinates.
...
6. Watch node count. Every brush face is a partitioning plane; an awkward face becomes a "supercut"
   ...
   ~128,000-point limit is what actually crashes); instability often starts well before, around
   45–55k.
...
- "Always intersect/deintersect touching brushes" — unnecessary on-grid, and often adds complexity.
  ...
  "Solidity is stored per-face" above; that's a narrow safety case, not a routine habit.)
