...
How to build the **non-box brush shapes** that real levels are made of — the constructions you reach
for once a plain `brush build cube` isn't enough. These are *shape* recipes (make a chamfered beam, an
octagon column, a ring cornice), distinct from the *feature* recipes in the parent dir (water, doors,
lifts). All are engine-generic CSG technique.

> **Build-verified.** Every pipeline here was executed end-to-end using only the CLI (no source, no
> hand-authored T3D) and the result checked against the target. So these are known-good verb
> sequences, not lore.
...
## Two workflow patterns worth internalizing
...
> **Provisional observation (📊 one map, no control — NOT yet a general claim).** In HK: WanChai Street,
> the add/subtract-twin workflow and facet-faked curves dominated the non-box geometry, and brush
> complexity stayed low (3–8-sided prisms; only stairs/domes were high-vertex). Whether that's
> DX-general or engine-general is unconfirmed until a second map / a UE1 control is measured — treat as
> "observed once," not fact.
