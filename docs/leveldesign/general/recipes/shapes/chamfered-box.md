...
- `brush build cube` is origin-centred on every axis (including Z), so the corner coordinates are
  known and the clip plane follows from them. A non-normalized normal (`1,0,1`) is fine — only its
  direction matters.
