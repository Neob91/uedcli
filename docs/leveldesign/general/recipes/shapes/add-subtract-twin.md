...
A workflow, not a shape — how real levels seat shaped trim, ledges, and platforms into walls.
Build a shaped additive solid, copy it, flip the copy to subtractive, and drop it into the wall so it
carves a recess the exact shape of the trim.
...
- Flipping add ↔ subtract is `CsgOper`. `actor duplicate` has no `--csg` flag, so set the property
  directly: `actor prop set <name> CsgOper=CSG_Subtract` (enum values are `CSG_Add` / `CSG_Subtract`).
...
- This is why much real geometry comes in blue/orange pairs: a shaped additive brush and a
  near-identical subtractive one nearby is this idiom — build the solid, carve its twin.
