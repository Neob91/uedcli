...
is an **L** rather than a rectangle. This is the simplest use of `brush build extrude`: you draw the
silhouette once and sweep it along the run.
...
- **`--at` is where profile `(0,0)` lands**, not the brush centre — the vertices are the numbers you
  typed, so the ledge lands exactly where you drew it. A consequence worth knowing before you reach
  for it: `--rotate` turns the actor about that same local origin, so a profile drawn away from
