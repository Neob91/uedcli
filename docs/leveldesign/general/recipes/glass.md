...
glass; since sheets never block collision, a thin Invisible Collision Hull around the sheet stops the
player. 📖
...
2. Build a sheet sized to the opening on a vertical axis (not floor/ceiling orientation — glass is a
   wall).
3. Add Special → Transparent Window with a glass texture selected, and check 2-Sided to see the glass
   from both rooms.
...
5. Add Special → Invisible Collision Hull on that box, textured with something you'll never see (the
   hull's faces are invisible in-game). This blocks the player.
6. Test (press P): in-game the hull vanishes but the glass texture shows and the player can't pass.

The collision hull must not touch surrounding walls or zone boundaries — a hull abutting geometry
causes HOM. Keep it a clean thin box just around the glass.
...
The sheet-plus-hull recipe makes glass as separate actors. When the glass must be one brush — most
importantly a mover door with a window (a mover is one brush, so a separate glass actor can't ride
it) — build it as one welded brush with `brush intersect`. This relies on the per-face-solidity fact
in [../geometry-and-bsp.md](../geometry-and-bsp.md#solidity-is-stored-per-face-not-per-brush).

The construction:

1. An additive solid slab — the door/wall panel.
2. A subtracted opening through it — the window hole, piercing the full thickness.
3. A semisolid additive pane filling that opening, textured glass + flagged Translucent. Semisolid,
   not solid: a solid pane's side faces would merge with the subtracted reveal walls, and translucency
   would then let you see into the panel's interior; a semisolid pane's coincident faces don't merge,
   so it sits flush in the opening with no gap and reads as real glass. 📖
...
- `--mover-class` makes the weld a base mover; author the swing/slide with `mover key rotate|move`
  ...
  intersect gives a one-brush framed window.
