...
`direction/README.md` (the topic index) is auto-loaded. Every other doc below — including the root
campaign docs `NATIVE-MATERIALIZE.md`, `USCRIPT-COMPILER.md`, and `GUI-PARITY.md` — is not in your
context; read the relevant one before the action it names. These one-liners are a router, not a substitute: do
...
- `NATIVE-MATERIALIZE.md` — read before ANY native-materialize or parity work: the native-materialize → UED22 full-byte-parity campaign — the goal, the UED22 reference recipe (MAP IMPORT + sacrificial dummy builder), the exact parity bar and its opus-confirmed exclusions, the lockstep-ladder method, and the ONE canonical parity script (`parity_gate.py`). Do not re-derive it.
- `USCRIPT-COMPILER.md` — read before ANY uscript-compiler work: the UnrealScript compiler → UCC.exe byte-parity campaign — the goal, the three reference substrates (UED22/UT99/DXORIG) and why each exists, the parity bar, current status + byte-exact package table, the crux RE findings (ordering algorithm, CRC, flags, conversation `#exec`), and the open gaps. Do not re-derive it. Detailed RE facts live in `dev/docs/unrealed/unrealscript/` (the sole home for that knowledge).
- `GUI-PARITY.md` — read before ANY GUI/UED22-fidelity RE work: the GUI click-detection/selection-rendering → UED22 fidelity campaign — why it's a root doc (escapes the `dev/docs/` per-edit approval gate), the two RE tools (live screenshot probe vs. disassembly) and when to use each, the per-topic parity bar, and the status table of open fidelity questions. Do not re-derive it.
...
- `dev/docs/rules/tests.md` — read before running tests. Run them via `bin/test`, never bare `pytest`; uedcli and its suite are host-native, not containerised. Default to the non-integration suite (`bin/test`'s own default); reach for `-m integration` only when the change actually needs it. Iterating: scope to what the change touches (`bin/test -k <module>`) — run the whole suite once, before committing.
