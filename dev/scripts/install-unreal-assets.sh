#!/usr/bin/env bash
# Set up a complete, working Unreal (1998/Gold) install for uedcli — end to end — from a source you
# supply.
#
# You supply the game EITHER as a local <SOURCE> directory OR as one or more --url downloads:
#   1. Assembles a full working game copy under  dev/games/<game>/   (default <game>=unreal).
...
#   2. Populates a staging tree  uned/UnrealAssets/  FROM that working copy — the same curated
#      subset uned/DeusExAssets and uned/UT99Assets carry: System/ (the stock Gold CD's `.u` code) +
#      Textures/ *.utx, Sounds/ *.uax, Music/ *.umx (content), + Maps/ *.unr[.uz] with --with-maps.
#      NOTHING IN uedcli READS uned/UnrealAssets/ YET — no project/substrate config points at it
#      (only Deus Ex is wired up today) — this script only stages the tree; wiring it in is
#      separate, later work.
#
# Everything written here is Unreal game content. BOTH dev/games/ and uned/UnrealAssets/ are
# gitignored and NEVER committed.
#
# Run with NO ARGUMENTS to install fully autonomously: with neither a <SOURCE> nor a --url, it
# defaults to the archive.org mirror of the Unreal Gold CD (DEFAULT_URL below), verified against a
# pinned checksum (DEFAULT_SHA256, cross-checked against the MD5 OldUnreal's own official
# installer expects for this exact file — see DEFAULT_SHA256 below). OldUnreal's own installer
# (github.com/OldUnreal/FullGameInstallers) uses this same archive.org item as a fallback download
# source — but you are responsible for having the right to whatever copy you install; override the
# default with your own <SOURCE> or --url if you'd rather supply your own (a disc you own, GOG,
# etc).
#
# Unpacking a download ALWAYS runs in the `uedcli-unpack` container (built once, on demand, shared
# with install-deusex-assets.sh/install-ut99-assets.sh), on every host, whether or not the host has
# the tool — one code path, one set of tool versions.
#
# NOT DONE HERE (both are real gaps, left for you or a follow-up run, matching this script's own
# "smallest change" scope):
#   - OldUnreal's own compatibility patch. This installs the STOCK Gold CD build; OldUnreal's
#     official installer additionally fetches and applies a patch from
#     github.com/OldUnreal/Unreal-testing after the base install. Apply it yourself if you need a
#     modern-patched build.
#   - Map decompression. Like the UT GOTY CD, the Gold CD may ship Maps/*.unr as *.unr.uz (Unreal's
#     own compression); this script copies them as-is with --with-maps. Decompressing needs
#     `ucc.exe decompress` (a Windows binary) — not run here. System/Textures/Sounds/Music (what
#     asset resolution actually needs) are NOT compressed on the CD, so this only affects
#     --with-maps.
#
# Usage:
#   dev/scripts/install-unreal-assets.sh [--game <name>] [--with-maps] [--dry-run]
#       (no SOURCE/--url -> the built-in DEFAULT_URL, autonomous)
#   dev/scripts/install-unreal-assets.sh [--game <name>] [--with-maps] [--dry-run] <SOURCE>
#   dev/scripts/install-unreal-assets.sh [--game <name>] [--with-maps] [--dry-run] \
...
# dev/scripts/): the working copy under dev/games/, and the substrate tree at uned/UnrealAssets/.
GAMES_PARENT="$(cd "$SELF_DIR/.." && pwd)/games"          # Tools/uedcli/dev/games
DEST_ROOT="$(cd "$SELF_DIR/../.." && pwd)/uned/UnrealAssets"
...
# the header's rights note). The archive.org mirror of the Unreal Gold CD (a plain ISO9660 image,
# ~645 MB) with a pinned checksum so the built-in download is verified, not blind. Override with
# <SOURCE> or --url.
DEFAULT_URL='https://archive.org/download/totallyunreal/UNREAL_GOLD.ISO'
# sha256, computed directly from the file. Cross-checked against OldUnreal's own installer's
# expected md5 for this exact file (a2dc0525242fce78c01e95b71914a3b4, from its install.php) so the
# trust chain is re-derivable without re-fetching 645 MB.
DEFAULT_SHA256='7e360d0cc9e5533f38859819fd3cbfea7c475ecd428f9f433b5f8e1d5742cbca'
...
# SAME image install-deusex-assets.sh/install-ut99-assets.sh build (uedcli-unpack:latest): shared,
# built once, cached.
...
ensure_unpack_image() {
    ...
    # SAME name, SAME Dockerfile as the other install-*-assets.sh scripts — whichever runs first
    # builds it and the others reuse it (docker image inspect short-circuits above). Keep the
    # Dockerfile blocks identical, or a later build silently wins with a different tool set.
...
unpack_one() {  # <file>
    ...
    case "$low" in
        ...
        *.zip)        run_unpacker "$UNPACK_DIR" "$CACHE_DIR" -- unzip -q -o "$f" -d "$UNPACK_DIR" ;;
        # The default source is an .iso (a plain ISO9660 CD image, same as the UT GOTY CD — read
        # directly by 7z, no InstallShield CAB layer).
...
pick_source_root() {  # <dir> -> prints the root, or exits 2
    ...
    if [[ -z "$roots" ]]; then
        echo "error: after unpacking, found no Unreal install root (a dir holding System/ +" >&2
        echo "       Textures/) under $root." >&2
...
if [[ "$dry_run" != 1 ]]; then
    ...
    if ! ls "$wc_sys"/[Uu]nreal[Ii].u >/dev/null 2>&1; then
        echo "warning: $wc_sys has no UnrealI.u (the single-player gameplay package) — is SOURCE a" >&2
        echo "         complete install? Continuing anyway." >&2
    fi
fi

# --- Step 2: populate uned/UnrealAssets/ from the working copy ---------------------------------
...
# Quick sanity report. Maps counts *.unr* (not just *.unr): the Gold CD may ship maps compressed
# (*.unr.uz, see the note below), so a plain *.unr count could misleadingly read near-zero.
...
echo "Substrate tree at $DEST_ROOT is staged (stock Gold CD build) but not yet wired into uedcli —"
