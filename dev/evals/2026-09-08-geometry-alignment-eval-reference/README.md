# Geometry-alignment eval — UNATCO

`index.html` is a manual grading webapp for geometry-alignment tasks (so far: widen Manderley's
office, raise its ceiling, both UNATCO HQ) — every picture on it is generated, never hand-built.
Designed to grow to dozens of tasks across 5 levels — see "Adding a new task." For how to actually
produce an execution (run a subagent against a task, isolated, with only the skill under test in
scope), see `EVAL-PROCEDURE.md` and `scripts/run_eval.py`; this doc covers the task-spec format and
the rendering/grading pipeline that consumes whatever trunk that produces.
