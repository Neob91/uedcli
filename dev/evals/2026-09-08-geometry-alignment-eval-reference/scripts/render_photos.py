...
def render_photos(task: dict, trunk: pathlib.Path, out_dir: pathlib.Path, prefix: str = "pan"):
    """Writes out_dir/<prefix>_<i>.png for i in 0..7. `trunk`'s UEDCLI_LEVEL
    is assumed "unatco" for now (single-level batch; generalize alongside
    the next level's specs if a level name ever differs from that)."""
