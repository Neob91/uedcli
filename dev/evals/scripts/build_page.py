...
// Enter in the score field saves (and advances, if inbox mode is active).
// NOT in the notes textarea -- Enter there must always just insert a
// newline (a mobile on-screen keyboard's return key has no Shift+Enter to
// fall back on, so overloading Enter as submit there blocks newlines
// entirely, not just on desktop with Shift held).
document.addEventListener('keydown', e => {
  if (e.key !== 'Enter' || e.shiftKey) return;
  if (!e.target.classList || !e.target.classList.contains('gscore')) return;
  e.preventDefault();
