...
**illegal as a real UnrealEngine object name**, not merely a value the allocator promises never to
mint: FNames are alnum/underscore only (`uedcli/preview_shots.py:21`'s stated basis for its own
filename-safety logic), and `*` is neither, so this string can never be a real actor Name. It is never
minted through `allocate_name` either (invariant D6 mints `Uedcli<Class><n>` with a numeric/random
suffix, a disjoint scheme), but that is now a second, redundant guarantee, not the only one.
...
**Discard.** `POST /api/session/{id}/discard {"actors": ["*Builder"]}` reuses the existing
...
`*Builder` — to the trunk-derived actor list, serialized through the exact same per-actor
...
- `POST /api/session/{id}/stage` (existing route, generalized) `{"actors": {name: {prop: value,
  ...
  brush's move/rotate needs with ZERO builder-brush-specific code: `*Builder` is just
...
- `POST /api/session/{id}/discard` (existing route, no change to its shape) — `{"actors":
  ["*Builder"]}` re-seeds it per "Data model", rather than leaving it absent.
...
- **Backend**: session creation seeds the reserved Name's staged entry from `levels/<level>/
  ...
  `*Builder` entry, serialized through the same per-actor path as a real actor; `build`
