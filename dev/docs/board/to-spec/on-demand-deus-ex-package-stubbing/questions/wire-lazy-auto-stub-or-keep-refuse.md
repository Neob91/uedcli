# Should `level materialize`/`photo` auto-stub a referenced v68 package, or keep refusing?
...
`level materialize`/`level photo` instead hit `packages.unloadable_v68_packages` and **refuse** the
...
- **Wire the auto-trigger (recommended).** Call `stub_missing_packages` on the unstubbed-v68 set at
  the materialize/photo resolution pre-pass, before the refuse check. A referenced v68 code package
