...
This worktree only, not yet committed to a spike dir: `_scratch/area51_attrib.py`/`_scratch/
tf_attrib.py` (static per-brush node-owner attribution), `_scratch/area51_prefix_search.py` (prefix
binary search), `_scratch/area51_remove1852.py` (decisive removal test). Coordinating session: commit
these under `dev/docs/spikes/` if kept, per `dev/docs/rules/spikes.md`.
