...
`uedcli/dxread.py` parses a `.dx` → fully-qualified `Level`: point actors (`StateFrame` +
...
- **Error contract:** a `dxread` parse failure (truncated/unknown tag/EOF mismatch) raises a
  ...
  never a traceback (uedcli house rule). `dxread` never returns a partial `Level`.
...
DEPENDENCIES`/`OBJ LIST` stay as editor facts; note uedcli reads natively), `decisions.md`
...
`uedcli/qualify.py` + `uedcli/verify.py` (the legs removed/converted), `uedcli/store_export.py`,
`uedcli/normalize.py` (`canonical_level_hash`, `canonicalize_self_refs`), `architecture.md`
