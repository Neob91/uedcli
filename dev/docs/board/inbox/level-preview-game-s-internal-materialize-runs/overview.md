...
priority = "p2"
kind = "chore"
summary = "`level preview --game`'s INTERNAL materialize runs the H3 post-verify, with no way to skip it"
+++

# `level preview --game`'s INTERNAL materialize runs the H3 post-verify, with no way to skip it
