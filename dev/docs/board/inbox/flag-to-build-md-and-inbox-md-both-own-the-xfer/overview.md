...
priority = "p3"
kind = "chore"
summary = "FLAG: `board/to-build/` and `board/inbox/` both own the `xfer` timeout work, and one item schedules a function for deletion that another schedules for repair"
+++

# FLAG: `board/to-build/` and `board/inbox/` both own the `xfer` timeout work, and one item schedules a function for deletion that another schedules for repair

Not my change.
`board/to-build/` says the inbox chore covers "only `driver.py`'s 8 calls + `xfer.remove`" and claims
