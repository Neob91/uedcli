...
priority = "p2"
kind = "debug"
summary = "`level photo --native` fills polygons by triangle FAN, which bleeds outside a concave face"
+++

# `level photo --native` fills polygons by triangle FAN, which bleeds outside a concave face
...
real. Surfaced while speccing `actor diagram --faces`, whose scanline fill handles concave faces
