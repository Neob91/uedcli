...
Promote the spike decoder to **`uedcli/utexture.py`** (`load_package`, `decode_texture`,
...
for each Texture export i in load_package(package_file):
    ...
    write mip-0 PNG to .uedcli/textures/<package>/<stem>.png  (same path convention as today)
...
`.uedcli/textures/<package>/<stem>.png` (derived, not stored — consumers existence-check and
...
- `unrealed/commands.md` "Texture → PNG" recipe (keep as an editor fact; note uedcli uses
  native decode).
...
## Refs
`spikes/2026-06-27-decontainerize-uedcli/01-native-texture-decode.md` +
