# uedcli de-containerization — roadmap & target architecture (design)

**Ephemeral spec.** Synthesizes the 2026-06-27 spike series
(`dev/docs/spikes/2026-06-27-decontainerize-uedcli/`) into a sequenced plan to remove
uedcli's Docker/wine/`.exe` dependency stack. Decisions land in `decisions.md`; durable
...
> sub-fact — a `Model` written *natively by uedcli* (not by the editor's `EDIT PASTE`) loads
...
Stop needing Docker/containers in uedcli by replacing every editor/`UCC`/`umodel`/
...
uedcli writes everything else natively. This captures most container reduction (texture,
...
`architecture.md` states uedcli aims to be a generic UE1 tool. De-containerization *helps*
...
0. **The pivotal one — geometry strategy:** commit to building **D2** (promoting it from
   ...
   REBUILD`-only-geometry intermediate** (editor builds just the `Model`, uedcli writes
