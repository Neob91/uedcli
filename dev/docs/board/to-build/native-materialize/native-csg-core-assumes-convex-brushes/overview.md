...
— the single non-convex staircase brush mis-builds on `level photo --native` + native `level
materialize` (concave notches fill solid). Confined: default UnrealEd materialize + `--game` photo
