...
`None`-means-needs-(re)build convention as before, PLUS a generation guard (below — a real
correctness bug in the first draft, found by review):

```python
_generation = [0]                                   # bumped on every invalidation
_trunk_ref: list[_LoadedTrunk | None] = [None]
_geometry_ref: list[_BuiltGeometry | None] = [None]

def _get_trunk() -> _LoadedTrunk: ...      # double-checked lock + generation guard, see below
def _get_geometry() -> _BuiltGeometry: ... # same, using _get_trunk()'s level for the build_scene call
```

**Generation guard — fixes a real race found in review.** The naive double-checked-locking sketch
(build under the lock, then unconditionally publish) has a live bug: a trunk edit can settle and
invalidate (`_trunk_ref[0] = _geometry_ref[0] = None`) WHILE a slow build (the CSG solve — seconds
to the measured 2m44s) is already in flight for the PRE-edit state. That in-flight build, unaware
anything changed, then publishes its stale result over the fresh `None` — and nothing re-invalidates
it until the next unrelated edit happens to fire the watcher again. This is not a momentary blip; the
wrong data can persist indefinitely. Fix: capture the generation number before starting a build, and
only publish if it hasn't changed by the time the build finishes — otherwise discard the stale
result and retry against whatever is current:

```python
def _get_geometry() -> _BuiltGeometry:
    ...
    with solve_lock:
        ...
        built = ...run build_scene()...
        if _generation[0] != gen_before:
            return _get_geometry()   # invalidated mid-build -- discard, retry against current state
        _geometry_ref[0] = built
        return built
```

The watcher's settle callback bumps `_generation[0]` in the SAME step it clears both refs (still no
lock, still before broadcasting — see "Invalidation" below), so any build in flight at that moment
is guaranteed to see the change when it checks.

**Accepted, flagged trade-off (review finding, not silently shipped): per-request games-config
changes no longer take effect without a restart, once the geometry cache is warm.**
`_scene_inputs`'s existing docstring documents today's behavior: config is recomputed fresh every
request "so a `--project`'s on-disk games config can change without a `serve` restart." Under this
cache, `_scene_inputs` is still called fresh every request, but a warm `_get_geometry()` returns the
cached build without ever consulting the freshly-recomputed config — so a games-config edit has no
effect until an unrelated trunk change invalidates the cache, or the process restarts. Accepted
because games-config edits are rare (a one-time project setup step) compared to trunk edits (the
thing this whole tool exists to make cheap and frequent); a `serve` restart is a reasonable
workaround for the rare case. Documented here explicitly so it isn't discovered as a surprise later.
...
- **Invalidation ordering / lock hazard**: unchanged from the original design — clear before
  ...
  hazard `app.py`'s own comment already warns about). **Also bump `_generation[0]` in this same
  step** (before clearing, or atomically together — order between the bump and the two ref-clears
  doesn't matter since all three happen with no lock and no other code reads `_generation` except
  the build functions checking it after their own slow work) — this is what makes a build that was
  already in flight when the edit landed discard itself instead of publishing stale data (see the
...
- **A generation-guard test specifically** (review finding — the concurrency test above does not
  exercise this): a build that's artificially made slow (e.g. inject a delay, or use a real
  multi-actor fixture whose `_scene_hashes` re-serialization is measurably non-instant) must be
  in flight when an invalidation fires mid-build, and the test must assert the STALE result is
  discarded, not published — i.e. after the slow build finishes, `_geometry_ref[0]` reflects the
  POST-edit state (rebuilt fresh), never the pre-edit result that thread computed. A test using a
  trivially-fast single-actor fixture cannot exercise this (every thread finishes before or after
  the invalidation, never during it) — the fixture must be slow enough, or the test must inject a
  deterministic delay into the build path, to force the race window open.
...
- A second, joint review (with the explicit-rebuild spec) found the single-atomic-snapshot design,
  ...
  the shape.
- A third review (of the resulting plan, 2026-09-14) found a real, unaddressed race: a slow build in
  flight when an invalidation lands can publish a stale result over the fresh `None`, with no bound
  on how long the staleness persists — the double-checked-locking sketch had no guard against this.
  This revision adds the generation counter (bumped by the same invalidation step that clears both
  refs; a build checks it before publishing and discards+retries if it changed mid-build), documents
  the games-config-needs-restart trade-off this cache introduces as an explicit, accepted decision
  rather than a silent regression, and adds a dedicated generation-guard test (the existing
  concurrency test's fixture is too fast to ever exercise the actual race window).
