...
**Status:** `uprops.py` and `propedit.py` are split and merged (`25a9325`), with the layering gate
and the wheel-packaging fix. The `utexture.py` slice is blocked on a filename collision —
`questions/utexture-decode-name-collision.md`.
...
item therefore discharges two of the four named modules so far — `utexture.py` is blocked, see
Status above; `preview.py`'s split is tracked separately
