...
priority = "p1"
kind = "implement"
summary = "SHIPPED 2026-08-30 (repartition_frontier rewritten, bspcsg.rs): UNATCO nodes/surfs/leaves are now byte-exact for the first time in this item's history (was d=+7 nodes). Root mechanism (found earlier the same day, full arc in the item body): the real editor's per-call bspRepartition reconstructs and correctly merges each subtree, but bspRefresh's real FArray::Remove discards the whole new node structure every single call -- while permanently keeping the Verts/Points it allocated. repartition_frontier now reproduces this exactly: runs the real reconstruction into a throwaway scratch clone, keeps only its Verts/Points growth, discards the node tree and surfs entirely -- model.nodes/model.surfs are never touched. Live-verified: UNATCO verts d=+2443 -> d=+5, points d=+16; stage-count verts growth 44325->54781 (+10456) against the established +10462 target (off by 6, 0.06%). Wanchai stays exact, verts d=+138 -> d=+74. TDD (red before, green after, 85/85). Found and fixed a SECOND real bug along the way: a lighting-bake crash (vert iVertex=-1) in a pre-existing points-compaction pass whose own doc comment had predicted this exact failure mode for a future orphan-vert source. Lighting spot-check against the existing lit golden now completes cleanly for UNATCO for the first time ever (was blocked/STALE) -- geometry/LightMap-record-count exact, 2692/3345 records fully identical, 99.23% shadow-bit agreement; remaining gaps match already-tracked issues in native-light-apply-bake-where-it-stands-and, not new regressions. Full bin/test: 12329 passed, 0 failed. See dev/docs/native-materialize-findings.md for the full technical write-up."
+++
...
## Same day, final follow-up in this chain: ran the "get the editor's real poly ORDER" live capture.
## The order hypothesis was wrong, but the capture found something bigger — closing the loop with
## the Wanchai no-op finding, and reframing the whole bug one level up. No fix this round.

New live capture (`fbs_root_poly_order.py`, reusing `repart_child_trace.py`'s proven breakpoint
scaffolding) at the editor's real `FindBestSplit` entry (`Editor.dll 0x100338EE`), gated to the FIRST
hit after `child=6108`'s `bspRepartition` entry (the root-level call): **`numpolys=29`, not 40** —
several entries have `nv=5`/`nv=6`, larger than any single original poly. Validated exactly as
directed: `k=21` has `i_link=3633, normal=(1,0,0), dist=508.0` — matching the previously-known editor
root-split plane `(1,0,0,508)` precisely, confirming the capture is correct. So the editor's real
`FindBestSplit` input is the MERGED 29-poly list, not the unmerged 40 native currently feeds it.

Then a cheap, no-capture check: native's OWN `reduce_repartition_polys` function
(`UEDCLI_REPART_ISOLATED_TREE`), applied to the SAME 40-poly input, produces **`merged_count=29`** —
an exact match — with the SAME root plane `(1,0,0,508)`. **Native's merge implementation is already
correct and reproduces the editor's real input exactly; `repartition_frontier`'s default path simply
never calls it.**

**This closes the loop with the Wanchai finding, same day, sibling item
(`wanchai-verts-points-residual-independently`): the editor's real per-call reconstruction is genuine,
correct work (merge included) that then gets discarded anyway — `bspRefresh`'s real `FArray::Remove`
throws away the freshly-built (merged) subtree because it lives past the pre-call `Nodes.Num`
boundary, leaving the OLD unmerged subtree (confirmed unchanged, `node_content_before_after.py`) as
the real persisted output.** So neither "add the merge step" nor "fix the poly order" is the right
fix in isolation — both would still commit SOMETHING into the persistent tree, when the evidence
(9/9 Wanchai calls, this 1 UNATCO call) says the real editor commits NOTHING for calls checked so
far. The open question for a future round: where and why does the tree's real final shape ever
differ from what existed before `repartition_frontier`'s subtree loop began, if no call checked yet
shows a persisted change. **No fix attempted or shipped this round** — this is a bigger, more
...
## SHIPPED, same day, final round: `repartition_frontier` rewritten to reproduce the resolved
## mechanism exactly. UNATCO node/surf/leaf-exact for the first time in this whole item's history.
## A second real bug (a lighting-bake crash) found and fixed along the way.

`repartition_frontier` (`bspcsg.rs`) now runs the real per-call reconstruction (merge always on,
matching `child=6108`'s confirmed `40→29`) into a throwaway `scratch = model.clone()`, keeps only
`scratch`'s `Verts`/`Points` growth, and discards everything else — `scratch`'s new node tree and its
own copy of the parent's rewritten child pointer never touch `model`. `model.nodes`/`model.surfs` are
therefore never modified by this pass at all, matching the confirmed real no-op exactly, while
`Verts`/`Points` permanently grow the way the real editor's discarded reconstruction does. TDD: wrote
`bspcsg::tests::repartition_frontier_is_a_node_noop_but_grows_verts` first (confirmed RED against the
old graft — 11 nodes vs 6 — then GREEN after the rewrite, 85/85 total).

**Live results, verts checked first as directed:** `UEDCLI_BSPCSG_STAGE_COUNTS`: verts grow
44325→54781 across the 209-call sequence (+10456), against the established target +10462 (off by 6,
0.06%) — nodes exactly 6314 at both checkpoints. `regression_gate.py`: UNATCO nodes/surfs/leaves all
`d=+0` for the FIRST TIME in this item's entire history (was `d=+7` nodes); verts `d=+5` (was
`+2443`); points `d=+16`. Wanchai stays exact throughout, verts `d=+74` (was `+138`, also improved).
Full `bin/test` (not `-k bspcsg`): 12329 passed, 0 failed, before and after.

**Second bug, not anticipated going in: a real lighting-bake crash, root-caused and fixed.** The
lighting spot-check (per the coordinator's own instruction, since UNATCO becoming node-exact makes
`native-light-apply-bake-where-it-stands-and`'s STALE UNATCO table finally checkable) crashed:
`lightmap bake: vert iVertex index -1 out of range`. Root cause: `reorder_points_canonical`
(`bspcsg.rs`, a pre-existing points-compaction pass) determined "surviving points" by walking only
`surf.p_base` and node-REACHABLE `vert.i_vertex` ranges — its own doc comment had predicted this
exact failure mode: "if a future `bsp_opt_geom` ever removed/repacked verts, an orphan could name a
dropped point and hit the `-1` sentinel here — re-audit this loop then." This fix's new orphan verts
are exactly that case (unlike `bsp_opt_geom`'s own orphans, which always duplicate an already-live
point, these can name a brand-new point no live node ring uses at all). Fixed by adding a third walk
covering every vert's own point directly, matching the function's own already-stated intent. Verified
against the existing lit golden (`_scratch/native-visgate-2026-08-29/golden_unatco_lit.dx`): the bake
now completes cleanly for UNATCO for the first time ever in this investigation — surfs/nodes/leaves/
vectors/LightMap-record-count all exact both sides, 2692/3345 records fully identical, 99.23%
shadow-bit agreement on grid+run-matched records. Remaining lighting gaps match already-tracked,
unrelated issues in that other item (Lights-entries count, light-run matching) — not new regressions;
re-measuring that item's own UNATCO table in full is its own follow-up, not done here.

Full technical write-up: `dev/docs/native-materialize-findings.md`. `bin/test -k bspcsg` 85/85 (was
84/84), full `bin/test` 12329/0 failed, `regression_gate.py` GATE: PASS. This closes the chain this
item has followed since its creation — the contradiction from the section above is now resolved by
construction (the fix implements exactly the mechanism that contradiction pointed at), not by further
investigation.
