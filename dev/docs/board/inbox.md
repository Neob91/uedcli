...
- `p2` `[OWNER — decide]` **`uned/DeusExAssets` is a COMMITTED symlink to a machine-specific absolute
  path, and it breaks the asset setup on any other machine.** `git ls-files -s uned/DeusExAssets`
