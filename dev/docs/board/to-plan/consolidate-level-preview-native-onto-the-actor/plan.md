...
One offline rasterizer (`preview.py`) serves both `actor diagram` (orthographic) and `level photo`
(freely-posed perspective, whole level). The Rust `render.rs` + the camera/scene half of
`preview_native.py` retire. `level photo` defaults to offline; `--game` opts into the faithful tier.
...
1. **`preview.py` — extract a `Projection` seam, NO behaviour change.** Route the existing ortho
   ...
   Pure refactor: every `actor diagram` golden (wire + textured, quad/single/breakdown) stays
...
3. **`cli/rendering.py` — a whole-level offline render entry.** Add `render_level_shots(level, shots,
   ...
   **faithful** `build_geometry_bspcsg` core), then per `ResolvedShot` render with a
   ...
   - **CSG-order sort.** Sort the actor set by `(order_value, name)` before the solve for a trunk
     ...
     `actor diagram`'s textured path so both match `materialize`. **Re-pin** any `actor diagram`
     textured golden whose set was out of order — the only intended change to `actor diagram` output.
...
4. **`cli/parsers/level.py` + `cli/commands/level.py` — flip the default, drop `--native`.** Parser:
   ...
   with `rendering.render_level_shots(...)` + the PNG writes. Update every `level photo --native`
   test to the new surface (`level photo` default / `level photo --game`).
...
6. **Docs (owner-approval gated).** Fold into `architecture.md` "Preview internals" — the default flip
   ...
   design; `docs/usage.md` `level photo` section (new default, no `--native`, `--fov` now default-
   tier); the `actor-preview-parity` `direction/` home extended to the offline `level photo` tier.
...
- **The CSG-order sort changes `actor diagram` textured output** for out-of-order sets — an intended
...
- **Default flip is user-visible.** `level photo` with no flag no longer touches docker/the game;
  tests asserting the old default must flip, and a bare `level photo` on an undecodable-texture
  level now exits 2 (batch) instead of rendering through the engine.

## Tests (spec "Tests")

- Step 1: `actor diagram` goldens byte-identical (the refactor gate).
