...
> (`docker/Dockerfile`, `bin/_dev-run.sh`) — they exist only in git history. The reason: uedcli
...
pure-Python change still runs the suite. The native tests `importorskip("uedcli_native")` and
...
`ln -s "$PWD/bin/uedcli" ~/.local/bin/uedcli`.
...
with the Python runtime, Pillow **and the compiled `uedcli_native` extension** baked in, so an end
user runs `uedcli` with nothing installed — no Docker, no Python, no cargo. Building it (and
