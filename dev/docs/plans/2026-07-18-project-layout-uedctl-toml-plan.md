# Plan: project layout reorg — free `uedctl.toml` at the repo root

*2026-07-18. Ephemeral (see docs rules). Implements
`specs/2026-07-17-project-layout-uedctl-toml.md`; the durable record is `decisions.md`
...
- **Four slices, each one commit, suite green (`bin/test`) at every boundary.** Slice 1 is the
  flag-day: after it lands, uedctl only recognizes the new layout, and the LUM repo is migrated in
...
## Slice 1 — the new project model + `uedctl.toml` cutover + LUM migration (flag-day)
...
- `_project_toml_path` → look for **`uedctl.toml`**; `load_project` accepts the root dir or the
  file. `_PROJECT_KEYS` loses `id`/`name` (they become unknown-key `ConfigError`s).
- `walk_up_project_dir` → `walk_up_root`: first ancestor **containing an `uedctl.toml` file**
  (nearest wins); no child-dir scan, no ambiguity branch. A malformed OR unreadable (OSError)
  `uedctl.toml` found by walk-up is a hard `ConfigError` (spec §4).
...
- `state_dir(root, *, create=False) -> <root>/.uedctl`. On create: `mkdir` catching
...
- The stash register is built at **TWO** sites: `dispatch._resolve_stash_register`
  ...
  (dispatch.py:989). Factor both through the one helper → `<root>/.uedctl/stash/`.
- `preview_game.py:124` delivery dir: `<root>/.uedctl/preview/`.
- `level_select`: the whole public API (`set_selected`/`get_selected`/`resolve_level`) takes
  ...
  `<root>/.uedctl/current-level`; retire level_select's own `tmp/.gitignore` writer
...
- The generic no-project error exists at **TWO** sites: `dispatch._resolve_project`
  ...
  latter delegate to `_resolve_project` so the new message ("no uedctl.toml found here or
...
- `cli.py:143` `--project` help: "project root (or its uedctl.toml)".

**LUM migration (same commit):**
- Write `<repo>/uedctl.toml`: `game`, any `paths` carried over, `maps = "uedctl/maps"`,
  `prefabs = "Prefabs"`, no `catalog` key; **drop `name`** (decision §7).
- Delete `uedctl/config.toml`, `uedctl/tmp/` (throwaway), `uedctl/.gitignore`.
- Do **NOT** touch `<repo>/.uedctl/` or the repo-root `.gitignore` yet — locks/staging still live
  there until slice 2.
...
ONCE in `conftest.py` (write a minimal `uedctl.toml`, point `UEDCTL_PROJECT` at it) — it is the
replacement seam for the env overrides retired in slice 3, but it must ALSO keep setting
`UEDCTL_REPO_ROOT` until slice 3 lands (verbs still on `state_root()`/`host_repo_root()` can't
...
`grep -rl "config.toml\|project_dir" uedctl/tests/`, at `98f5f2b24`: `test_project_show.py`,
...
- Thread the resolved `Project` (or its `state_dir`) as an explicit parameter from `dispatch`
  ...
  - `qualify.py` — export tmp ONLY (qualify.py:320, 345). Its `root = host_repo_root()`
    ...
    the project into it.
  - The texture-verb `lock_dir` (dispatch.py:578) → `<root>/.uedctl/locks/` — but the derivation
    ...
    `catalog_dir.parent / ".uedctl" / "locks"` fallback (make `lock_dir` required) — it is only
    coincidentally right under the new layout.
- Retire `repo_paths.state_root()` (nothing imports it after this slice).
- **LUM migration remainder:** delete the old `<repo>/.uedctl/` (throwaway) and drop the
  `.uedctl/` line from the repo-root `.gitignore` (self-ignore covers the recreated dir).
- Tests: `test_editor.py`, `test_editor_ini.py` (monkeypatches `editor.state_root` at :33,52),
  ...
  texture lock tests; assert locks/staging land under `<root>/.uedctl/`.
...
- **New `tool_assets.py`:** `tool_root() = Path(__file__).resolve().parent.parent` (→
  `Tools/uedctl/`), plus named accessors `uned_dir() = tool_root() / "uned"` (compose dir +
...
- **cwd-relative CLI paths:** `dispatch.py:269` (relative `--out` joins cwd, not
  ...
  legacy remap goes.
- **Retire the env overrides** `UEDCTL_REPO_ROOT` / `UEDCTL_PREFAB_DIR` / `UEDCTL_TEXTURE_CATALOG`
  (grep-clean); the conftest fixture drops its `UEDCTL_REPO_ROOT` leg (slice 1 note).
  ...
  today) with `UEDCTL_TEST_INSTALL` as override, and the module-level call in `test_uprops.py`
...
- While touching `preview_game.py`: route `game-preview.lock` (preview_game.py:379) through
  `config._user_home()` so `$UEDCTL_HOME` is honored (spec §5).
- Tests: `test_repo_paths.py` shrinks to `tool_assets` tests; `test_stub.py`,
  `test_packages.py`, `test_editor.py` drop the `UEDCTL_REPO_ROOT` seam; `test_dxpkg.py`,
...
- `architecture.md`: grep-sweep for any remaining `<project>/uedctl/`, `config.toml`-as-project,
...
- Tool `CLAUDE.md` / `docs/README.md`: any `uedctl/config.toml` mention.
