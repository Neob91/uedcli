...
**Goal:** Add a flat, multi-valued, uedcli-side `label` dimension to actors (per-actor `labels` sidecar; `actor label add|remove|clear|get`; `find --label`/`--no-label`; `actor add --label`; carrier round-trip), and overhaul `actor duplicate` (inherit labels + always a fresh `dup-<rand>`; `--label` additive; REQUIRED `--by`/`--at` placement).
...
**Tech Stack:** Python 3.12, dataclasses, argparse, pytest via `bin/test`. **Spec:** `dev/docs/specs/2026-07-22-actor-labels.md` (symbol-anchored; read it for rationale). **Template to mirror:** the `folder` dimension — study these before starting: `uedcli/folderlib.py`, `t3dtree.py:200-263` (folder sidecar r/w), `dispatch.py:170-210` (`_reject_nonlevel_target_for_folders`, `_actor_folder`), `dispatch.py:1442-1458` (the delta-write diff + `_loaded_folders`), `_ingest_actor_t3d` `folder_override` (`dispatch.py:1789`), `model.py` `_FOLDER_CARRIER` + `Actor.folder`, `query.py` `list_actors`(`folders`/`no_folder`) + `actor_show_block`.

**Shared rules for EVERY task:** shared checkout on branch `uedcli-impl` with concurrent agents — commit ONLY the named files by explicit pathspec (never `git add .`/`-a`/dir); `git push` after each (rebase your own commits if the remote moved; never force-push/amend). Test from `Tools/uedcli` with `bin/test`. Short imperative commit subjects, no AI attribution. Errors name the offending value + exit 2 — never a traceback.
...
**Files:** Modify `uedcli/model.py` (the `Actor` dataclass — add a field beside `folder`), `uedcli/folderlib.py` (export a segment validator). Test: `uedcli/tests/test_labellib.py` (NEW).
...
- [ ] Test: `validate_segment("lighting")` ok; `validate_segment("a.b")`, `validate_segment("")`, `validate_segment("a/b")` raise. Commit `uedcli/model.py uedcli/folderlib.py uedcli/tests/test_labellib.py`.

### Task 1.2 — `labellib.py` (validate, flat matcher, carrier)

**Files:** Create `uedcli/labellib.py`. Test: `uedcli/tests/test_labellib.py`.
...
- [ ] Carrier: `_LABELS_CARRIER = re.compile(r"^\s*//\s*uedcli-labels:\s*(.+?)\s*$")` and `format_labels_carrier(labels) -> str` returning `"    // uedcli-labels: " + ",".join(sorted(labels))` (comma-joined, sorted). Parsing splits the captured group on `,`, strips, drops blanks → a set.
- [ ] Tests: `validate_label("-x")` raises; `match_label("dup-*", "dup-a1")` True, `match_label("Torch*", "torch3")` True (case-insensitive), `match_label("a?", "ab")` raises (char-class rejected); carrier round-trips `{"b","a"}` → `// uedcli-labels: a,b` → `{"a","b"}`. Commit `uedcli/labellib.py uedcli/tests/test_labellib.py`.

### Task 1.3 — `labels` sidecar in `t3dtree.py` (sorted, atomic, remove-on-empty)

**Files:** Modify `uedcli/t3dtree.py` (`write_actor_tree` ~line 205, `read_actor_tree` ~line 261 — beside the `folder` block). Test: `uedcli/tests/test_t3dtree_labels.py` (NEW).
...
**Files:** Modify `uedcli/dispatch.py` (`TrunkLevelSource.__init__`/`load`/`save`, ~lines 1385-1458). Test: `uedcli/tests/test_labels_delta_write.py` (NEW).
...
- [ ] Test: `parse_t3d` of a block containing `// uedcli-labels: a,b` yields `actor.labels == {"a","b"}`. Commit.
...
**Files:** Modify `uedcli/query.py` (`list_actors` — add `labels`/`no_label` params mirroring `folders`/`no_folder`; `actor_show_block` — emit the labels carrier and RENAME its `with_folder` param → `with_sidecars`, updating all callers). Test: `uedcli/tests/test_query_labels.py` (NEW).
...
- [ ] Tests: `list_actors(lv, labels=["dup-*"])` returns only dup-labelled actors; `no_label=True` returns only unlabelled; `actor_show_block(a, with_sidecars=True)` emits `// uedcli-labels:` for a labelled actor and round-trips via `parse_t3d`. Commit `uedcli/query.py` + all touched callers + the test.

### Task 2.2 — `actor label add|remove|clear|get` (mirror `_actor_folder`)

**Files:** Modify `uedcli/cli.py` (a `label` subparser mirroring `folder`), `uedcli/dispatch.py` (`_actor_label` handler mirroring `_actor_folder`; a `_reject_nonlevel_target_for_labels` mirroring the folder guard; wire into `dispatch`). Test: `uedcli/tests/test_labels_verbs.py` (NEW).
...
- [ ] Tests: `add` unions, `remove` subtracts (missing = no-op), `clear` empties + removes the file, `get` output shape (+`--json`); a bad `--label` (`-x` or `a.b`) or unknown name leaves ALL untouched (exit 2 naming it); `-` reads stdin, empty stdin → no-op exit 0; each mutating verb echoes touched Names to stdout; `--tree stash/x` → exit 2 "trunk only". Commit `uedcli/cli.py uedcli/dispatch.py uedcli/tests/test_labels_verbs.py`.

### Task 2.3 — `find --label` / `--no-label` + `actor add --label`

**Files:** Modify `uedcli/cli.py` (`find` gains `--label` (append) + `--no-label` in the existing folder mutex group's sibling; `actor add` gains repeatable `--label`), `uedcli/dispatch.py` (the `find` handler passes `labels`/`no_label` to `list_actors` + validates `--label` patterns; `_ingest_actor_t3d` gains a `labels_override` param SET BY THE ADD HANDLER ONLY). Test: extend `uedcli/tests/test_labels_verbs.py`.
...
- [ ] Tests: `find --label 'dup-*'` matches; repeated `--label` ORs; `--label` ANDs with `--folder`; `--no-label`; a `?`/`[` pattern → exit 2; `actor add --label lit` stamps every added actor; an explicit `--label` overrides a `// uedcli-labels:` carrier; the carrier alone round-trips via `show | add -`. Commit the 3 files.
...
**Files:** Modify `uedcli/cli.py` (the `duplicate` parser: a REQUIRED mutually-exclusive `--by`/`--at` group; repeatable `--label`), `uedcli/dispatch.py` (the `duplicate` handler + `_ingest_actor_t3d` `labels_add` channel + the translate). Test: `uedcli/tests/test_duplicate_labels.py` (NEW).
