# Plan: uedctl global CLI over multiple projects

Implements `specs/2026-06-29-uedctl-global-cli-projects-design.md`. **Ephemeral** — folds into
`architecture.md`/`decisions.md` on landing.

## Strategy: additive, behind a legacy fallback; safe slices first

The spec mandates a legacy fallback (§9.1): until a `uedctl.toml` exists, the current
repo-bound resolution stays. Every slice below is built **additively** so that, with no
`uedctl.toml`/`config.toml` present, behavior is **byte-identical to today** and the existing test
...
- **CRITICAL — tier-3 (session's recorded project id) needs a session-schema change** that lives in
  ...
  until F. Flagged.
- **Env precedence (defined):** `--project` > `UEDCTL_PROJECT` (NEW env, distinct from
  `UEDCTL_SESSION`) > tier-3 (deferred) > walk-up > None. The existing `UEDCTL_REPO_ROOT` stays the
  legacy repo-root locator (used only on the legacy path). `UEDCTL_TEXTURE_CATALOG`/`PREFAB_DIR`
...
| F | **Session store relocation** to `~/.uedctl/projects/<id>/` | no | **high** (touches store/apply/locks) | **NO — supervised** |
| G | **`uedctl migrate`** | **gated #1** | high (destructive) | **NO** |
...
**Goal:** `pipx install` (or `pip install -e`) puts `uedctl` on `$PATH` with Pillow bundled; the
`No module named PIL` host-interpreter bug disappears.

**Steps:**
1. Add `Tools/uedctl/pyproject.toml`:
   - `[project]` name `uedctl`, version, `requires-python = ">=3.12"`, `dependencies = ["Pillow>=11"]`.
   - `[project.scripts] uedctl = "uedctl.cli:main"` (entry point → existing `cli.main`).
   - `[tool.setuptools] packages = ["uedctl"]` (or `find`), exclude `tests`.
2. Confirm `uedctl/__main__.py` + `cli.main()` need no change (entry point reuses `main`).
3. README/architecture note: install via `pipx install ./Tools/uedctl` (editable for dev:
   `pip install -e`).

**Tests/verify:** `python -m build` (or `pip install -e .` in a throwaway venv) succeeds; `uedctl
...
**New module `uedctl/config.py`:**
- `@dataclass(frozen=True) UedctlConfig` — parsed `~/.uedctl/config.toml`: `substrates: dict[str,
  Substrate]`, `defaults`. `Substrate{paths, catalog, image}`.
- `@dataclass(frozen=True) Project` — parsed `<project>/uedctl.toml`: `root, id, name, substrate,
  paths, catalog`.
- `load_user_config(path=None) -> UedctlConfig | None` — `tomllib`; `None` if absent. `--config`
...
- `load_project(dir_or_toml) -> Project` — read `uedctl.toml`; validate; relative `paths`/`catalog`
...
- `resolve_project(*, project_flag, env, session_project_id, cwd) -> Project | None` — the §4
  precedence (1 flag → 2 env → 3 session-recorded id → 4 walk-up `uedctl.toml` → else None).
...
**New `uedctl/texture_store.py`:**
- `store_root()` = `~/.uedctl/textures/` (overridable). `package_dir(pkg_hash, schema)`,
...
- `project init [--name] [--substrate]` (mint `uedctl.toml` + uuid), `project ls/show/rm`.
- `config get/set` (edit `~/.uedctl/config.toml`).
...
- **F (session relocation):** `repo_paths.session_store_root` → `~/.uedctl/projects/<id>/store`.
...
- **G (`uedctl migrate`):** destructive move of in-repo `.uedctl/` → `~/.uedctl/`. **Gated on
...
After each built slice: `cd Tools/uedctl && .venv-uedctl/bin/python -m pytest uedctl -q` (offline
