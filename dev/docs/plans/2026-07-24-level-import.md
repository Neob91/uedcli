...
**Prior art to study before starting** (do NOT reinvent): `uedctl/uprops.py` (`render_default_tag`, `_decode_struct_bin`(`_at`), `resolve_class_defaults`, `Prop`, `_decode_property` array `type_ref`); `uedctl/upackage.py` (`read_property_tags`/`PropertyTag`, `object_path`/`object_class_name`/`name_of_ref`, `read_compact_index`, `Package`); `uedctl/native/actor_write.py` (`state_frame`, `write_prop`, `write_fpoly`/`write_upolys_body` — the encode mirrors); `uedctl/native/umodel.py` (`parse_model_body`, `field_0x54`); `uedctl/classindex.py` (`descends_from`, `ENGINE_ACTOR`, `qualify_and_validate`); `uedctl/dispatch.py` (`_level_create` ~2109, `_resolve_level_source`, `_tree_flag` in `cli.py`); `uedctl/stash_register.py` (`write_stash`), `uedctl/stashlib.py` (`write_tree_box`, `referenced_packages`); `uedctl/trunk.py` (`write_level`/`append_rank`); spikes `dev/docs/spikes/2026-06-27-decontainerize-uedctl/07-native-actor-bodies.md` (+ `harness/native_dx_actors.py`) and `.../harness/upolys_decode.py`.

**Shared rules for EVERY task:** shared checkout on branch `uedctl-impl` with concurrent agents — commit ONLY named files by explicit pathspec (never `git add .`/`-a`/dir); `git push` after each (never force-push/amend/rebase already-pushed). Test from `Tools/uedctl` via `bin/test`. Short imperative commit subjects, no AI attribution. Every CLI error names the offending value + exit 2 — never a traceback. Keep the schema-free hash path (`normalize`) untouched.
...
**Files:** Create `uedctl/mapimport.py` (seed) with `_skip_state_frame(pkg, export) -> int`. Test: `uedctl/tests/test_mapimport_stateframe.py` (NEW).
...
- [ ] Test: `state_frame(cls) + write_props(...)` bytes → `_skip_state_frame` lands exactly at the property `None`-terminated list start (`read_property_tags` then parses cleanly); Node=0 vs Node≠0 both handled. Cite Spike 07 in a docstring. Commit `uedctl/mapimport.py uedctl/tests/test_mapimport_stateframe.py`.

### Task 1.2 — FPoly/UPolys decode (`mapimport.decode_upolys`)
**Files:** `uedctl/mapimport.py` (`decode_fpoly`, `decode_upolys`). Test: `uedctl/tests/test_mapimport_upolys.py` (NEW).
...
**Files:** Modify `uedctl/uprops.py` (`Prop` + `_decode_property`). Test: `uedctl/tests/test_uprops_array_inner.py` (NEW).
...
- [ ] Test: a class with `var array<int> Foo` / `var array<Object> Bar` → `Prop.array_inner.kind` correct. Commit `uedctl/uprops.py uedctl/tests/test_uprops_array_inner.py`.

### Task 1.4 — Dynamic-array value decode (`mapimport`)
**Files:** `uedctl/mapimport.py` (array branch). Test: `uedctl/tests/test_mapimport_array.py` (NEW).
...
**Files:** `uedctl/mapimport.py` (`render_prop`, wrapping `uprops.render_default_tag`). Test: `uedctl/tests/test_mapimport_render.py` (NEW).
...
**Files:** `uedctl/mapimport.py` (`render_actor`). Test: `uedctl/tests/test_mapimport_actor.py` (NEW).
...
**Files:** `uedctl/mapimport.py` (`import_map`). Test: `uedctl/tests/test_mapimport_importmap.py` (NEW).
...
**Files:** Modify `uedctl/dispatch.py` (`_resolve_import_dest`). Test: `uedctl/tests/test_import_dest.py` (NEW).
...
- [ ] Tests: level dest writes a readable trunk; stash dest writes a readable stash entry; existing box w/o `--overwrite` → exit 2; `prefab/x` → exit 2; two same-named actors → exit 2 naming it. Commit `uedctl/dispatch.py uedctl/tests/test_import_dest.py`.

### Task 4.2 — CLI + dispatch handler + strict validation
**Files:** Modify `uedctl/cli.py` (`level import` subparser), `uedctl/dispatch.py` (handler). Test: `uedctl/tests/test_import_verb.py` (NEW).
...
- [ ] Tests: import a fixture `.dx` into a level and into a stash (names on stdout, summary on stderr); non-package file → exit 2; unresolved ref (fixture referencing an off-path class) → exit 2 naming it; no project → exit 2. Commit `uedctl/cli.py uedctl/dispatch.py uedctl/tests/test_import_verb.py`.
...
**Files:** `uedctl/tests/fixtures/import/` (a committed small retail `.dx` + a PRE-COMMITTED UCC `MyLevel.T3D` artifact / its canonical hash). Test: `uedctl/tests/test_import_goldens.py` (NEW).
...
**Files:** `uedctl/tests/test_import_integration.py` (NEW, `-m integration`).
