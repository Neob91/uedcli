...
Traps from reverse-engineering and live-debugging the Deus Ex game runtime headless
under wine. Confidence: ✅ live-verified this repo · 🔬 disasm/observed · 📖 inferred.
...
Two separate substrates — do not mix their addresses:
...
- Use `dev/docs/spikes/2026-07-15-native-materialize/harness/dishere.py`:
  ...
  (On-disk file offset == RVA for these DLLs.) `pe.py` has `pe.read_at_va`,
  `pe.image_base`, `pe.disasm`, `pe.exports`.
- `adis.py` has an argv bug and silently prints nothing — prefer `dishere.py`.
- Finding a big function by its struct-offset refs: scan `.text` for a window that
  references many distinct struct offsets (e.g. `UModel::Serialize` touches Nodes `+0x58`,
...
- `WINEDEBUG=+seh` logs first-chance exceptions. Filter gotcha: wine writes
...
Reading a CPU register at a `Render.dll` fault in the headless game is hard. In order of usefulness:
...
- Boots are ~50% flaky due to an intermittent wine boot-deadlock (root cause of "nl=0 /
  ...
  esync/fsync) but it still fires intermittently. Detect it fast and retry:
...
- A dark lightmap record (`iLightActors=-1`) skips the whole light loop → renders fine. A
  far-away light produces dark records and is excluded from the per-surface relevant-light list,
  so it is never processed — a "NativeDark" repro built with a far light does not exercise
  `AddLight` at all. Don't treat "NativeDark renders" as proof that `AddLight(yourLight)` works —
  build the dark repro with an in-range light whose records you force dark another way.
- Collision (`Engine.dll @ 0x10300000`): `UModel::LineCheck`/`PointCheck` gate solidity on
  ...
  node `+0x34` (stride 1), `iLeaf[side]` an i32 at node `+0x38` (stride 4). (Full detail:
