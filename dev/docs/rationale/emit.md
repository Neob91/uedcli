# T3D emit — why the trunk states what it states

Engineering decisions about `uedcli/emit.py`, the module that turns the in-memory model
(`uedcli/model.py`) back into T3D text. It is a **single choke point with four consumers**, which is
what makes a decision here load-bearing:
...
A spelling emitted here therefore appears on all four at once — including on both sides of the
...
- **The geometry half of the post-verify is a WHOLE-TEXT compare** — `normalize._geometry_text`
  ...
  is a difference wherever it sits. It does not even *report* as "a pan differs": the human
  diagnostic (`verify._first_diff`) pairs the two texts up by line number, so one extra line shifts
  every following one and the message names a bogus *vertex* mismatch. This shipped as a hard bug: after `brush poly align`, which writes its seed's pan onto
  every target face, a freshly built brush (whose faces carry no `Pan` at all) gained
  `Pan U=0 V=0` on each aligned face, and the next `level materialize` aborted with
  `post-verify mismatch: … differs in GEOMETRY at line 43` and wrote **nothing** — the whole
  documented `brush poly find … | brush poly align …` → build workflow could not complete.
- **The "never omit a property to mean zero" write-side rule does not reach here.** That rule
  ...
  "default" is zero, unconditionally, so the two spellings are genuinely interchangeable on import
  and omitting one cannot mean anything else. The same reasoning already governs `Flags` in the same
...
- **An existing trunk that already holds `Pan U=0 V=0` is rewritten the next time anything saves that
  ...
  content change — but it is unexplained if you do not know this, hence the note.
