...
## Two rasterizers, kept — the stdlib-only path is worth the duplication

`preview.py` rasterizes orthographically in pure Python (z-buffer, per-face mip, affine UV,
occlusion, on-face decals); `uedcli_native.render_frame` rasterizes perspective in Rust for
`level preview --native`. Both fill a textured polygon, and they stay separate.

**Why it is this way.** `actor preview --faces wire` is the tier that still works when nothing else
does — no game install, no compiled extension, no container. That holds only because `preview.py`
imports nothing outside the stdlib and returns PPM/P6 bytes in memory. Merging the two rasterizers
means either routing `wire` through the Rust extension, which destroys that property, or porting
perspective into Python and retiring working Rust. The duplication is real and is the cheaper of the
three.

**Rejected.**

- **One rasterizer behind two projection front-ends.** The shape the unify board item originally
  proposed. Killed on the `--faces wire` dependency above (owner, 2026-08-22).

**Refs.** `uedcli/preview.py` module docstring (the stdlib-only contract);
`uedcli/preview_native.py` `render_shots`; board item
`unify-the-logic-behind-level-preview-native-and`.

## One CSG core across both preview tiers — `build_geometry_bspcsg`

`solve_world_surfaces` (ortho `--faces textured`) and `build_scene` (`level preview --native`) call
two different Rust cores. Both converge on `build_geometry_bspcsg`.

**Why it is this way.** `bspcsg` is the incremental `bspBrushCSG` port, the later of the two, already
materialize's default, and the only one the byte-parity work measures. Leaving the tiers split lets
one trunk carve two ways with nothing guaranteeing they agree. The switch costs no preview output:
`_brush_inputs` and `_mover_actor_world_polys` both call `_reject_scaled`, so the native tier already
refuses every scaled brush before the coarse core sees one — its scale support is unreachable from
preview and matters only to materialize, which picks its core separately.

**Rejected.**

- **Both tiers on the coarse `build_geometry`.** Moves the textured tier off the faithful core and
  away from the parity work, buying only scale support that is gated out anyway.
- **Keeping the split as deliberate.** Costs a reader every time two previews of one level disagree.

**Refs.** `uedcli/preview_native.py` `solve_world_surfaces`/`build_scene`/`_reject_scaled`;
`dev/docs/architecture.md` (materialize's default core); board items
`unify-the-logic-behind-level-preview-native-and`, `bspcsg-core-apply-scaled-brushes`.
