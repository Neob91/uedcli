...
A project is identified by a **free-standing `uedcli.toml` at the repo root**, exactly as
`pyproject.toml` or `.git` identifies a tree. The directory containing that file **is** the project
root. There is no `uedcli/` subdir, no project `id`, no registry, and no scaffold verb — **the root
...
- **A `<project>/uedcli/config.toml` project dir with the root as its parent** — an indirection that
  forced a parallel tree beside the user's own dirs.
- **A tracked `.uedcli/` dotdir holding the project's config** — a hidden dir for visible,
...
- **Central per-project state under `~/.uedcli/projects/<id>/`** — the *sole* reason a project
