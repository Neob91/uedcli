...
```
uedctl brush poly set Panel:2 --texture DeusExDeco.Rivet --target prefab/door
uedctl actor move Panel --by 0,0,16 --target prefab/door
```
...
- Prefab library root resolves the usual way (`prefab_library_root()` + `UEDCTL_PREFAB_DIR`); content
