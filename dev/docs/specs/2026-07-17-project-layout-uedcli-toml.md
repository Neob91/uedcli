...
Today a **project** is a subdirectory (conventionally `uedcli/`) holding a `config.toml`, found by
scanning each ancestor's *child dirs* for a project-schema config; the **project root** is that
dir's *parent*, and everything uedcli manages (`maps/`, `texture-catalog/`, `prefabs/`, gitignored
`tmp/`) nests inside the project dir. After this change a project is simply **a repo with a
free-standing `uedcli.toml` at its root** (à la `pyproject.toml`): the dir containing `uedcli.toml`
IS the project root, the file declares (as relative paths, with conventional defaults) where each
managed dir lives — so uedcli can point at a repo's *existing* `Maps/`/`Prefabs/` instead of a
parallel tree — and all machine-local throwaway state moves into one in-repo, gitignored,
**self-ignoring** `.uedcli/` beside it. The per-user `~/.uedcli/` is untouched. The legacy
...
- **project root** — the directory containing `uedcli.toml`. It is the project's identity (a path;
...
- **managed dirs** — the dirs uedcli reads/writes for a project: the **maps dir** (T3D trunks, one
  `<level>/` per level), the **prefabs dir** (durable prefab library), the **texture-catalog dir**
  (tracked classification manifests).
- **state dir** — `<root>/.uedcli/`: machine-local, never-tracked scratch (stash, locks, staging,
  delivered preview maps, the selected-level pointer). Derivable/throwaway; safe to delete.
- **per-user home** — `~/.uedcli/` (or `$UEDCLI_HOME`): the `[games.*]` config (`config.toml`) and
  the cross-project derivable `cache/{textures,stubs}`. **Unchanged by this spec.**
- **tool-install assets** — files that belong to the uedcli *installation*, not to any project:
  the docker-compose dir + UED22 substrate (`Tools/uedcli/uned/`), `Tools/umodel_win32`. See §6a.

## 3. `uedcli.toml` — schema
...
maps = "uedcli/maps"                 # optional, default "maps"
...
- **Dropped keys:** `id`, `name` (the registry they served is dead — 2026-07-05 14:58 §4). An
  `uedcli.toml` containing them → the standard unknown-key `ConfigError`.
- **Schema guard kept:** a `[games.*]` table inside `uedcli.toml` (or a `game` key inside
  `~/.uedcli/config.toml`) is a named `ConfigError` — the two files keep mutually-exclusive
...
1. **`--project PATH`** — a path to the project **root dir** or to the **`uedcli.toml` file
   itself**. (Bare *names* stay an error naming the registry's absence, as today.)
2. **`UEDCLI_PROJECT`** — same handling.
3. **Walk-up:** from cwd toward the filesystem root, the **first ancestor directory containing an
   `uedcli.toml` file** is the project root. Nearest wins (nested projects shadow outer ones,
   ...
   disappears (an ancestor has at most one `uedcli.toml`).
4. None found → `None`; verbs that need a project keep their clean exit-2 `_ProjectError`, whose
   generic message now names `uedcli.toml` ("no uedcli.toml found here or above …").

`~/.uedcli/` can never be mistaken for a project: its config is named `config.toml`, and the
schema guard rejects a project-shaped file there anyway. A **malformed or unreadable**
`uedcli.toml` found by walk-up is a **hard `ConfigError`** (both cases — a permission-denied
...
## 5. The `.uedcli/` state dir

All machine-local project state converges on `<root>/.uedcli/`:

| Entry | Holds | Today |
|---|---|---|
| `.uedcli/stash/<id>/` | stash register entries | `<project>/tmp/stash/` |
| `.uedcli/preview/` | hash-named delivered maps for `--game` preview | `<project>/tmp/preview/` |
| `.uedcli/locks/` | editor/target `flock`s *(texture flocks moved post-build to the catalog-adjacent `<catalog>/.locks/` — decisions.md 2026-07-18 07:53)* | `<repo>/.uedcli/locks/` (repo_paths) |
| `.uedcli/tmp/` | materialize/qualify staging, override inis, docker-cp temps, `keep_build` rejects | `<repo>/.uedcli/tmp/` (repo_paths) |
| `.uedcli/current-level` | the machine-local selected-level pointer (`level select`) | `<project>/tmp/current-level` (level_select.py) |

- **Self-ignoring:** whenever uedcli creates `.uedcli/`, it also writes `.uedcli/.gitignore`
...
- The old `repo_paths.state_root()` was *already* `<repo>/.uedcli/`, so for a root-level project
  the locks/tmp paths are byte-identical — what changes is **derivation** (from `uedcli.toml`
  discovery, not the CLAUDE.md-marker repo walk) and that stash/preview/current-level join them
  there.
- Atomicity note: materialize staging stays under `.uedcli/tmp/` precisely so `os.replace` onto an
  in-repo `--out` stays same-filesystem (the existing EXDEV fallback covers the rest).
- The per-user `~/.uedcli/game-preview.lock` (warm game-preview container serialization) stays
  per-user — the warm container is per-*user*, not per-project. While touching it: route the path
  through `config._user_home()` so it honors `$UEDCLI_HOME` like everything else per-user.
- Degenerate edge, accepted as harmless: a project rooted at `$HOME` makes the state dir literally
  `~/.uedcli/` — the state subdirs and a `*` `.gitignore` land beside the games `config.toml` and
...
| Retiree | Replacement |
|---|---|
| `host_repo_root()` (CLAUDE.md-marker walk-up) + `UEDCLI_REPO_ROOT` | project root from `uedcli.toml` discovery for PROJECT state; the package-relative anchor (§6a) for TOOL assets |
| `state_root()` | `<root>/.uedcli/` via the resolved project |
| `prefab_library_root()` + `UEDCLI_PREFAB_DIR` | `uedcli.toml` `prefabs` key (default `prefabs/`) |
| `texture_catalog_root()` + `UEDCLI_TEXTURE_CATALOG` | `uedcli.toml` `catalog` key (default `texture-catalog/`) |
...
- **Test seam for the retired env vars:** tests that used `UEDCLI_REPO_ROOT`/`UEDCLI_PREFAB_DIR`/
  `UEDCLI_TEXTURE_CATALOG` for isolation switch to a tmp-dir project fixture (write a minimal
  `uedcli.toml`, point `UEDCLI_PROJECT`/`--project` at it) — the same seam production uses.
- Survivors: `stub_cache_root()`/`texture_images_root()` (already per-user `~/.uedcli/cache/`,
  ...
  baked install) move behind a test-only env (`UEDCLI_TEST_INSTALL`, resolved in conftest), out of
...
assets** — `editor._compose_dir()` (`Tools/uedcli/uned/`, the docker-compose dir + pre-bake inis),
...
**package-relative**: resolved from the installed `uedcli` package's own location
(`Path(__file__)` → the source tree's `Tools/uedcli/`), via one new helper (e.g.
...
- `load_project(path)` accepts the root dir or the `uedcli.toml` path; `_project_toml_path` looks
  for `uedcli.toml` (not `config.toml`).
...
- `dispatch._resolve_project`'s generic error message names `uedcli.toml` (§4.4).

## 8. Migration (hard cutover, LUM in the same change)

- **No dual support** (decision §4). If walk-up finds no `uedcli.toml` but an old-layout
  `<child>/config.toml` project *would* have matched, the `_ProjectError` message says so
  explicitly: `found old-layout project dir <path>/uedcli/ — this layout is retired; move its
  config.toml to <path>/uedcli.toml (see docs) and delete its tmp/`. (Detection: the same cheap
...
- **LUM migrates in the same change** (decision entry §7 — zero-move option):
  - `uedcli/config.toml` → `<repo>/uedcli.toml`, **dropping the retired `name` key**, gaining
    `maps = "uedcli/maps"` (the trunks stay in place; the old `uedcli/` dir lives on as a plain
  ...
  - `uedcli/tmp/` is throwaway — deleted, not moved (stash is scratch by definition), along with
    `uedcli/.gitignore` (whose only job was ignoring `tmp/`).
  - The pre-existing `<repo>/.uedcli/` (old `state_root()` output) is deleted — throwaway by
    definition — so its next creation writes the self-ignore file; the `.uedcli/` line in the
...
1. A repo with only `game = "deusex"` in `<root>/uedcli.toml` resolves: root = that dir;
...
3. First state-dir use (`level select`, stash write, materialize, `--game` preview) creates
   `.uedcli/` **with** a `*` `.gitignore`; `git status` in a fresh repo shows only `uedcli.toml`
   and tracked managed dirs.
4. No code path resolves a CLAUDE.md-marker repo root or reads
   `UEDCLI_REPO_ROOT`/`UEDCLI_PREFAB_DIR`/`UEDCLI_TEXTURE_CATALOG`; editor-driving verbs still
...
6. Migrated LUM: full offline suite green; `level materialize`/`preview --game` work against the
   migrated tree.
7. Nested-project case: cwd inside an inner repo with its own `uedcli.toml` binds to the inner
   one.
