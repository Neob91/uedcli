# Spec: uedctl as a global CLI over multiple projects (config, projects, layered assets)
...
> **Reader orientation.** This spec turns uedctl from a tool that *lives inside one content repo and
...
> `~/.uedctl/` layout and the content-addressed texture store; (5) how the shared editor is wired.
...
Today uedctl is **repo-bound by assumption**:
- `repo_paths.host_repo_root()` finds "the project" by walking up for a `CLAUDE.md` + `Tools/uedctl`
...
- Runtime state (`.uedctl/`: sessions, locks, caches) sits at the repo root.
...
|---|---|---|---|
| **Tool** | the `uedctl` Python package | installed once | on `$PATH` (pipx) |
| **Game** (internally "substrate") | a UE1 game's base packages (the "OG Deus Ex" install); the editor is the single shared UED22 image | per-**game**, shared by all its projects | game asset paths declared in `~/.uedctl/config.toml` `[games.*]` |
| **Project** | a content tree whose authored work uedctl manages | per-content-tree | the tree's root; uedctl's home is a **project dir** inside it (conventionally `uedctl/`, marked by its `config.toml`) — see §3.3/§5 |
| **Session** | a unit of in-progress editing that merges into the project's committed trunk | transient; never in the content tree | `~/.uedctl/projects/<id>/` (central) — **but see §9: replacing sessions with git branches is a deferred pivot** |
...
tree; sessions are uedctl's private, transient workspace** (`direction.md`).
...
- **Relative globs anchor on the right base dir:**
  - `~/.uedctl/config.toml` → globs **MUST be absolute** (no project root to anchor to; a relative
...
### 3.2 `~/.uedctl/config.toml` — the per-user base (games)
...
A project is a content tree; uedctl's home inside it is the **project dir** — conventionally
`<project-root>/uedctl/`, but any name (it is identified by holding a `config.toml`, not by its
name). The project dir is tracked, travels with a clone, and holds everything uedctl manages:
...
id    = "0192f3a1-..."                           # uuid, minted by `uedctl project init` (§4)
...
**project** dir/config. The `~/.uedctl/config.toml` home config never false-matches project walk-up
...
my_cool_project/              # PROJECT ROOT — the user's tree (docs, their own stuff, ...)
  ...
  uedctl/                     # PROJECT DIR — uedctl's home; conventionally `uedctl`
...
**`~/.uedctl/config.toml`** (per-user). At least one `[games.*]` table is required. No other tables.

| Key | Type | Req? | Meaning |
|---|---|---|---|
| `[games.<name>].paths` | string (colon-glob, **absolute**) | yes | the game's base package globs |

**`<project-dir>/config.toml`** (per-project, tracked; the project dir is conventionally `uedctl/`).
...
> in-tree, so there is no central `~/.uedctl/projects/<id>/` bucket to key. Project resolution still
...
existing `--session`/`UEDCTL_SESSION` discipline):
...
2. **`UEDCTL_PROJECT`** — ambient default (same path/name handling as tier 1).
...
4. **Walk-up from cwd** — find the nearest ancestor `D` with an **immediate child dir whose
   `config.toml` has the project schema** (conventionally `D/uedctl/`, but any name — identity is the
...
**Project id (DECIDED): a uuid in the project dir's `config.toml`, minted by `uedctl project init`.**
`<id>` keys the central state bucket `~/.uedctl/projects/<id>/`. The uuid lives in the tracked
...
> `~/.uedctl/projects/<id>/`, id stays a uuid (a path id would orphan sessions on move). See §9.

### 4.1 CLI surface (new/changed verbs)

- **`uedctl project init [--name N] --game G`** — create the project dir `<cwd>/uedctl/` and mint its
  ...
  `~/.uedctl/projects/<id>/` (sessions/locks/shots) after confirm, **never the tracked project dir**.
  A project is "registered" the first time uedctl resolves its `config.toml`. **NOTE:** `project
...
- **`uedctl config [get|set] …`** — read/edit `~/.uedctl/config.toml` (per-game paths).
...
- **`uedctl texture gc` / `sync --prune`** (§6.0). All existing verbs get project-scoped overlay
...
## 5. The `~/.uedctl/` layout + the project dir

> **SUPERSEDED — 2026-07-05 (decisions.md 2026-07-05 14:58 UTC).** Machine-local **per-project**
> state is NOT central. It lives **in-tree** under `<project>/uedctl/`: tracked
> `config.toml`/`maps/`/`texture-catalog/`/`prefabs/`, plus a **gitignored `tmp/`** (stash, `flock`s,
> docker-cp/apply temps, shots). **There is no `~/.uedctl/projects/<id>/` and no session store** —
> a level is edited on a **git feature branch** and merged into trunk with `git merge` (§9's
> deferred pivot is now adopted). `~/.uedctl/` holds only per-user state: **`config.toml`** + a
> **`cache/{textures,stubs}`** (content-addressed image + stub caches — shared cross-project,
> derivable, never committed; regroups the `~/.uedctl/{textures,stubs}` below under `cache/`). The
> project-dir tracked layout below stands; the `projects/<id>/` block does not.

The global home is **`~/.uedctl/`** (decisions.md 2026-07-01; the XDG split was considered and
rejected — keep one obvious home dir). The project dir is the **non-dot** `uedctl/`, so `.uedctl`
...
~/.uedctl/
  ...
  textures/                           # content-addressed texture IMAGE cache (§6) — shared, derivable
    ...
    data/<pixel-hash>.png
  stubs/                              # v69 stub cache (was <repo>/.uedctl/cache/stubs/) — derivable
```
...
- The **texture image cache and stub cache are NOT per-project** — content-addressed / derivable,
  shared across projects. A `~/.uedctl` wipe loses only *un-applied sessions* (durable record is the
...
**The project dir** (`<project-root>/uedctl/`, conventional name) holds the tracked, clone-travelling
...
`~/.uedctl/projects/<id>/store/`. `level apply` still writes its durable artifact — the `.dx` / T3D
tree — into the content tree, and does `--git-commit` against that tree's working repo. Per-target /
per-session `flock`s live under `~/.uedctl/projects/<id>/locks/` (target lock keyed on the resolved
`--out` abspath, so two sessions can't race a destination).

## 6. Content-addressed texture image store

The decoded-image store is content-addressed under `~/.uedctl/textures/`, shared per-user across all
projects and games:
...
- **Image files are NEVER in the project** — derived, living in the content-addressed home cache
  (`~/.uedctl/textures/`, §6). Regenerable via `texture sync`; a cache wipe costs only a re-decode.
...
- **At `apply`/materialize, shadowing is HOST-SIDE.** uedctl loads each package with an explicit
...
- **Attraction:** `direction.md` already targets a git-committed T3D trunk with map files as build
  ...
  free; git's per-actor 3-way merge ≈ uedctl's hand-rolled per-actor merge — collapsing the
...
- **Costs / risks:** supersedes the 2026-06-18 model and a lot of built code (real migration); the
  ...
  not semantic (intra-actor property conflicts); how uedctl edits a branch (git worktree per session
...
- **Editor-image bootstrap:** a `pipx`-installed uedctl has no repo, yet the single UED22 image is
  currently *built* from `Tools/uedctl/uned/`. How the installed tool obtains it — bundle the build
...
- **Project registry store:** name-based `--project <name>` needs a name→id index; where it lives (a
  `~/.uedctl/projects.toml` index vs a scan of `projects/*/config`) is **open** (config.py stubs it as
...
- **Migration DROPPED** (decisions.md 2026-06-29 05:18 UTC): no `uedctl migrate`; existing in-repo
  `.uedctl/` + `texture-catalog/` are not carried. The legacy fallback (below) stays so nothing breaks
...
project config, has `Tools/uedctl`" and uses the legacy `host_repo_root`/`substrate_search_dirs` path.
...
| Today | Target |
|---|---|
| `host_repo_root()` walks up for `CLAUDE.md`+`Tools/uedctl` | project resolution per §4 (project-dir `config.toml` marker; root = the project dir's parent) |
...
| `.uedctl/` at repo root (sessions/locks/caches in-tree) | `~/.uedctl/projects/<id>/` (sessions/locks/tmp/shots) + `~/.uedctl/{textures,stubs}` central |
| `.uedctl/textures/<package>/<name>.png` flat, per-repo | `~/.uedctl/textures/{packages/<pkg-hash>.<schema>/index.json, data/<pixel-hash>.png}` content-addressed, shared (§6) |
...
| `UEDCTL_*` env knobs only | per-user `config.toml` + project `config.toml`, env/flags still override (§4) |
| run via `.venv-uedctl/bin/python -m uedctl` | `pipx install uedctl` → `uedctl` on `$PATH` |
...
project dir, texture model), 2026-06-30 06:18 (project dir = conventional `uedctl/`, root = parent),
06-30 18:47 (home `~/.uedctl/`, substrate=game, drop manifest), 06-30 21:07 (config key `game`),
