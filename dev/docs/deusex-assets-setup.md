...
version-69 `.u` the editor loads). To materialize/preview actual maps, and to
...
Deus Ex is a commercial game with no free official source; you supply your own
copy. You only need the files on disk (the game is never run), so any of these works:

- Buy it (~$8–10, already patched). [GOG.com](https://www.gog.com/en/game/deus_ex)
  (DRM-free GOTY Edition) or Steam. Both ship at `1.112fm`, so there is nothing to patch —
  point the script at the installed folder; a DRM-free GOG copy is the easiest to sync from.
- Internet Archive (free download, copyright-gray).
  ...
  GOTY download. It is an unofficial redistribution of a game still sold
  commercially, not a licensed free source.

On the patch: the baseline you want is `1.112fm`, the final official Deus Ex build. GOG/Steam and the
GOTY Edition already ship at `1.112fm`, so none of the sources above needs patching —
which is why `install-deusex-assets.sh` has no patch step. `1.112fm` also added
...
One command takes you from an obtained copy to a wired-up setup. Point it at a SOURCE —
...
`/deusex` mount or boot-time entrypoint Paths wiring (decisions.md 2026-07-14 19:21).
...
versions: textures are package version 61/68, the code `.u` are version 68. Only a patch that
re-versions the packages to something exotic could surprise the stubber; standard patched installs
are fine.
...
If you're starting from the raw retail installer rather than an installed game, point SOURCE at
the installer directory: `install-deusex-assets.sh` detects the `deusex.ace` archive and extracts it
into the working copy. It requires `unace` on `PATH` (e.g. `apt-get install unace`); if it is
...
`.umx` music is not in the retail ACE — its `Music/` carries no `.umx`. An ACE-sourced setup has
