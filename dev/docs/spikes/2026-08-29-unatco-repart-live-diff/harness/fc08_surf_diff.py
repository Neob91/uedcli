#!/usr/bin/env python3
"""Per-brush surf-count attribution for freeclinic08 (native surfs = golden+1).

Same brush-index convention as breadth_gate.py / a51_node_n6_diff.py: native
`BspSurf.i_actor` is a 0-based index into the world-CSG brush `names` list
(set at `bi as i32` in build.rs); golden `BspSurf.i_actor` is an editor
obj-ref resolved via `epkg.name_of_ref`.

Usage: .venv/bin/python fc08_surf_diff.py
"""
import os
import sys
