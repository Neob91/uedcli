...
`_scratch/bspspike/`).
**Why:** the offline-BSP-engine decision (board item `bsp-issue-detector`) named **one gating
...
Proceed with the offline BSP engine (board item `bsp-issue-detector`). The heuristic gate is
