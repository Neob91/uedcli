...
def render_scenario(task_id: str, level: str, frame: str, scen_id: str, scen: dict, out_dir: pathlib.Path):
    ...
    print("rendered", f"{task_id}/{scen_id}", f"({scen['view']} view, {len(scen['members'])} actors)")
...
if __name__ == "__main__":
    for task_id, task in TASKS.items():
        ...
        for scen_id, scen in task["scenarios"].items():
            render_scenario(task_id, task["level"], task["frame"], scen_id, scen, out_dir)
