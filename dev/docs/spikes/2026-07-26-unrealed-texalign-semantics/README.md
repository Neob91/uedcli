...
`Pan` is set to `(0, 0)`. A face failing the guard is left untouched.
...
touched **only `Origin` lines** — 12 of them, on exactly the four brushes whose faces had a non-zero
anchor Z. It is the "make every wall's vertical texture phase agree" operation.
...
`USize`. `PanU` is always 0. All that is *measured* is the write. Since `Pan` is added to the texel
...
substrate swap or a drift in the documented rule trips a red test.
...
each threshold pinned live to a 0.002-wide window around the `.rdata` constant. (`WALLPAN`'s second
guard — `|TextureV.Z| > 0.05` — is confirmed by the main fixture instead: a vertical face whose
`TextureV` was horizontal, `(0.8, −0.6, 0)`, was left alone by `WALLPAN` even though its anchor sat
at `z = 30`.)
...
rounds. That is `quirks.md` "Stability", not a property of this verb.
...
| UnrealEd mode | measured behaviour | uedcli equivalent | verdict |
|---------------|---|---|---
| `FLOOR` | orthographic projection down world Z; anchor `(0,0,d/N.Z)`; `TU = −proj(X̂)`, `TV = −proj(Ŷ)`; stretches on a slope; guard `\|N.Z\|>0.05` | `brush poly align --floor` | **diverges on all three of orientation, anchor and slope handling** — see below |
| `WALLDIR` | unit frame along the face's own horizontal direction, V forced downwards; anchor untouched; guard `\|N.Z\|<0.95` | `brush poly align --wall` | **closest analogue, still diverges** — different basis, different anchor, and uedcli's V can point **up** |
| `WALLX` | projection down world X; anchor `(d/N.X,0,0)`; guard `\|N.X\|>0.05` | — | **no equivalent** |
| `WALLY` | projection down world Y; anchor `(0,d/N.Y,0)`; guard `\|N.Y\|>0.05` | — | **no equivalent** |
| `WALLPAN` | slide the anchor along `TextureV` to world `Z = 0`; axes and pan untouched | — | **no equivalent** (`brush poly set --pan-to/--pan-by` moves the *integer texel* pan, a different quantity) |
| `DEFAULT` | frame regenerated from the polygon winding; unit; anchor untouched | — | **no equivalent, and none wanted** — winding-order dependent |
| `CLAMP` | `DEFAULT` + `PanV = VSize − 1` | — | **no equivalent**; purpose undetermined |
| `ONETILE` | **nothing** | proposed `brush poly align one-tile` | **there is nothing to conform to** — the spec's `one-tile` is a uedcli invention, not a port |
...
still: `FLOOR` stretches by `1/|proj|`, `--floor` does not — and `--floor` refuses a slanted face
outright (`_check_orientation`).
...
6. **Slanted faces.** uedcli's `_check_orientation` rejects a slanted face from both `--wall` and
   `--floor`. UnrealEd accepts one in every mode whose guard it passes, and simply stretches. If
   `align` grows slant support, the projection family is the model to copy.
...
| `guards.py`         | reads the §5 captures; writes `guards.json` |
...
- `uedcli/tests/test_engine_facts.py` — five regressions: `test_texalign_parser_maps_nine_tokens…`,
