"""Pins the UED22 facts the WanChai N31 / N35 / N40 ladder steps turned up (2026-09-05).
...
3. `bspOptGeom`'s `MergeNearPoints(Model, 0.25)` (`Editor.dll 0x33dc0`) remaps `FBspSurf.pBase`
   through the same table it remaps `FVert.iVertex` through. Skipping the surf half kept a
   merged-away point alive under a `pBase`. Fix: `bspoptgeom.rs::merge_near_points`.
...
Run just this file (project rule: never the whole suite):
  TMPDIR=$PWD/_scratch/pttmp .venv/bin/python -m pytest -p no:cacheprovider \\
    dev/docs/spikes/2026-09-03-incremental-actor-parity/harness/test_wanchai_ladder_facts.py
"""
...
def test_mergenearpoints_remaps_surf_pbase_too(pe_mod):
    """`bspOptGeom`'s point weld rewrites BOTH reference arrays (`bspoptgeom.rs::merge_near_points`)."""
    pe = pe_mod
    # Verts loop: walk Model->Verts (count +0x6c, data +0x68, stride 8), remap iVertex.
    assert pe.read_at_va(str(ED), 0x10033ECE, 3).hex() == "3b4b6c"      # cmp ecx,[ebx+0x6c]
    assert pe.read_at_va(str(ED), 0x10033ED3, 3).hex() == "8b5368"      # mov edx,[ebx+0x68]
    assert pe.read_at_va(str(ED), 0x10033EE5, 6).hex() == "8b04868904ca"  # iVertex = remap[iVertex]
    # Surfs loop, immediately after: count +0x9c, data +0x98, stride 0x40, field Surf+8 = pBase,
    # guarded on 0 <= pBase < Points.Num (+0x8c) — the half native used to skip.
    assert pe.read_at_va(str(ED), 0x10033EF3, 6).hex() == "3b8b9c000000"  # cmp ecx,[ebx+0x9c]
    assert pe.read_at_va(str(ED), 0x10033EFB, 5).hex() == "8bd1c1e206"    # edx = ecx << 6
    assert pe.read_at_va(str(ED), 0x10033F00, 10).hex() == "8b83980000008b440208"
    assert pe.read_at_va(str(ED), 0x10033F0E, 6).hex() == "3b838c000000"  # cmp eax,[ebx+0x8c]
    assert pe.read_at_va(str(ED), 0x10033F16, 3).hex() == "8b0486"        # eax = remap[pBase]
    assert pe.read_at_va(str(ED), 0x10033F1F, 4).hex() == "89441a08"      # Surf+8 = eax
