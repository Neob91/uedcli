#!/usr/bin/env python3
"""drive.py — runs INSIDE the container after boot_game.sh reports READY. Drives the spike
link on :7777 to capture three frames of the SAME room + SAME pose, incrementally:
  hud_baseline.png       no clean            -> HUD + first-person weapon SHOW
  hud_generic_clean.png  Clean generic       -> stock fields nulled (myHUD/Weapon/bBehindView/Flash)
  hud_dxdriver_clean.png  + Clean dxhud       -> plus the DeusEx game lever (ShowHud 0)
Each frame is X-grabbed off :99 (SoftDrv window). PNGs land in /work.
The camera: eye at room center (0,0,0) looking +X at the Amark-marked wall (yaw/pitch 0)."""
import socket, subprocess, sys, time

PORT = 7777
POSE = (0, 0, 0, 0, 0)          # x y z pitchUU yawUU — center, look +X
WEAPON = "DeusEx.WeaponPistol"
SETTLE = 0.6
_rid = 0
...
def main():
    wait_possessed()
    s = connect()
    ok, lines = cmd(s, "Give " + WEAPON)
    print("  Give:", ok, b" ".join(lines[-1:]).decode(errors="replace"), flush=True)
    time.sleep(SETTLE)
    pose_and_grab(s, "/work/hud_baseline.png")
    ok, _ = cmd(s, "Clean generic"); print("  Clean generic:", ok, flush=True)
    pose_and_grab(s, "/work/hud_generic_clean.png")
    ok, _ = cmd(s, "Clean dxhud"); print("  Clean dxhud:", ok, flush=True)
    pose_and_grab(s, "/work/hud_dxdriver_clean.png")
    s.close()
