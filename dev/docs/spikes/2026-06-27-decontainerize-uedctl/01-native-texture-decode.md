...
> `Tools/uedctl/uned/DeusExAssets/{Textures,System}` (v61/v68). Do NOT run the harness
> against `uned/UED22/*.u` — those are v69 *stubs* and produce different counts.

## Question

Can uedctl extract textures from `.utx`/`.u` packages in **pure Python**, replacing
the only current path — `UCC.exe batchexport <pkg> Texture pcx Z:\…` run in a wine
container (`uedctl/texture.py`)? This is the explicit "reverse-engineer texture
...
`uedctl/texture.py` is one of the container's reasons to exist (`texture sync` →
...
the format) directly from bytes uedctl already knows how to read (`dxpkg`).
...
- Promotion to `uedctl/` (a `utexture.py` module + wiring into `texture sync`) is a
