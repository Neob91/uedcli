#!/usr/bin/env python3
"""Garage first-divergence (`Brush21`, world-CSG idx 39) offline localization.

`describe`: Brush21's authored shape + world-bbox overlap candidates among the first 39 brushes.
`pairs`: native counts for [candidate, Brush21] 2-brush subsets (and candidate alone), to pick the
minimal live A/B case. Native-only — the editor side of the minimal case is `wg_minimal_golden.py`.
"""
import sys
...
def main():
    ps = PSL.PrefixSearch("06_hongkong_wanchai_garage", CACHE / "maps/06_hongkong_wanchai_garage",
                          Path("/tmp/wg-localize-unused"), CACHE)
    ti = ps.brush_names.index(TARGET)
    ta = ps.level.actors[TARGET]
    tprops = dict(ta.props)
    print(f"{TARGET}: idx {ti}, cls={ta.cls}, polys={len(ta.brush.polys)}, "
          f"CsgOper={tprops.get('CsgOper')}, PolyFlags={tprops.get('PolyFlags')}, "
          f"Rotation={tprops.get('Rotation')}, PrePivot={tprops.get('PrePivot')}, "
          f"MainScale={tprops.get('MainScale')}, PostScale={tprops.get('PostScale')}, "
    ...
    for nm in ps.brush_names[:ti]:
        ...
        if overlaps(bbox(a), tb):
            ...
            print(f"  overlap: {nm} (idx {ps.brush_names.index(nm)}) CsgOper={p.get('CsgOper')} "
                  ...
                  f"MainScale={p.get('MainScale')} PostScale={p.get('PostScale')}")
