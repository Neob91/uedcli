...
WANT_ILINK = int(sys.argv[3])  # dump full rings for this i_link only (vs the order-only original)
...
def main() -> int:
    ...
    try:
        ...
        pre_dump = DUMP_POLYS.replace("__STAGE__", "PREMERGE").replace("__WANT_ILINK__", str(WANT_ILINK))
        post_dump = DUMP_POLYS.replace("__STAGE__", "POSTMERGE").replace("__WANT_ILINK__", str(WANT_ILINK))
