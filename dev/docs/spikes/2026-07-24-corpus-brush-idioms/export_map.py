...
writes it as a uedctl trunk so the offline read/preview verbs (`actor find`, `actor preview`,
...
Resolves the project from $UEDCTL_PROJECT and the game paths from ~/.uedctl/config.toml, exactly like
...
from uedctl import config, container_assets, store_export, trunk, xfer   # noqa: E402
from uedctl.model import parse_t3d                                       # noqa: E402
from uedctl.stub import ephemeral_build_container                        # noqa: E402


def export_to_trunk(host_dx: str, out_level_dir: str) -> int:
    proj_env = os.environ.get("UEDCTL_PROJECT")
    if not proj_env:
        print("set UEDCTL_PROJECT to a dir containing uedctl.toml", file=sys.stderr)
        return 2
    project = config.load_project(proj_env)
    user_config = config.load_user_config()
    if user_config is None:
        print("no ~/.uedctl/config.toml with [games.<name>] paths", file=sys.stderr)
        return 2
