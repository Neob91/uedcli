...
set $tail = $fin - 0x150ac0 + 0x150643
break *$cn
commands
silent
set $t = $ecx
...
def main():
    ...
    try:
        ...
        subprocess.run(["docker", "exec", "-d", container, "bash", "-c",
                        "exec gdb -batch -x /tmp/cn.gdb > /tmp/cn.log 2>&1"], check=True)
        for _ in range(240):
            out = subprocess.run(["docker", "exec", container, "bash", "-c",
                                  "grep -c ORACLE_ATTACHED /tmp/cn.log 2>/dev/null || true"],
                                 capture_output=True, text=True).stdout.strip()
            if out and out != "0":
                break
            time.sleep(0.5)
        else:
            diag = subprocess.run(["docker", "exec", container, "cat", "/tmp/cn.log"],
        ...
        while time.monotonic() < dl:
            n = int(subprocess.run(["docker", "exec", container, "bash", "-c",
                                    "grep -c '^CN ' /tmp/cn.log 2>/dev/null || true"],
            ...
            if n != last:
                ...
                break
            time.sleep(1.0)
        subprocess.run(["docker", "cp", f"{container}:/tmp/cn.log", str(OUT)], check=True)
