...
Two jobs (see `dev/docs/architecture.md` "Native CSG build performance"):
  ...
  time   — time `uedcli_native.build_geometry` over UNATCO brush-count subsets, isolating the
...
CASTLE = "/home/neob91/Games/LutrisDX/drive_c/DX/LUM/_scratch/castle/uedcli/maps/foobar"
UNATCO = "/home/neob91/Games/LutrisDX/drive_c/DX/LUM/_scratch/unatco/uedcli/maps/unatco"
...
def cmd_time(counts):
    for n in counts:
        ...
        t0 = time.time()
        built = uedcli_native.build_geometry(bs)
        dt = time.time() - t0
