...
| **[direction/](direction/README.md)** | "What are we building **toward**?" — what **Andrzej** decided: product intent AND process rulings | one doc per topic, **revised in place** (no history — git keeps that) | **agents may NEVER write it without his explicit yes** (`CLAUDE.md` "Direction docs") |
...
| **[board/inbox.md](board/inbox.md)** | "What's noticed but not yet sorted?" (head of stream — captures ideas/bugs/chores, AI flags for Andrzej, and his own open questions) | raw capture → triage | living |
| **[board/to-spec.md](board/to-spec.md)** · **[to-spike.md](board/to-spike.md)** · **[to-plan.md](board/to-plan.md)** | "What's next, by stage?" | one home per item | living |
