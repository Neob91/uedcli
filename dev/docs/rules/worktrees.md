...
from. A worktree is a second working directory for the same repository, checked out on its own
branch: the files are separate on disk, the git history is shared, so nothing is cloned and nothing
is pushed to move work between them.
...
it stays on the checked-out branch and follows `CLAUDE.md` "Commits".
