...
Wait cheaply: re-invoking the model to check costs a full context read
each time (full price once past the prompt cache's ~5-min TTL), so do not
poll on short model wake-ups.

- Run it as a tracked background job and let the harness re-invoke you the
  moment it exits — completion wakes you for free.
