...
the executable half of the `CLAUDE.md` "Documentation" rule that "every claim
about how UnrealEd behaves carries its evidence": the prose cites the spike,
the test *enforces* it. (A spike whose result is a one-off decision, not a
standing fact, needs no test — use judgement.)
