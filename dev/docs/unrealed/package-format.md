...
Is a Deus Ex package (`.u`/`.dx`/`.utx`) different from a stock Unreal/Unreal Tournament
package, and is v68-vs-v69 a compatibility gate?

No. A Deus Ex package and an Unreal/UT package are the same UnrealEngine-1 binary format. A
Deus Ex code package won't load into the UT-lineage UED22 editor because of class-graph
divergence (a DeusEx-flavored `Engine`/`Core`) plus a mesh-format difference, not the
version field.
...
`uedcli/dxpkg.py` reads the magic, version, name table, and import table for versions 61,
68, and 69; 68 and 69 share the same code path (`ver>=64`: compact-index length + string +
4-byte flags). Only version 61 (five old content packages:
`CoreTexDetail`/`CoreTexWater`/`Palettes`/`Render`/`TITAN`) uses a different name-table
encoding (null-terminated string, no length prefix). So:

- 61 → 68 is a real name-table format change.
- 68 → 69 is a minor bump that changes nothing in the header / name table / import table
  that uedcli parses.

Verified: the v68 `DeusEx.u` (11293 names, 3151 imports) and the v69 `DeusEx.u` (7017
names, 1952 imports) both parse through the identical `ver>=64` reader.

---

## The v69 editor ships and loads v68 packages ✅

The committed UED22 substrate — the UT-lineage v69 editor's own code — contains 7
version-68 packages alongside 25 version-69 ones (`ConSys.u` in `uned/UED22/` is itself
version 68):
...
A package's import table names the packages its objects come from. `DeusEx.u`'s imports
name `Core` and `Engine` — the classes it inherits from and the natives it calls. Those
references resolve by name against whatever `Engine.u`/`Core.u` the editor has loaded. Deus
Ex ships its own `Engine`/`Core` whose class members and native signatures differ from
Unreal Tournament's, so a Deus Ex code package loaded into the UT-lineage editor hits UT's
`Engine`/`Core`, where the expected symbol is absent or different → link failure. The
version byte is irrelevant.
...
actor`. Rebuilding through `EDIT PASTE` produced 68 BSP nodes and it loaded, spawned, and
rendered in-game first try (🔬 verified 2026-06-28, `quirks.md` "How brushes enter the
level"). The version gap was not involved.

---

## Object body layouts (byte-exact) 🔬

Native readers decode object bodies straight from the package with no editor. Layouts verified
against real Deus Ex files:
...
the directed pathnode→pathnode edges the editor builds with `PATHS DEFINE`. Each is 21 bytes:
...
`PlayerStart` surfaced unreachable nav regions across 4 maps with no editor/game. Caveats: the record
*count* isn't decoded (found as the longest self-validating run) and the header before the records is
...
`bHasComp` and `CompFormat` are tagged properties, not raw bytes after `Mips`. Reading two raw
bytes after `Mips` then a second array fails on every real sample: over the whole Deus Ex tree
(2026-07-25) it gives 107 skip-offset mismatches + 100 non-EOF bodies. Reading the flags from the
property list and parsing `CompMips` immediately after `Mips` lands exactly on the declared body
end for 207 / 207 previously-failing `Texture` exports, and consumes zero bytes when the flag is
absent or false.
...
own `LUM/Textures/LUM_CoreTex.utx`, 207 over the whole tree; `CompMips` explains 100 % of them on
every root measured. Every `bHasComp` texture measured is `(Format ⇒ 0, CompFormat = 3)`: a P8
original with a DXT1 copy. Prefer `Mips` — the higher-fidelity original; `CompMips` is lossy.

`Format` names the `Mips` array's layout, `CompFormat` the `CompMips` array's. Judging one array
against the other's code is a wrong image, not an error: all 69 measured `CompMips` arrays are DXT1
while their `Mips` are P8.

Two integrity signals on v68/v69, one on v61. On v68/v69 each mip's `WidthOffset` is a free per-mip
check — after reading `Data` the cursor must equal it. v61 has no skip offsets, so the only check is
that the body ends where the export table says. uedcli's decoder therefore records each texture's
leftover byte count rather than raising, and both checks span both arrays.
...
(`len(mip.data) == 0`), never from a class name — which matters because a class-name rule would
miss every modded procedural class.

#### Reading the pixel layout off the mip chain ✅

A mip chain is self-describing. Block-compressed formats store `ceil(w/4) × ceil(h/4)` blocks, so
their chains floor at one block — an 8- or 16-byte tail. Linear formats keep scaling as `w·h·N` all
the way to 1×1. That names the layout without any per-game table, which matters because slot
numbers are not portable: `ETextureFormat` dumped from three installs has 8 slots (Unreal Gold
v69), 122 (UED22/227 v69) and 5 (Deus Ex v68), and slot 2 is 8 bytes/px in one (`RGB64`) but 2 in
another (`R5G6B5`). A hardcoded table mis-slices real data and reports a bogus size mismatch.

The data is not always decisive. A `w × h` mip of `w·h` bytes is byte-identically explained by P8
(`w·h·1`) and by a 16-byte block layout (`(w/4)(h/4)·16 = w·h`) whenever both dimensions are
multiples of 4 — the chain only gives itself away once it descends below one block. Over 18,176
texture exports, 45.8 % fit two or more layouts, so the `Format` code is a primary path, not an
edge case.
...
"Effective" means the stored byte if the property is present, else 0. UE1 omits any property equal
to its class default, so an absent `Format` is not a missing code — it is byte 0, which is P8 in
all three enums. A `Format` property is physically present on 11 of 18,176 exports, so the implied
0 resolves 8,324 of the 8,327 ambiguous chains.

The veto matters. 227's slot 8 is `TEXF_BC4`, a single-channel 8-byte-block format whose mip chain
is byte-for-byte BC1's size and fits it uniquely. Without the veto a BC4 texture draws as BC1 — a
wrong image on a file whose own code says it is not BC1. Slot 9 collides the same way; 10 and 11
collide with the 16-byte class. Measured firing rate on real content: zero (all 11 stored codes
are 3 or 7).

So an uncoded 8-byte-block chain is taken for BC1 by assumption, not deduction — the data cannot
separate BC1 from BC4. The assumption is safe because a genuine BC4 export has `Format = 8 ≠ 0`,
writes the byte, and the veto catches it; what is assumed is that no writer emits a non-BC1
8-byte-block chain while omitting `Format`.

The limit, with its scope:
...
error, not a guess. Measured frequency on real content: zero, because P8 is a fitted candidate in
every ambiguous chain that stores no code.

BC3's alpha half identifies it. All 4,096 blocks of `DmRiot.unr:Poster01`'s mip 0 carry
...
above. Every measurement in this section is recorded in
...
Spikes `../spikes/2026-07-25-native-texture-formats/` and
`../spikes/2026-06-27-decontainerize-uedcli/01-native-texture-decode.md` proved the P8 path
...
(terminated by `None`) → a format `FName` (`it`/`s3m`/`xm`/`mod`/`wav`, = the asset's extension) → a
`TLazyArray` blob that is the raw asset file byte-for-byte. On v68/69 the `TLazyArray` header is
`i32 skip` + `ci count` + `count` bytes (the same lazy-array textures and meshes use). No
re-encoding — extraction is a straight copy. Verified by extracting `Training_Music` (`format=it`,
1,449,937 B) from `Area51Bunker_Music.umx` to a playable Impulse Tracker `.it`; `USound` parses
identically (66 `wav` objects with correct sizes from `MoverSFX.uax`).
...
non-zero) — exactly when the export table's flags word for that object carries `RF_HasStack`
(`0x02000000`). Everything else in the body, including the `None`-terminated tagged-property list,
follows it.

It is not only actors. In retail Deus Ex maps a few `Model` and `Polys` exports — plain data
objects, not actors — also carry `RF_HasStack` and therefore a StateFrame. Measured over the first
twelve `DX/Maps/*.dx` (2026-07-27, host-native decode with `upackage`):
...
`native.umodel.parse_model_body` reach EOF on 21 of 21; entering the body at its raw `soff` instead
desyncs by the StateFrame's length and fails with a truncated read.

Rule for any body reader: decide on the export's flags, never on the object's class. uedcli's
`mapimport._skip_state_frame` takes the export record for this reason.
...
The array wins. Measured over the 88 retail Deus Ex maps (2026-07-27, host-native decode) and
checked against UnrealEd's own `UCC batchexport` of the same maps:
...
`Light`, `Spotlight`, `HidePoint`, `Teleporter`, `PlayerStart`, `MapExit`, `ZoneInfo`: deleted
actors whose export was never reclaimed, dead weight in the file. Confirmed by exporting three
affected maps (`02_NYC_Warehouse` 3 orphans, `DXMP_Cathedral` 14, `DXMP_Area51Bunker` 221): zero
orphans in the editor's output. The accounting is exact — on `02_NYC_Warehouse`, 2131 roster
entries minus 4 viewport cameras = 2127 actors exported.

Rule for any reader: walk the `Actors` array, treat an off-roster actor object as absent.
...
`CameraPoint → Keypoint → Actor → Object` — and absent from the UT-lineage v69 one, another
instance of class-graph divergence.

Two consequences. The native decoder handles those maps and the editor route does not, so an editor
export is not an oracle for all 88 maps. And resolve retail classes against the game's `System/`,
never against UED22 — doing the latter makes `Engine.CameraPoint` look like it is not an actor, and
two Endgame maps then look like decode failures when the decode is right.
...
package's name table. Two traps:

- Index 0 is an ordinary name, not a sentinel. In every Deus Ex map sampled (2026-07-27) name
  index 0 is `OUTSIDE` — the editor's default face label — and it is genuinely used: 7399 of
  `02_NYC_Street.dx`'s 10690 authored polygons carry it.
- "No label" is the index of the name `None`, wherever it sits (index 2 in `00_Training.dx`). The
  name table's order is per package; nothing pins `None` to a fixed slot.

So the test for an unset `Item` is `pkg.names[idx] == "None"`, never `idx == 0`. Treating 0 as unset
silently deletes every `Item=OUTSIDE` from a decoded map — a data loss no error reports, visible
only by reading the emitted T3D. Same for any `FName`-valued field, including the property-list
terminator (`UPolys`' body begins with a compact `None` index, also not `0`).
...
- Never explain a Deus-Ex-won't-load-in-Unreal (or vice-versa) symptom as "wrong version".
  Look at the class graph (which `Engine`/`Core` is loaded) for code, and at CSG/BSP (did
  the brushes carve?) for maps.
- The committed substrate deliberately mixes v68 and v69 packages; that is expected.

## See also

- [`t3d.md`](t3d.md) — the T3D on-the-wire text format (the `MAP EXPORT`/`IMPORTADD`
  actor/geometry text, not the binary package).
