...
This file spells out the evidence behind the claims in `kb/`: the sources crawled (with each source's
scope, which is load-bearing), the binary verifications done against the shipped Deus Ex packages, the
confidence tiers, the access notes for re-running the crawl, and the residual gaps external reading
cannot close.

Repo convention: every claim about how UnrealEd behaves carries its evidence, and an undated, uncited
assertion cannot be trusted. This KB was built by a multi-pass crawl of the community tutorial corpus,
reconciled against the shipped game binaries and uedcli's own offline decoders — the binaries win
every disagreement.
...
| **decompiled** | disassembly spike | grounded in the CSG/BSP disassembly spike (§4) — read from the compiled instructions and constants (machine code and constant tables, not just string literals), a tier stronger than 📖. |
...
Facts read straight from the shipped packages — the KB's highest-confidence non-decompiled tier, and
the seeds of the engine-facts regression the measurement spike pins (per the spikes rule).
...
- **`LE_Negative` is absent from DeusEx `Engine.u`.** The UT2004-era `LE_Negative` spatial-light
  ...
  🔬), so the old lighting guide has a doc bug, corrected in [`lighting.md`](./lighting.md).
  *(Verified against the game `System/Engine.u` — the package uedcli reads (config `paths` =
  ...
  slip past a naive grep, which is how the bogus "16 members / lacks Shock/Disco/Shell/Rotor"
...
The BSP-hole mechanism is grounded not in tutorials but in the CSG/BSP disassembly spike:

> [`../../../spikes/2026-06-24-bsp-csg-hole-mechanism-from-binary.md`](../../../spikes/2026-06-24-bsp-csg-hole-mechanism-from-binary.md)

It is the ground truth wherever tutorials and disassembly disagree, and it lets
[`csg-bsp.md`](./csg-bsp.md) state the mechanism (discrete numeric-validity tolerance bands —
the 0.25 uu split band, the < ~1e-4 uu colinear-collapse, the < 3-vertex / zero-area `FPoly` discard)
and reject the folk explanation ("floating-point overflow / the engine gives up on the maths",
false). The engine facts it pins are re-asserted by committed regressions so a violation trips a red
