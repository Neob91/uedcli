...
UE1 splits collision into colliding (do I participate at all / can I be touched) and blocking (do I
physically stop movement):
...
Ladders in DX are texture-driven, not a physics mode — see [`textures.md`](textures.md) §7.1.
...
- **[DX]** adds its own particle system — the `ParticleGenerator` family (`WaterDrips`, `LaserEmitter`,
  ...
  [`README.md`](README.md) §10.1. DX-specific; does not exist in stock UT99.
...
The figures, decoded from the shipped `DeusEx.u` (full table in
