...
To place a configured actor into the trunk, pipe the generator to `actor add -`:
...
| `AutoTurret` / `AutoTurretSmall` | ceiling/wall auto-turret | place these, not the `…Gun` variants. The turret itself is a `DeusExDecoration` 🔬; the hackable part is its auto-spawned `AutoTurretGun` (`extends HackableDevices`), `hackStrength` 0.5 (50%) |
