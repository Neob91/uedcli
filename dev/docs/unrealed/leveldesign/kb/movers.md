...
A mover is one brush promoted to an actor, moving between authored poses — the engine's mechanism for
doors, lifts, elevators, drawbridges, crushers, rotating fans, breakable glass, and anything else that
shifts at runtime.
...
The UnrealEd keyframe recording flow is inverted:
1. The mover starts at key 0 (its authored pose).
2. Select the target key first (e.g. "Key 1"), then move/rotate the brush to where that key should be.

You select the destination slot, then pose — not "move then record". uedcli sidesteps this: keys are
authored as T3D offsets via the `mover key` verbs (§6), so the inverted flow never applies here.
...
- Base default is `BumpOpenTimed` 🔬 (read from `Engine.Mover`), but `DeusExMover` overrides its own
  default to `TriggerToggle` 🔬 — so a DX door built via `--mover-class DeusEx.DeusExMover` starts as a
  trigger-toggle unless you set `--prop InitialState=…`.
...
- Return-groups and `MoverEncroachType` are distinct. A return-group is several movers that move
  together under one leader (one mover drives the group); `MoverEncroachType` is the independent
  per-mover collision-response setting above.
...
A mover is an actor, so — unlike baked BSP surfaces — it is lit at runtime, and by default samples
lighting from its key-0 pose only. A door that opens away from its lit key-0 position renders black in
its open pose (the "black door").
...
- Use Special-Lit rings (`bSpecialLit` lights + `PF_SpecialLit` surfaces — see
  ...
  pose.
- Set `bDynamicLightMover=True` and use `WorldRaytraceKey` / `BrushRaytraceKey` to control which key
  pose the mover is lit from.
