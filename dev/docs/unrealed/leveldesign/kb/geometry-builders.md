...
Native brush builders (with their real UnrealEd parameters), third-party extended builders, brush
clipping, curved geometry, UE1 terrain, and MeshMaker. Feeds [csg-bsp.md](./csg-bsp.md): a builder
produces the `FPoly` list a CSG op commits.

**uedcli mapping.** uedcli exposes a subset of native builders as `brush build {cube, cylinder, cone,
sheet, staircase, spiral}`, plus the 2D-shape-editor sweeps `brush build {extrude, revolve}` (§4).
Each prints a T3D snippet to stdout, committed with `… | actor add -`. Verbs are noted inline; the
rest are editor-GUI shapes with no uedcli verb, kept because their params inform what a good shape
looks like.
...
expressions (e.g. `=64+128`) 📖; each builder remembers its params for the session.
...
| **`ConeBuilder`** | Height, CapHeight, Outer/InnerRadius, Sides | `brush build cone` | a pyramid / frustum (truncated cone when CapHeight < Height) |
...
Third-party `.u` add-ons (installing them edits the `[UnrealEd.EditorEngine]` builder entries),
filling gaps in the native set:
...
- **Extruder** — sweep a 2D profile along a `PathPoints[]` list (absolute or relative) for pipes /
  curved tubes; auto-caps unless the path is a closed loop.

GUI builders, not uedcli verbs — the sanctioned way to get subtractable spirals and swept tubes
without off-grid vertex editing.
...
- Place clip markers with Ctrl+RMB in a 2D view — 2 markers define a cut plane (vertical in that
  view); a 3rd marker tilts it to an arbitrary angle — still one planar cut, not a compound
  multi-plane cut.
...
> curves. Off-grid vertex edits are a prime source of the tolerance-band holes in
...
UE1 terrain is 100% brush-based — there is no heightmap `TerrainInfo` actor. *(Debunked:* the
heightmap `TerrainInfo` is UE2; do not look for it in DX.)
...
Outdoor philosophy: keep ≤150 polys in view ([zones-performance.md](./zones-performance.md) §3);
block sightlines with the terrain itself so total polys can be high but never all visible at once.
...
To fix a faceted brush pillar, convert it to a mesh Decoration so it stops cutting the BSP.
...
Geodesic spheres, volumetric star-sheets, terrain tessellation, the Tarquin extended builders,
clipping, Bézier curves, and MeshMaker have no uedcli generator — author their output as a T3D
snippet (or a prefab) and bring it in via the normal `actor add -` path.
