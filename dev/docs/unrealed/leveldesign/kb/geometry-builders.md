...
clipping, curved geometry, UE1 terrain, and MeshMaker. Feeds [csg-bsp.md](./csg-bsp.md): a builder produces
the `FPoly` list a CSG op commits.
...
(§4) — each prints a T3D snippet to stdout, committed with `… | actor add -`. Verbs are noted inline; the
rest are editor-GUI shapes with no uedcli verb, kept here because their params inform what a good shape
looks like.
...
| **`ConeBuilder`** | Height, CapHeight, Outer/InnerRadius, Sides | `brush build cone` | a pyramid / frustum (a truncated cone when CapHeight < Height) |
...
These are GUI builders, not uedcli verbs — the sanctioned way to get subtractable spirals and swept tubes
...
- Every 2D-shape-editor operation yields one brush ✅ — extrude, revolve and sheet all produce the
  ...
  *(Andrzej, from direct UnrealEd use, 2026-07-25.)* `brush build revolve` mirrors this: one non-convex
  brush per sweep, not one per segment — `decisions.md` 2026-07-25 00:14 UTC (D5).
- **Curved corridors** — 2D-editor Revolve: move the green pivot away from the cross-section, then
  ...
  cross-section" means drawing the profile away from `u = 0` (`decisions.md` 2026-07-25 01:05 UTC,
  D10). The `--segments` default is one facet per 22.5°, the 16-pieces-per-turn density above 📖 —
  inferred semantics, not a measurement. The straight case is `brush build extrude --point U,V …
  --depth D`.
...
The fix for a faceted brush pillar: convert it to a mesh Decoration so it stops cutting the BSP and
risking holes.
...
Bézier curves, and MeshMaker are editor-GUI / external-tool shapes with no uedcli generator — author their
output as a T3D snippet (or a prefab) and bring it in via the normal `actor add -` path.
