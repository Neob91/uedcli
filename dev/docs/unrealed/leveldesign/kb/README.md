...
The UnrealEngine-1 / Deus Ex level-design knowledge gathered while building uedcli, from a multi-pass
crawl of the community tutorial corpus reconciled against the shipped game binaries and uedcli's own
offline decoders. This `kb/` directory is the internal superset (it lives under `dev/docs/`); it keeps
everything collected, including asset-creation, modding-adjacent, and editor-GUI depth that a uedcli user
never needs.

> The user cut is elsewhere. The reader-facing subset — level-design craft scoped to what someone
> authoring a level with uedcli needs — lives outside `dev/` at
> [`docs/leveldesign/`](../../../../../docs/leveldesign/). This `kb/` tree is the superset it is drawn from; when
> the two disagree, `kb/` records what was found and the user doc is the editorial cut.

> What the user cut excludes (relevant when editing `docs/leveldesign/`). The user cut documents only what
> a uedcli author can set or observe: placeable classes, their editor-editable (`var()`) properties, the
> verbs, and the craft. It omits anything a mapper can't author or run — engine-internal / non-editable
> (plain `var`) properties (e.g. a `ScriptedPawn`'s `Restlessness`/`SightPercentage`, the agitation/fear
> sustain-decay constants, `bCanConverse`), decompiled internals and byte/offset detail, the `🔬`
> live-probe marker, asset-creation / modding depth, and editor-GUI trivia. Those live only here in `kb/`.
> Rule of thumb: if a property has no editor category (a plain `var`, not `var(Category)`), it does not
> belong in `docs/leveldesign/` — name it here in `kb/`. uedcli distinguishes the two: a property whose
> decoded schema `category` is `None` is non-editable — see `uprops`.
...
- ✅ uedcli-used / live-verified · 🔬 live-probed against the real binary/editor · 📖 extracted from
...
- **[DX]** = Deus-Ex-specific. Keep these apart: DX subclasses and presets differ from stock UE1, and
  UT-only content/gametypes do not transfer.
...
| **[geometry-builders.md](./geometry-builders.md)** | [ENGINE] | native brush builders (Cube/Cylinder/Cone/Tetrahedron/Sheet/Volumetric/three stair builders/Terrain) with full params, Tarquin extended builders, brush clipping, curved geometry (Revolve/Bézier/iris doorway), UE1 terrain (no heightmap TerrainInfo), MeshMaker — mapped onto the uedcli `brush build` verbs |
...
| **[editor-ui.md](./editor-ui.md)** | [ENGINE] | editor UI & console reference — hotkeys, brush colours, browsers, the prefab `.T3D` import/export split (GUI trivia; uedcli drives verbs, not this) |
...
talks, and 🔬 direct greps + uedcli decodes of `DX/System/{Engine,DeusEx,Fire}.u` and `DX/Textures/*.utx`.
