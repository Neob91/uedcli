...
while building uedcli — the distilled output of a multi-pass crawl of the community tutorial corpus,
reconciled against the shipped game binaries and uedcli's own offline decoders. This `kb/` directory is
the *internal* superset (it lives under `dev/docs/`); it deliberately keeps **everything collected**,
including asset-creation, modding-adjacent, and editor-GUI depth that a uedcli user never needs. Nothing
...
> someone *authoring a level with uedcli* actually needs — lives outside `dev/` at
...
> The user cut documents only what a uedcli *author* can actually **set or observe**: placeable classes,
...
> NOT belong in `docs/leveldesign/`** — name it here in `kb/`, not there. (uedcli distinguishes the two:
