...
UnrealEd's exec commands are almost undocumented publicly; the binaries are the source of truth. The
exec parser literals (`ParseCommand(&Str, TEXT("VERB"))`) and the frontend's format strings live in
the DLLs/EXE; the `commands.md` catalog was built from them. Everything here is static (reads files,
never runs the editor) — run it on the bind-mounted `Tools/uedcli/uned/UED22/*.dll` directly, or
inside any container.

## Strings are UTF-16LE
Engine `TCHAR` string literals are wide (UTF-16LE); only the C++ symbol/RTTI names are ASCII. So
`strings`/`grep "CAMERA"` finds nothing — search the wide form. (`strings` isn't installed in the
image; use Python or `grep -a`.)
...
run index; consecutive verbs = one handler's `else if` chain. This revealed the whole
...
`appFailAssert` source paths out of `.rdata`. This pinned down the BSP-hole mechanism
...
`EDIT COPY` / the log window / `wmctrl`, then tear the container down. This pinned down `SELECTNAME`
and `CAMERA OPEN` (and disproved the old "no select-by-name" claim).
