...
## Nothing here changes texel DENSITY to fit a face ✅
...
- **The anchor is a WORLD AXIS, not the face.** Two faces on the same plane, or on different planes
  at the same height, therefore share one continuous texture grid keyed to the world origin — which
  is the whole point. Measured: a 128³ cube centred at `(512, 96, 48)` has its top face's `Origin`
...
**So a level designer's memory of "one tile" in UnrealEd is the GUI's Surface-properties scaling
