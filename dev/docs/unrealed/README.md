...
Verified facts about driving UnrealEd 2.2 headless (the `dx-lum-uned` container:
UED22 under wine on Xvfb `:99` + fluxbox, driven by `wine_ctl.py` over `docker exec`).
Public docs are nearly nonexistent, so document new findings here (per `../../CLAUDE.md`).
Evidence lives in `../spikes/`.
...
| [`texalign.md`](texalign.md) | What `POLY TEXALIGN`'s nine surface-alignment modes do to a surface's texture frame — the per-mode formula, guard threshold and anchor, measured live; that `ONETILE`/`WALLCOLUMN` are no-ops and `TEXELS=` is ignored; and how all of it differs from uedcli's own `brush poly align`. |
...
| [`quirks.md`](quirks.md) | Gotchas — `MAP IMPORTADD` grid-snap, `Texture=` demand-load, selectability (paste vs IMPORTADD), `SELECTNAME`, CSG model, paste drift, no coplanar merge, etc. |
...
| [`leveldesign/`](leveldesign/README.md) | Level-design craft (what makes a good, buildable level): the dev reference is [`leveldesign/kb/`](leveldesign/kb/README.md) (geometry/BSP, zoning, lighting, textures, movers, actors/collision/pathing, the DX class catalog, NPC AI, human scale, design craft), binary-fact-checked. The reader-facing user cut is at [`docs/leveldesign/`](../../../docs/leveldesign/README.md). |
