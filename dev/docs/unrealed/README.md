...
| [`texalign.md`](texalign.md) | What `POLY TEXALIGN`'s **nine** surface-alignment modes actually do to a surface's texture frame — the per-mode formula, guard threshold and anchor, measured live; that `ONETILE`/`WALLCOLUMN` are no-ops and `TEXELS=` is ignored; and how all of it differs from uedcli's own `brush poly align`. |
