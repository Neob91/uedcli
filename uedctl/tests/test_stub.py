...
from uedctl.stub import assemble_stub_source, inject_edit_package, inject_stub_cache_path
...
def test_ephemeral_build_container_wires_the_uniform_resource_mounts_and_crafted_paths_ini(
    ...
    from uedctl import container_assets
    import uedctl.stub as stub_mod

    root = tmp_path
    (root / "CLAUDE.md").write_text("x")
    (root / "Tools" / "uedctl").mkdir(parents=True)
    calls = {}
    ...
    monkeypatch.setattr("uedctl.editor.engine_ini_mount",
                        lambda name, mounts, sd: (f"/host/{name}.ini",
                                              ["-v", f"/host/{name}.ini:/opt/UED22/unrealtournament.ini"]))
    monkeypatch.setattr("uedctl.editor._engine_ini_path", lambda name, sd: tmp_path / f"{name}.ini")
    ...
    with stub_mod.ephemeral_build_container(mounts=mounts,
                                            state_dir=tmp_path / ".uedctl") as name:
...
def test_build_stub_produces_a_loadable_v69_package(tmp_path):
    ...
    from uedctl import container_assets
    from uedctl.stub import build_stub, ephemeral_build_container
    from uedctl.dxpkg import parse_header
    from uedctl.tests.conftest import install_root
...
def test_ensure_stub_returns_the_cache_without_rebuilding_on_a_hit(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("UEDCTL_HOME", str(root / "home"))     # stub cache under tmp, not real ~/.uedctl
    ...
    with mock.patch.object(stub_mod, "build_stub", side_effect=fake_build), \
         mock.patch("uedctl.stub_closure.direct_packages", return_value=set()):
...
def test_stub_errors_are_runtimeerrors_so_callers_catch_them_cleanly():
    ...
    from uedctl.stub_closure import StubClosureError
...
def test_ephemeral_build_container_passes_stub_cache_env_for_the_compose_mount(
        tmp_path, monkeypatch):
    """The BUILD container shares the compose `/stubs` volume (`${UEDCTL_STUB_CACHE:-…}`), and
    /stubs is FIRST on its Paths so previously-built stubs shadow v68 deps during `UCC make` — so
    it must pass the resolved `config.stub_cache_root()` (honoring $UEDCTL_HOME) in the compose
    ...
    monkeypatch.setenv("UEDCTL_HOME", str(tmp_path / "custom-home"))
    seen = {}
    ...
    with stub_mod.ephemeral_build_container(mounts=None, state_dir=tmp_path / ".uedctl"):
        pass
    assert seen["env"]["UEDCTL_STUB_CACHE"] == str(config.stub_cache_root())
