...
from uedctl.model import Actor, parse_t3d
from uedctl.tests.conftest import read_fixture
from uedctl.writes import (
    actor_bounds,
    add_actor,
    allocate_name,
)
...
def test_add_actor_sets_grid_then_importadds():
    ...
    with mock.patch("uedctl.writes._write_container_file", return_value="/repo/Temp/x.t3d") as wf:
        add_actor(drv, a)
...
def test_allocate_name_avoids_existing():
    level = parse_t3d(
        "Begin Map\nBegin Actor Class=Light Name=UedctlLight0\n    Name=\"UedctlLight0\"\nEnd Actor\nEnd Map")
    name = allocate_name(level, "Light")
    assert name == "UedctlLight1"           # skips the taken UedctlLight0
...
def test_add_actor_updates_level_on_success():
    ...
    a = Actor(name="UedctlLight0", cls="Light", location=(0.0, 0.0, 0.0))
    with mock.patch("uedctl.writes._write_container_file", return_value="/repo/Temp/x.t3d"):
        add_actor(drv, a, level=level)
    assert "UedctlLight0" in level.actors
