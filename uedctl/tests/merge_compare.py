...
from uedctl import brushcsg
from uedctl.model import parse_t3d
from uedctl.tests import intersect_cases
...
def native_links(case_id: str) -> list[int]:
    ...
    Read straight off the FFI tuple rather than the emitted T3D, because uedctl's `Polygon` model
    has never carried `Link` — see `oracle_links`.
    """
    import uedctl_native
    from uedctl.native.materialize import _build_brush_input

    case = intersect_cases.CASES[case_id]
    actors = intersect_cases.build_actors(case_id)
    world, builder = brushcsg.build_scaffolding(
        actors, deintersect=case["verb"] == "deintersect")
    faces = uedctl_native.intersect_brushset(
...
def oracle_links(t3d: str) -> list[int]:
    ...
    Parsed from the raw text: `Polygon.Link` is not part of uedctl's model (nothing in the tool
...
def load_golden_text(case_id: str) -> str:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"missing golden {path} — regenerate with `UEDCTL_REGEN_GOLDENS=1 bin/test "
            f"uedctl/tests/test_integration_intersect_oracle.py -m integration`")
