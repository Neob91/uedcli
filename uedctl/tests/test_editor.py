...
from uedctl import editor as editormod
from uedctl.editor import editor_container
...
def test_stop_editor_removes_container_and_volume(monkeypatch, tmp_path):
    ...
    editormod.stop_editor("session/0193abcd-0000-7000-8000-000000000001", tmp_path / ".uedctl")
...
def test_ensure_editor_passes_stub_cache_env_for_the_compose_mount(monkeypatch, tmp_path):
    """The compose `/stubs` volume source is `${UEDCTL_STUB_CACHE:-…}` — ensure_editor must pass
    the resolved `config.stub_cache_root()` (which honors $UEDCTL_HOME) in the subprocess env, so
    ...
    assert seen["env"]["UEDCTL_STUB_CACHE"] == expect
