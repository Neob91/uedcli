...
from uedctl.container_assets import paths_ini_lines, resource_mounts
from uedctl.packages import (ensure_load, ensure_load_message,
...
def test_ensure_load_writes_full_paths_and_remaps_obj_loads_via_the_mounts(tmp_path):
    ...
    with mock.patch("uedctl.packages.write_paths_and_reload", autospec=True) as wpr:
...
def test_ensure_load_skips_obj_load_for_a_package_not_on_the_search_path(tmp_path):
    driver = mock.Mock(container="c")
    with mock.patch("uedctl.packages.write_paths_and_reload", autospec=True):
...
def test_unloadable_v68_flags_only_a_dot_u_under_a_config_mount():
    ...
    entries = [
        ...
        ("DeusExItems", "/home/u/.uedctl/cache/stubs/DeusExItems.u"),  # v69 stub, not a mount → safe
...
def test_ensure_load_refuses_an_unstubbed_v68_package_before_touching_the_editor():
    ...
    with mock.patch("uedctl.packages.obj_load_entries",
...
def test_remap_stub_cache_to_stubs_mount(tmp_path, monkeypatch):
    # The stub cache is the per-user `$UEDCTL_HOME/cache/stubs`, OUTSIDE repo_root; its fixed
...
def test_remap_rejects_a_non_container_visible_path(tmp_path):
    ...
    host = str(tmp_path / ".uedctl" / "store" / "main" / "x.t3d")
    with pytest.raises(ValueError):
