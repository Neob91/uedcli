...
from uedctl import config, uprops

pytestmark = pytest.mark.integration


def _resolver():
    # conftest points $UEDCTL_HOME at a tmp dir for isolation; an INTEGRATION test wants the
    # developer's REAL games config, so read it by explicit path (read-only).
    uc = config.load_user_config(override=os.path.expanduser("~/.uedctl/config.toml"))
    if uc is None:
        pytest.skip("no ~/.uedctl games config")
