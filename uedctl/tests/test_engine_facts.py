...
def test_t3d_import_strips_double_slash_comments():
    ...
    `Exact==0` that `ImportProperties` passes) — this is what lets a `// uedctl-folder: <path>`
...
def test_sheer_coeff_matches_the_disassembled_piecewise_snap():
    ...
    """
    from uedctl.transform import sheer_coeff
...
def test_fscale_emission_rule_byte_matches_the_editor():
    ...
    from uedctl.transform import FScale, emit_fscale
...
def test_world_light_radius_matches_the_source_formula():
    ...
    from uedctl.preview import world_light_radius
...
def test_world_sound_radius_matches_the_source_formula():
    ...
    from uedctl.preview import world_sound_radius
...
def test_sprite_footprint_is_drawscale_times_texel_dims():
    ...
    from uedctl.preview import sprite_footprint
...
def test_level_actors_array_is_int_num_max_then_compact_refs():
    ...
    from uedctl.native.level_write import write_level_body
...
def test_editor_frotator_omits_zero_components_and_never_writes_an_all_zero_rotation():
    ...
    uedctl's producers wrote all three components, so a yaw-only actor never text-matched its own
...
def test_the_sheer_axis_enum_matches_the_real_core_package():
    ...
    its values."""
    from uedctl import typedprops
    from uedctl.transform import DEFAULT_SHEER_AXIS, _SHEER_OFFDIAG
...
def test_amover_postload_unconditionally_stamps_the_savedpos_savedrot_sentinels():
    ...
    uedctl-authored mover omits both, the map it materializes into always comes back carrying the
