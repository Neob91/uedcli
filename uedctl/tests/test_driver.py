...
from uedctl import driver as _driver
from uedctl.driver import Driver, DriverError, package_header_problem
from uedctl.tests.conftest import install_system_root


def test_exec_shells_out_to_wine_ctl():
    d = Driver(container="test-ctr")
    with mock.patch("uedctl.driver.subprocess.run") as run:
...
def test_screenshot_shoots_into_work_then_cps_out_to_the_host_path():
    ...
    with mock.patch("uedctl.driver.subprocess.run", autospec=True, side_effect=fake_run):
...
def test_map_sendto_rejects_bad_arg():
    import pytest
    from uedctl.driver import DriverError
    d = Driver(container="c")
...
def _drive_map_save(ctr, clock, **kw):
    ...
    d = Driver(container="c")
    with mock.patch.object(d, "exec", autospec=True) as ex, \
            mock.patch("uedctl.driver.time.sleep", clock.sleep), \
            mock.patch("uedctl.driver.time.monotonic", clock.monotonic), \
            mock.patch("uedctl.driver.subprocess.run", autospec=True, side_effect=ctr.run):
...
def test_container_probes_quote_a_hostile_path():
    ...
    d = Driver(container="c")
    ctr = _FakeContainer([None])
    with mock.patch("uedctl.driver.subprocess.run", autospec=True, side_effect=ctr.run):
...
def test_container_stat_trusts_the_sentinel_not_the_docker_exit_code():
    ...
    d = Driver(container="c")
    with mock.patch("uedctl.driver.subprocess.run", autospec=True) as run:
...
def test_container_stat_bounds_the_docker_call_instead_of_hanging():
    ...
    d = Driver(container="c")
    with mock.patch("uedctl.driver.subprocess.run", autospec=True,
...
def test_the_drivers_package_magic_matches_the_one_package_core():
    ...
    from uedctl.upackage import MAGIC
...
def test_selectname_issues_selectname_with_actor_name():
    ...
    d = Driver(container="c")
    with mock.patch.object(d, "exec", autospec=True) as ex:
        d.selectname("UedctlLight0")
    ex.assert_called_once_with("SELECTNAME NAME=UedctlLight0")
...
def test_camera_align_with_name_issues_align_name():
    ...
    d = Driver(container="c")
    with mock.patch.object(d, "exec", autospec=True) as ex:
        d.camera_align(name="UedctlLight0")
    ex.assert_called_once_with("CAMERA ALIGN NAME=UedctlLight0")
