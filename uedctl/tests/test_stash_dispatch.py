...
from uedctl import dispatch, stash_register, trunk
...
def _project_with_level(tmp_path, monkeypatch, name="lvl"):
    """A `<tmp>/repo` project ROOT (new layout: `<root>/uedctl.toml`) with `name` set as the ambient
    `$UEDCTL_LEVEL`. The ambient trunk level is the git-native replacement for the old session's
...
def _register(proj):
    return stash_register.FileStashRegister(proj / ".uedctl" / "stash")
...
def test_it_applies_a_stash_merging_translated_grouped_renamed_actors(tmp_path, monkeypatch):
    ...
    from uedctl.writes import actor_bounds
    a = got.actors[placed]
...
def test_it_lists_prefabs_with_an_explicit_dir_and_no_project(tmp_path, monkeypatch, capsys):
    ...
    # resolve one (chdir to an isolated dir so walk-up can't find an ambient uedctl.toml).
...
def test_prefab_list_without_a_project_or_dir_errors(tmp_path, monkeypatch, capsys):
    ...
    assert rc == 2
    assert "not in a uedctl project" in capsys.readouterr().err
...
def test_stash_show_corrupt_meta_is_clean_not_a_traceback(tmp_path, monkeypatch, capsys):
    ...
    (proj / ".uedctl" / "stash" / "bay" / "meta.json").write_text("{ not valid json")
