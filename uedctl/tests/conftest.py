...
# (production is the composed config search path). `UEDCTL_TEST_INSTALL` overrides; the no-env
# fallback anchors on THIS file's location (the dev-checkout layout), so collection never fails.

def install_root() -> Path:
    env = os.environ.get("UEDCTL_TEST_INSTALL")
    if env:
        return Path(env)
...
def _isolate_uedctl_home(tmp_path, monkeypatch):
    """Point `$UEDCTL_HOME` at a per-test tmp dir so NO test can read or write the developer's real
    `~/.uedctl` — neither the config nor the derivable cache (`cache/{stubs,textures}`). A test that
    needs a specific home sets `UEDCTL_HOME` itself; its setenv/delenv runs after this fixture and
    overrides."""
    monkeypatch.setenv("UEDCTL_HOME", str(tmp_path / "uedctl-home"))
...
def _schema_cache_off(monkeypatch):
    ...
    in-process memo so a bundle cached by one test can never leak into another (each `uedctl` command
    is normally a fresh process, so cross-test memo persistence is a test-harness artefact)."""
    from uedctl import schema_cache
...
def _isolate_project_resolution(tmp_path_factory, monkeypatch):
    ...
    `$UEDCTL_PROJECT`, and chdir into a neutral tmp dir so cwd walk-up can never climb out of tmp
    and find the dx_lum repo's own `uedctl.toml` (a test that forgot its project fixture would
    ...
    # Also drop an inherited $UEDCTL_LEVEL (the ambient level source since 2026-07-20): a developer
    # who runs `bin/test` with a level exported must not have it silently satisfy a verb that a test
    # expects to fail with "no level". A test that needs an ambient level sets UEDCTL_LEVEL itself
...
def tmp_project(tmp_path, monkeypatch) -> Path:
    """A minimal NEW-layout project rooted at a tmp dir: writes `<root>/uedctl.toml` and points
    `UEDCTL_PROJECT` at the root, so project resolution needs no cwd walk-up — the same seam
    production uses. Returns the root Path (managed dirs default to `<root>/{maps,prefabs,
    texture-catalog}`; state lands in the self-ignoring `<root>/.uedctl/`)."""
    root = tmp_path / "proj"
    root.mkdir()
    (root / "uedctl.toml").write_text('game = "deusex"\n')
    monkeypatch.setenv("UEDCTL_PROJECT", str(root))
    return root
...
class StubDefaults:
    ...
    def for_class(self, fqcn: str):
        from uedctl.classdefaults import ClassInfo
