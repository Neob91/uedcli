...
def _mat_ok(monkeypatch, body=b"m"):
    ...
    monkeypatch.setattr("uedctl.apply.run_materialize", fake)
...
def _install_warm(monkeypatch, w, tmp_path):
    ...
    monkeypatch.setattr("uedctl.container_assets.resource_mounts", lambda d: [])
...
def test_acquire_lock_timeout_named(monkeypatch, tmp_path):
    ...
    # The lock now lives under config._user_home() → honors $UEDCTL_HOME (spec 2026-07-17 §5);
    # point it at a tmp home and pre-hold the flock there.
    monkeypatch.setenv("UEDCTL_HOME", str(tmp_path / "home"))
    (tmp_path / "home").mkdir()
...
def test_render_shots_pitch_clamped_and_uu_in_request(monkeypatch, tmp_path):
    ...
    from uedctl.rotation import deg_to_uu
    assert n == 1
...
def test_resolve_all_trunk_at_actor_still_host_side(monkeypatch):
    ...
    monkeypatch.setattr("uedctl.preview_native.actor_aim_point", lambda level, n: (5, 6, 7))
