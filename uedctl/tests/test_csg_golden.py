...
Two layers:
  ...
  * INTEGRATION (`-m integration`) — mint an ephemeral editor, build each corpus
    ...
    This is also the re-bless path (`python -m uedctl.native.csg_golden`).
"""
...
from uedctl.native import csg_golden as g
...
def test_at_least_one_fixture_is_frozen():
    assert FIXTURE_CASES, (
        "no csg_golden fixtures frozen yet — run `python -m uedctl.native.csg_golden` "
...
def ephemeral_driver():
    ...
    from uedctl import editor as editor_mod
