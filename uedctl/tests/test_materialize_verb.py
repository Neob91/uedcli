...
from uedctl.apply import ApplyResult, run_materialize
from uedctl.model import Actor, Level
from uedctl import xfer
from uedctl.verify import VerifyResult
...
def test_it_refuses_to_overwrite_an_existing_out(tmp_path, _stub_editor):
    ...
    r = run_materialize(level=_one_actor_level(), packages=[],
                        ...
                        state_dir=tmp_path / '.uedctl', out_path=str(out), overwrite=False)
...
def test_it_allows_overwrite_with_the_flag(tmp_path, _stub_editor):
    ...
    r = run_materialize(level=_one_actor_level(), packages=[],
                        schema_resolver=_NO_PACKAGES,
                        state_dir=tmp_path / '.uedctl', out_path=str(out), overwrite=True)
    assert r.rc == 0
...
def test_materialize_resolves_the_trunk(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("uedctl.apply.run_materialize", fake_run)
    monkeypatch.setattr("uedctl.dispatch._composed_load_set", lambda p: ["Engine", "DeusExDeco"])
    monkeypatch.setattr("uedctl.dispatch._composed_dirs", lambda p: ["/g/Textures"])
...
def test_materialize_warns_on_duplicate_order_value(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("uedctl.apply.run_materialize",
                        lambda **kw: ApplyResult(rc=0, message="ok"))
    monkeypatch.setattr("uedctl.dispatch._composed_load_set", lambda p: [])
    monkeypatch.setattr("uedctl.dispatch._composed_dirs", lambda p: [])
    rc = dispatch.dispatch(_ns(cmd="level", sub="materialize", project=str(proj),
                               out=str(tmp_path / "out.dx"), overwrite=False))
    assert rc == 0
...
def test_doctor_no_project_errors_cleanly(tmp_path, monkeypatch, capsys):
    # With no uedctl project there is no level to lint; must exit 2 with a message, NOT a traceback.
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("UEDCTL_PROJECT", "")
    rc = dispatch.dispatch(_ns(cmd="level", sub="doctor", project=None,
                               json=False, severity=None, category=None))
    assert rc == 2
    err = capsys.readouterr().err
    assert "not in a uedctl project" in err and "Traceback" not in err
...
def test_materialize_missing_out_errors_before_the_load_set(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("uedctl.dispatch._composed_load_set",
