...
def _drive_prop(args, level, stdin="", *, src=None):
    ...
    with mock.patch("uedctl.dispatch._resolve_level_source", return_value=src), \
            ...
            mock.patch("uedctl.dispatch._class_schema", _schema), \
            ...
            mock.patch("uedctl.dispatch._enum_names", lambda p, project=None: p.enum_value_names):
        rc = dispatch(args)
    return rc, src
...
def test_build_add_prop_pipe_end_to_end(capsys):
    ...
    with mock.patch("uedctl.dispatch._resolve_level_source", return_value=src), \
            mock.patch("uedctl.dispatch._validate_ingest_actors", lambda actors, args: None), \
