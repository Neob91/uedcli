"""Stash on the git-native path: capture writes to the self-ignoring `<root>/.uedctl/stash/` register,
...
def test_it_captures_the_trunk_into_the_gitignored_register(tmp_path, monkeypatch):
    ...
    assert (proj / ".uedctl" / ".gitignore").read_text() == "*\n"
    reg_dir = proj / ".uedctl" / "stash" / "cap1"
