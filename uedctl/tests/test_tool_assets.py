"""Tests for `uedctl.tool_assets` — the PACKAGE-RELATIVE tool-install asset anchors
...
def test_tool_root_is_the_dir_holding_the_package():
    ...
    assert (root / "uedctl" / "tool_assets.py").is_file()   # …/Tools/uedctl/ holds the package
...
def test_umodel_dir_is_a_sibling_of_the_tool_root():
    ...
    # Tools/uedctl (the packaging item inherits this exception explicitly).
...
def test_anchors_are_cwd_independent(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("UEDCTL_REPO_ROOT", raising=False)   # retired env must not matter
