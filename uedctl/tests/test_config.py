"""Tests for the global-CLI config + path-resolution module (`uedctl/config.py`).
...
uedctl.toml` (the dir containing the file IS the project root), required `game`, root-relative
...
= bare stem), project resolution by `uedctl.toml` walk-up (nearest wins), and the self-ignoring
`.uedctl/` state dir.
"""
...
def test_it_builds_the_value_objects():
    ...
    user = config.UserConfig(games={"deusex": sub}, source="/home/me/.uedctl/config.toml")
    proj = config.Project(root="/p", game="deusex", paths="Maps", source="/p/uedctl.toml")
...
def test_it_loads_a_project_with_the_marker_dir_as_the_root(tmp_path):
    ...
    _write(str(root / "uedctl.toml"), 'game = "deusex"\npaths = "Maps:LUM"\n')
    proj = config.load_project(str(root))
    assert proj.root == str(root)                       # the dir holding uedctl.toml IS the root
    assert proj.game == "deusex"
    assert proj.paths == "Maps:LUM"


def test_it_loads_a_project_given_its_uedctl_toml_path(tmp_path):
    toml = tmp_path / "proj" / "uedctl.toml"
...
def test_it_errors_when_a_project_omits_the_required_game(tmp_path):
    root = tmp_path / "p"
    _write(str(root / "uedctl.toml"), 'paths = "Maps"\n')
    with pytest.raises(config.ConfigError) as e:
...
def test_it_errors_when_the_project_config_is_missing(tmp_path):
    ...
    assert "uedctl.toml" in str(e.value)
...
def test_it_points_managed_dirs_at_existing_repo_dirs(tmp_path):
    # The spec's headline case: a repo's EXISTING `Prefabs/`/`uedctl/maps` dirs are used in
    # place — no parallel tree (acceptance §10.2).
    root = tmp_path / "repo"
    _write(str(root / "uedctl.toml"),
           'game = "deusex"\nmaps = "uedctl/maps"\nprefabs = "Prefabs"\n')
    proj = config.load_project(str(root))
    assert config.project_maps_dir(proj) == str(root / "uedctl" / "maps")
    assert config.project_prefabs_dir(proj) == str(root / "Prefabs")
...
def test_it_composes_search_dirs_project_first_then_base_deduped_by_dir(tmp_path):
    ...
    _write(str(root / "uedctl.toml"), f'game = "deusex"\npaths = "Textures:{shared}"\n')
    proj = config.load_project(str(root))
...
def test_it_composes_files_stem_dedups_across_extensions_keep_first(tmp_path):
    ...
    user = _user_cfg(tmp_path, deusex=str(base))
    _write(str(tmp_path / "proj" / "uedctl.toml"), 'game = "deusex"\n')
...
def _project_at(root, *, game="deusex"):
    """Write `<root>/uedctl.toml`; returns the root."""
    _write(os.path.join(root, "uedctl.toml"), f'game = "{game}"\n')
    return root


def test_it_walks_up_to_the_nearest_uedctl_toml(tmp_path):
...
def test_it_does_not_discover_an_old_layout_child_config_toml(tmp_path):
    # The old `<child>/config.toml` layout is retired: walk-up keys on the `uedctl.toml`
    # FILENAME only (no child-dir scan, no schema sniffing).
    _write(str(tmp_path / "proj" / "uedctl" / "config.toml"), 'game = "deusex"\n')
...
def test_an_unreadable_marker_found_by_walk_up_is_a_hard_error(tmp_path):
    # A permission-denied uedctl.toml IS the project marker — never silently climbed past
    # (binding to an outer project would be worse than erroring; spec §4).
    if os.geteuid() == 0:
        pytest.skip("running as root — file modes don't deny reads")
    root = tmp_path / "proj"
    marker = _write(str(root / "uedctl.toml"), 'game = "deusex"\n')
    os.chmod(marker, 0)
    try:
        with pytest.raises(config.ConfigError) as e:
            config.resolve_project(cwd=str(root))
...
def test_it_finds_an_old_layout_project_dir_for_the_error_hint(tmp_path):
    ...
    _write(str(tmp_path / "repo" / "uedctl" / "config.toml"), 'game = "deusex"\n')
    deep = tmp_path / "repo" / "sub"
    os.makedirs(deep, exist_ok=True)
    assert config.find_old_layout_project_dir(str(deep)) == str(tmp_path / "repo" / "uedctl")
...
def test_state_dir_without_create_is_a_pure_path(tmp_path):
    ...
    assert d == tmp_path / ".uedctl" and not d.exists()
...
def test_state_subdir_creates_the_nested_dir_and_the_self_ignore(tmp_path):
    ...
    assert d == tmp_path / ".uedctl" / "stash" and d.is_dir()
    assert (tmp_path / ".uedctl" / ".gitignore").read_text() == "*\n"
...
def test_it_anchors_project_paths_on_the_root(tmp_path):
    # `paths` elements resolve against the project root (the uedctl.toml dir).
    base = tmp_path / "base"
    _touch_pkg(str(base), "Engine.u")
    user = _user_cfg(tmp_path, deusex=str(base))
    root = tmp_path / "proj"
...
def test_it_expands_home_for_the_user_config_path(monkeypatch):
    monkeypatch.setenv("UEDCTL_HOME", "/custom/home")
    assert config.user_config_path() == "/custom/home/config.toml"
    monkeypatch.delenv("UEDCTL_HOME", raising=False)
    assert config.user_config_path().endswith("/.uedctl/config.toml")


def test_user_cache_home_honors_uedctl_home(monkeypatch, tmp_path):
    monkeypatch.setenv("UEDCTL_HOME", str(tmp_path / "myhome"))
...
def test_stub_cache_root_is_under_the_user_cache_home_and_creates_only_when_asked(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("UEDCTL_HOME", str(tmp_path))
...
def test_user_cache_home_defaults_under_dot_uedctl(monkeypatch, tmp_path):
    # No $UEDCTL_HOME → `~/.uedctl/cache`; monkeypatch `Path.home` so the real home is never touched.
    monkeypatch.delenv("UEDCTL_HOME", raising=False)
    monkeypatch.setattr(config.Path, "home", classmethod(lambda cls: tmp_path / "fakehome"))
    assert config.user_cache_home() == tmp_path / "fakehome" / ".uedctl" / "cache"


def test_project_non_string_or_empty_keys_are_named_errors(tmp_path):
    """A hand-written `uedctl.toml` with a wrong-typed or empty value must be a named `ConfigError`
    ...
    for body, key in (('game = "dx"\nmaps = 3\n', "maps"),
                      ...
                      ('game = "dx"\ncatalog = "  "\n', "catalog"),
                      ('game = ""\n', "game")):
        (tmp_path / "uedctl.toml").write_text(body)
...
def test_walk_up_hard_errors_on_a_non_regular_file_marker(tmp_path):
    """A dangling-symlink (or directory) `uedctl.toml` must hard-error, not be climbed past —
    ...
    (outer / "uedctl.toml").write_text('game = "dx"\n')
    (outer / "inner" / "uedctl.toml").symlink_to(tmp_path / "gone")
    with pytest.raises(config.ConfigError, match="not a regular file"):
        config.walk_up_root(str(inner))


def test_project_file_marker_must_be_named_uedctl_toml(tmp_path):
    """--project/UEDCTL_PROJECT pointing at a FILE accepts only an actual `uedctl.toml` — any
