...
def _run(level, propsub, tokens=(), *, kv=False, schema=None, defaults=None):
    ...
    with mock.patch("uedctl.dispatch._resolve_level_source", return_value=src), \
            ...
            mock.patch("uedctl.dispatch._class_defaults",
                       lambda cls, project=None: dict(defaults if defaults is not None
                                                      else _DEFAULTS)), \
            mock.patch("uedctl.dispatch._struct_members",
                       lambda p, project=None: _members_for(p)), \
            mock.patch("uedctl.dispatch._enum_names",
...
def test_hard_reject_never_resolves_the_schema():
    ...
    with mock.patch("uedctl.dispatch._resolve_level_source", return_value=src), \
            mock.patch("uedctl.dispatch._resolve_project",
                       side_effect=dispatch_mod._ProjectError("no project")), \
            mock.patch("uedctl.dispatch._class_schema", schema):
        rc = dispatch_mod._dispatch(args)
    assert rc == 2
