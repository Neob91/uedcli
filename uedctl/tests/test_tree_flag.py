...
source is the ambient `$UEDCTL_LEVEL` (not the old machine-local pointer). Covers: the
...
from uedctl import builders, cli, dispatch, stash_register, stashlib, trunk
...
def _project(tmp_path, monkeypatch, name="lvl", *, env=True):
    ...
    # tests seed — the uedctl.toml key replaced the retired UEDCTL_PREFAB_DIR env override.
    (proj / "uedctl.toml").write_text(
    ...
    if env:
        monkeypatch.setenv("UEDCTL_LEVEL", name)         # the ambient default level
    return proj, name
...
def test_tree_level_edits_a_non_ambient_level(tmp_path, monkeypatch):
    proj, _ = _project(tmp_path, monkeypatch, name="lvl")  # $UEDCTL_LEVEL='lvl'
...
@pytest.mark.parametrize("argv", [
    ...
    ["brush", "clip", "W", "--axis", "x", "--coord", "0", "--tree", "stash/s"],
