...
from uedctl import dispatch, level_select, trunk
from uedctl.model import Actor, Brush, Level


def _project(tmp_path, name="20_AireGardens"):
    """A minimal uedctl project ROOT with one (empty) level trunk. Returns (root, level name)."""
...
def test_resolve_project_errors_when_none_found(tmp_path, monkeypatch):
    monkeypatch.delenv("UEDCTL_PROJECT", raising=False)
    monkeypatch.chdir(tmp_path)
    with pytest.raises(dispatch._ProjectError):
        dispatch._resolve_project(_ns(project=None))


def test_resolve_project_accepts_a_uedctl_toml_file(tmp_path):
    proj, _ = _project(tmp_path)
    got = dispatch._resolve_project(_ns(project=str(proj / "uedctl.toml")))
    assert got.root == str(proj)


def test_bad_project_config_exits_2_not_a_traceback(tmp_path):
    # A user-schema uedctl.toml (has [games.*]) is not a valid PROJECT config → config.ConfigError,
    # which must be caught to a clean exit 2, never a bare traceback.
    bad = tmp_path / "bad"
    bad.mkdir(parents=True)
    (bad / "uedctl.toml").write_text("[games.deusex]\npaths = \"x\"\n")
...
def test_resolve_project_names_the_old_layout_in_the_error(tmp_path, monkeypatch, capsys):
    ...
    # `<child>/config.toml` project but no `uedctl.toml` — gets the explicit migration hint.
    (tmp_path / "uedctl").mkdir()
    (tmp_path / "uedctl" / "config.toml").write_text('game = "deusex"\n')
    monkeypatch.delenv("UEDCTL_PROJECT", raising=False)
    monkeypatch.chdir(tmp_path)
    ...
    assert str(tmp_path / "uedctl.toml") in msg            # names the migration target


# --- Task B: the ambient level ($UEDCTL_LEVEL), resolved by resolve_level ---
...
def test_level_create_prints_the_export_hint(tmp_path, capsys):
    ...
    # removed `--select` it prints the `export UEDCTL_LEVEL=<name>` hint to stderr (decisions
    # 2026-07-20).
    proj, _ = _project(tmp_path)
    assert dispatch.dispatch(_ns(cmd="level", sub="create", name="Fresh", project=str(proj))) == 0
    assert "export UEDCTL_LEVEL=Fresh" in capsys.readouterr().err
...
def test_level_status_reports_counts_and_dup_order(tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setenv("UEDCTL_LEVEL", name)
...
def test_level_status_json_no_selection_is_explicit_null(tmp_path, capsys, monkeypatch):
    import json
    monkeypatch.delenv("UEDCTL_LEVEL", raising=False)
    proj, _ = _project(tmp_path)
...
def test_level_list_prints_names_to_stdout_count_to_stderr(tmp_path, capsys, monkeypatch):
    ...
    assert "2 level(s)" in cap.err and "no level set ($UEDCTL_LEVEL unset)" in cap.err
...
def test_level_list_notes_the_active_level_on_stderr(tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setenv("UEDCTL_LEVEL", "castle")
    capsys.readouterr()
    dispatch.dispatch(_ns(cmd="level", sub="list", json=False, project=str(proj)))
    assert "$UEDCTL_LEVEL=castle" in capsys.readouterr().err
...
def test_level_list_flags_a_stale_active_level(tmp_path, capsys, monkeypatch):
    """A `$UEDCTL_LEVEL` whose level no longer lists (its actors/ tree removed) must be flagged
    ...
    assert dispatch.dispatch(_ns(cmd="level", sub="list", json=False, project=str(proj))) == 0
    cap = capsys.readouterr()
    assert cap.out == "" and "$UEDCTL_LEVEL=castle (not listed)" in cap.err
...
# The removed `.uedctl/current-level` pointer is gone (the level is now the ambient
# $UEDCTL_LEVEL — decisions 2026-07-20). The self-ignore guarantee it used to carry now
...
def test_state_dir_writes_the_self_ignore(tmp_path):
    ...
    assert d == root / ".uedctl"
    assert (root / ".uedctl" / ".gitignore").read_text() == "*\n"   # bare `*` is canonical
...
def test_level_create_and_resolve_reject_dotted_names(tmp_path):
    ...
    and `resolve_level` (the ambient-$UEDCTL_LEVEL reader) must reject it."""
    proj, _ = _project(tmp_path)
