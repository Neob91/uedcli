...
from uedctl import dispatch as dispatch_mod
...
from uedctl.vertex import weld_vertices
...
def _run_find(args, level, hierarchy=_LIGHT_H):
    with mock.patch("uedctl.dispatch._resolve_level_source", return_value=_fake_src(level)), \
            mock.patch("uedctl.dispatch._class_index", return_value=_fake_class_index(hierarchy)), \
            mock.patch("uedctl.dispatch._resolve_project", return_value=object()):
...
def test_dispatch_vertex_list_prints_table(capsys):
    ...
    with mock.patch("uedctl.dispatch.Driver"), \
         mock.patch("uedctl.dispatch._resolve_level_source", return_value=_fake_src(_brush_level())):
        rc = dispatch(args)
    assert rc == 0
...
def test_dispatch_vertex_move_by_converts_world_to_local_model_side():
    ...
    with mock.patch("uedctl.dispatch.Driver"), \
         mock.patch("uedctl.dispatch._resolve_level_source", return_value=src):
        rc = dispatch(args)
    assert rc == 0
...
def test_add_skips_the_builder_brush_via_is_builder_brush(tmp_path):
    ...
    with mock.patch("uedctl.dispatch._resolve_level_source", return_value=src), \
         mock.patch("uedctl.dispatch.Driver", return_value=drv), \
         mock.patch("uedctl.dispatch.writes.add_actor", autospec=True) as add:
        dispatch(args)
...
def test_dispatch_build_staircase_writes_one_actor(capsys):
    ...
    assert rc == 0
    out = capsys.readouterr().out
    from uedctl.model import parse_t3d_actors
...
def test_reads_require_a_project_and_error_without_one(tmp_path, capsys, monkeypatch):
    ...
    args = SimpleNamespace(cmd="actor", sub="find", name=[], cls=[], group=[], prop=[], kind=None,
                           container="c")
    with mock.patch("uedctl.dispatch.Driver", autospec=True), \
         mock.patch.dict(_os.environ, {"UEDCTL_PROJECT": ""}, clear=False):
        rc = dispatch(args)
    # Git-native trunk: a read outside any uedctl project is a clean exit 2, not a traceback.
    err = capsys.readouterr().err
    assert rc == 2 and "not in a uedctl project" in err
...
def _preview_proj(tmp_path, actors, order, monkeypatch):
    """Write a trunk with the given actors, select it via $UEDCTL_LEVEL, return the project dir."""
...
def _patch_user_config(monkeypatch):
    from uedctl import dispatch as D
...
def test_level_preview_native_routes_shots_to_render(tmp_path, monkeypatch):
    from uedctl import dispatch as D, builders
    ...
    seen = {}
    monkeypatch.setattr("uedctl.preview_native.render_shots",
    ...
    from uedctl.preview_native import DEFAULT_FOV
...
def test_level_preview_game_routes_to_preview_game(tmp_path, monkeypatch):
    from uedctl import dispatch as D
    seen = {}
    monkeypatch.setattr("uedctl.preview_game.render_shots",
...
def test_level_preview_native_error_is_clean_exit_2(tmp_path, monkeypatch, capsys):
    from uedctl import dispatch as D, builders
    from uedctl.preview_native import NativePreviewError
    room = builders.make_brush_actor("Room", builders.cube(600, 600, 300, "T"),
                                     location=(0, 0, 0), csg="subtract")
    proj = _preview_proj(tmp_path, {"Room": room}, ["Room"], monkeypatch)
    monkeypatch.setattr("uedctl.preview_native.render_shots",
...
def test_brush_vertex_move_allows_a_low_bit_rotation_brush():
    ...
    src = _fake_src(lv)
    with mock.patch("uedctl.dispatch._resolve_level_source", return_value=src), \
         mock.patch("uedctl.vertex.move_vertices",
...
def test_brush_vertex_move_at_round_trips_through_prepivot():
    ...
    from uedctl.rotation import actor_prepivot
    ...
    with mock.patch("uedctl.dispatch._resolve_level_source", return_value=_fake_src(lv)), \
         mock.patch("uedctl.vertex.move_vertices", return_value=a.brush) as mv:
        assert dispatch(args) == 0
...
def test_brush_vertex_move_handles_a_rotated_brush():
    ...
    from uedctl.rotation import actor_matrix, actor_prepivot, local_offset
...
def _tex_project(tmp_path, monkeypatch, *, content_name="CoreTexMetal.utx"):
    """A real on-disk project + per-user games config for the config-driven texture sync path: a
    base game dir holding one content package, a UEDCTL_HOME games config pointing at it, and a
    project ROOT naming that game. Returns the root path (feeds `--project`)."""
    base = tmp_path / "game" / "Textures"
    base.mkdir(parents=True)
    (base / content_name).write_bytes(b"\x00")
    home = tmp_path / "uedhome"
    home.mkdir()
    (home / "config.toml").write_text(f'[games.deusex]\npaths = "{base}"\n')
    monkeypatch.setenv("UEDCTL_HOME", str(home))
...
def test_dispatch_texture_sync_loops_content_packages_from_the_composed_path(tmp_path, monkeypatch,
                                                                             capsys):
    from types import SimpleNamespace
    from pathlib import Path
    import contextlib
    proj = _tex_project(tmp_path, monkeypatch)
    args = SimpleNamespace(cmd="texture", sub="sync", package=None, force=False,
                           catalog_dir=None, project=str(proj))
    fake_m = mock.Mock(textures={"Skins.Wall": object()})

    @contextlib.contextmanager
    def fake_container(**kw):                     # kw = mounts + state_dir (no repo-root arg)
        ms = kw.get("mounts")
    ...
    with mock.patch("uedctl.dispatch._resolve_level_source") as resolve, \
         mock.patch("uedctl.stub.ephemeral_build_container", fake_container), \
         mock.patch("uedctl.texture_catalog.sync_package", return_value=fake_m) as syncp:
        rc = dispatch(args)
    assert rc == 0
    resolve.assert_not_called()                                  # level-source-free
    assert syncp.call_args.kwargs["package"] == "CoreTexMetal"   # discovered from the composed path
    assert syncp.call_args.kwargs["container"] == "uned-stub-fake"   # the ad-hoc container
    assert syncp.call_args.kwargs["catalog_dir"] == str(proj / "texture-catalog")  # PROJECT catalog
    # the per-package flock home is CATALOG-adjacent (decision 2026-07-18): writers to one catalog
    # share one lock domain even across projects pointing at the same shared catalog dir
    assert syncp.call_args.kwargs["lock_dir"] == str(proj / "texture-catalog" / ".locks")
    assert "CoreTexMetal" in capsys.readouterr().out


def test_dispatch_texture_sync_container_start_failure_is_clean_exit2(tmp_path, monkeypatch, capsys):
    from types import SimpleNamespace
    from uedctl.stub import StubBuildError
    proj = _tex_project(tmp_path, monkeypatch)
    args = SimpleNamespace(cmd="texture", sub="sync", package=None, force=False, catalog_dir=None,
                           project=str(proj))

    def boom(**kw):                               # kw = mounts + state_dir
        raise StubBuildError("could not start build container: docker daemon not running")

    with mock.patch("uedctl.stub.ephemeral_build_container", boom):
        rc = dispatch(args)                                # must NOT raise a traceback
    assert rc == 2
    assert "could not start build container" in capsys.readouterr().err


def test_dispatch_texture_sync_unknown_package_is_clean_exit2(tmp_path, monkeypatch, capsys):
    from types import SimpleNamespace
    proj = _tex_project(tmp_path, monkeypatch)
    args = SimpleNamespace(cmd="texture", sub="sync", package="NoSuchPkg", force=False,
                           catalog_dir=None, project=str(proj))
    rc = dispatch(args)
    assert rc == 2
    assert "NoSuchPkg" in capsys.readouterr().err                # names the offender, no traceback


def test_dispatch_texture_classify_set_unknown_ref_is_clean_exit2(tmp_path, monkeypatch, capsys):
    from types import SimpleNamespace
    proj = _tex_project(tmp_path, monkeypatch)
    args = SimpleNamespace(cmd="texture", sub="classify", csub="set", ref="P.Nope",
                           tags=["x"], description=None, colors=None, catalog_dir=None,
                           project=str(proj))
    with mock.patch("uedctl.texture_catalog.load_manifest",
                    return_value=mock.Mock(textures={})), \
         mock.patch("uedctl.texture_catalog.manifest_path", return_value="x"):
        rc = dispatch(args)
    assert rc == 2
    assert "P.Nope" in capsys.readouterr().err                   # names the offender, no traceback


def test_dispatch_texture_classify_set_corrupt_catalog_is_clean_exit2(tmp_path, monkeypatch, capsys):
    from types import SimpleNamespace
    import json
    proj = _tex_project(tmp_path, monkeypatch)
    args = SimpleNamespace(cmd="texture", sub="classify", csub="set", ref="P.Tex",
                           tags=["x"], description=None, colors=None, catalog_dir=None,
                           project=str(proj))
    # load_manifest reads a corrupt JSON → ValueError; it now sits INSIDE the try, so this is a
    # clean exit 2 (was a bare JSONDecodeError traceback before).
    with mock.patch("uedctl.texture_catalog.load_manifest",
                    side_effect=json.JSONDecodeError("bad", "{", 0)), \
         mock.patch("uedctl.texture_catalog.manifest_path", return_value="x"):
        rc = dispatch(args)
    assert rc == 2
    assert "P.Tex" in capsys.readouterr().err


def test_dispatch_texture_read_verb_with_explicit_catalog_dir_needs_no_project(tmp_path, monkeypatch,
                                                                               capsys):
    from types import SimpleNamespace
    ...
    monkeypatch.setattr("uedctl.dispatch._resolve_project",
    ...
    args = SimpleNamespace(cmd="texture", sub="list", package=None, catalog_dir=str(tmp_path),
                           unclassified=False, classified=False, stale=False, removed=False,
                           project=None)
    rc = dispatch(args)                                 # must NOT call _resolve_project
    assert rc == 0                                      # read the explicit catalog, clean exit
    assert "MyPkg" not in capsys.readouterr().err       # (empty textures — nothing printed to stderr)


def test_dispatch_texture_classify_set_with_explicit_catalog_dir_needs_no_project(
        tmp_path, monkeypatch, capsys):
    from types import SimpleNamespace
    from uedctl import texture_catalog as tc
    # The WRITE verb `classify set` with an explicit --catalog-dir also resolves NO project
    ...
    m = tc.Manifest(package="MyPkg", package_file="MyPkg.utx", package_hash="sha256:bb",
                    textures={"Skins.Wood": entry})
    tc.save_manifest(tc.manifest_path(tmp_path, "MyPkg"), m)
    args = SimpleNamespace(cmd="texture", sub="classify", csub="set", ref="MyPkg.Wood",
                           tags=["wood"], description=None, colors=None,
                           catalog_dir=str(tmp_path), project=None)
    rc = dispatch(args)                                 # must NOT call _resolve_project
    assert rc == 0
    assert "MyPkg.Wood" in capsys.readouterr().out
    locks = tmp_path / ".locks"
    assert (locks / "texture-MyPkg.lock").exists()      # catalog-adjacent lock domain
    assert (locks / ".gitignore").read_text() == "*\n"  # self-ignoring, like .uedctl/
...
def _find_setup(tmp_path, monkeypatch, actors: dict, order: list[str]) -> tuple:
    ...
    lv.order = order
    monkeypatch.setattr("uedctl.dispatch._resolve_level_source",
...
def _find_seams(monkeypatch):
    ...
    monkeypatch.setattr("uedctl.dispatch._class_schema",
                        lambda cls, project=None: dict(schema))
    monkeypatch.setattr("uedctl.dispatch._class_defaults",
                        lambda cls, project=None: dict(defaults))
    monkeypatch.setattr("uedctl.dispatch._struct_members", lambda p, project=None: [])
    monkeypatch.setattr("uedctl.dispatch._enum_names",
...
def test_actor_find_without_prop_never_touches_schema(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("uedctl.dispatch._class_schema", boom)
    args = _find_args(branch, group=["cells"])
    rc = dispatch_mod._dispatch(args)
    assert rc == 0 and capsys.readouterr().out == "L1\n"
...
def test_mover_index_returns_the_resolved_class_index():
    ...
    with mock.patch("uedctl.dispatch._resolve_project", return_value=object()), \
            mock.patch("uedctl.dispatch._class_index", return_value=idx) as ci:
...
def test_mover_index_names_the_verb_when_the_path_carries_no_packages(capsys):
    ...
    from uedctl.tests.conftest import StubClassIndex
    with mock.patch("uedctl.dispatch._resolve_project", return_value=object()), \
            mock.patch("uedctl.dispatch._class_index",
...
def _run_prop_set(level, tokens, schema):
    ...
    src = _fake_src(level)
    with mock.patch("uedctl.dispatch._resolve_level_source", return_value=src), \
            mock.patch("uedctl.dispatch._resolve_project",
                       side_effect=dispatch_mod._ProjectError("no project")), \
            mock.patch("uedctl.dispatch._class_schema", lambda cls, project=None: dict(schema)):
...
def test_stash_capture_missing_from_t3d_is_clean_exit2(capsys):
    ...
    # with "not in a uedctl project" instead of the file error we're asserting).
    args = SimpleNamespace(cmd="stash", sub="capture", from_t3d=["/no/such/x.t3d"],
                           names=[], id=None, force=False, project=None)
    with mock.patch("uedctl.dispatch._resolve_stash_register", return_value=mock.Mock()):
        rc = dispatch(args)
    assert rc == 2
...
def test_level_preview_list_actors_routes_and_prints(tmp_path, monkeypatch, capsys):
    from uedctl import dispatch as D
    seen = {}
    monkeypatch.setattr("uedctl.preview_game.list_actors",
...
def test_maps_key_pointing_at_a_file_is_a_clean_filesystem_error(tmp_path, capsys):
    ...
    import argparse
    from uedctl import dispatch
    proj = tmp_path / "repo"
    proj.mkdir()
    (proj / "uedctl.toml").write_text('game = "dx"\nmaps = "mapsf"\n')
...
# ── _git_hint: report the PROJECT's own repo, not uedctl's (board bug 2) ──────
...
def test_git_hint_reports_the_projects_own_branch(tmp_path):
    from uedctl.dispatch import _git_hint
    proj = tmp_path / "proj"
...
def test_git_hint_does_not_leak_the_tool_repo_branch(tmp_path):
    # A scratch project living INSIDE uedctl's own source tree: git walks up to the tool repo. We
    # must report "not a git repo" instead of the tool repo's branch (the reported bug).
    from uedctl.dispatch import _git_hint
    tool = tmp_path / "toolrepo"
    tool_top = _git_init(tool, branch="uedctl-impl")
    proj = tool / "_scratch" / "proj"
    proj.mkdir(parents=True)
    hint = _git_hint(proj, proj / "maps", tool_repo_root=tool_top)
    assert hint is not None
    assert "uedctl-impl" not in hint and "not a git repo" in hint
...
def test_warn_duplicate_point_locations_fires_on_shared_spot(capsys):
    from uedctl.dispatch import _warn_duplicate_point_locations
    lvl = Level()
