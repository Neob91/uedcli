"""Persistent package-schema cache (`uedctl.schema_cache`) — spec
...
IN via `cache_on`. Each uses the autouse per-test `$UEDCTL_HOME` tmp dir, so entries land in a tmp
`cache/schema/` and never touch the developer's real `~/.uedctl`.
"""
...
from uedctl import config, schema_cache, uprops
...
def cache_on(monkeypatch):
    ...
    monkeypatch.setenv("UEDCTL_SCHEMA_CACHE", "on")
    _clear_memos()
    yield
    _clear_memos()
...
def test_frozen_golden_bundle_matches_fresh_decode():
    ...
    assert _GOLDEN.is_file(), (
        ...
        "name='fire', need_props=True).golden_bytes() under UEDCTL_SCHEMA_CACHE=off")
    monkey_off = os.environ.get("UEDCTL_SCHEMA_CACHE")
    os.environ["UEDCTL_SCHEMA_CACHE"] = "off"
    try:
        _clear_memos()
        fresh = schema_cache.load_package_schema(str(_FIRE), name="fire", need_props=True).golden_bytes()
    finally:
        if monkey_off is None:
            os.environ.pop("UEDCTL_SCHEMA_CACHE", None)
        else:
            os.environ["UEDCTL_SCHEMA_CACHE"] = monkey_off
...
def test_class_list_path_does_not_decode_own_props(cache_on, fire_copy, monkeypatch):
    ...
    files = _blobs(os.environ["UEDCTL_HOME"])
...
def test_need_props_writes_both_blobs_and_loads_own_props(cache_on, fire_copy):
    ...
    exts = sorted(f.suffix for f in _blobs(os.environ["UEDCTL_HOME"]))
...
def test_stat_change_is_a_miss(cache_on, fire_copy, monkeypatch):
    ...
    assert len(_blobs(os.environ["UEDCTL_HOME"], "disc")) == 2   # both entries, no stale serve
...
def test_utime_restored_stale_bytes_ARE_served_the_old_entry(cache_on, fire_copy, monkeypatch):
    ...
    Documents the known limitation — and that `UEDCTL_SCHEMA_CACHE=off` bypasses it."""
    ...
    monkeypatch.setenv("UEDCTL_SCHEMA_CACHE", "off")       # escape hatch re-decodes the ACTUAL bytes
    _clear_memos()
    schema_cache.load_package_schema(str(fire_copy))
...
def test_version_bump_is_a_miss(cache_on, fire_copy, monkeypatch):
    ...
    dirs = {p.parent.name for p in _blobs(os.environ["UEDCTL_HOME"], "disc")}
...
def test_realpath_keying_shares_one_entry(cache_on, fire_copy, tmp_path, monkeypatch):
    ...
    assert calls[0] == 1
    assert len(_blobs(os.environ["UEDCTL_HOME"], "disc")) == 1
...
def test_corrupt_entry_is_a_miss_not_an_error(cache_on, fire_copy, monkeypatch):
    ...
    entry = _blobs(os.environ["UEDCTL_HOME"], "disc")[0]
...
def test_parallel_writers_produce_a_valid_entry(cache_on, fire_copy):
    ...
    disc = _blobs(os.environ["UEDCTL_HOME"], "disc")
    prop = _blobs(os.environ["UEDCTL_HOME"], "prop")
...
def test_unwritable_cache_surfaces_as_error_not_swallowed(cache_on, fire_copy, monkeypatch):
    """A cache-write failure (classically a root-owned `~/.uedctl/cache`) MUST surface as a clean,
    ...
    two fixes: chown the dir, or `UEDCTL_SCHEMA_CACHE=off`."""
    def boom(path, data):
    ...
    with pytest.raises(schema_cache.CacheWriteError) as ei:
        schema_cache.load_package_schema(str(fire_copy))
    msg = str(ei.value)
    assert "chown" in msg and "UEDCTL_SCHEMA_CACHE=off" in msg   # actionable, not a bare OSError
...
def test_unwritable_cache_is_bypassed_by_the_off_escape_hatch(fire_copy, monkeypatch):
    ...
    monkeypatch.setenv("UEDCTL_SCHEMA_CACHE", "off")
    def boom(path, data):
        raise PermissionError(13, "Permission denied")
    monkeypatch.setattr(schema_cache, "_atomic_write", boom)
    _clear_memos()
...
def test_off_never_reads_or_writes(fire_copy, monkeypatch):
    ...
    assert _blobs(os.environ["UEDCTL_HOME"]) == []         # nothing written


def test_clear_removes_the_dir_and_is_a_noop_when_absent(cache_on, fire_copy):
    schema_cache.load_package_schema(str(fire_copy))
    root = config.schema_cache_root()
    assert root.is_dir() and _blobs(os.environ["UEDCTL_HOME"])
...
def test_sweep_defaults_to_env_or_constant_caps(cache_on, monkeypatch):
    ...
    monkeypatch.setenv("UEDCTL_SCHEMA_CACHE_MAX_BYTES", "1024")
...
def test_default_cap_env_parsing_and_fallback(monkeypatch):
    monkeypatch.delenv("UEDCTL_SCHEMA_CACHE_MAX_BYTES", raising=False)
    assert schema_cache._default_max_bytes() == schema_cache.SCHEMA_CACHE_MAX_BYTES
    monkeypatch.setenv("UEDCTL_SCHEMA_CACHE_MAX_BYTES", "4096")
    assert schema_cache._default_max_bytes() == 4096
    for bad in ("garbage", "-5", ""):                       # garbage / negative / blank ⇒ fall back
        monkeypatch.setenv("UEDCTL_SCHEMA_CACHE_MAX_BYTES", bad)
        assert schema_cache._default_max_bytes() == schema_cache.SCHEMA_CACHE_MAX_BYTES
    monkeypatch.delenv("UEDCTL_SCHEMA_CACHE_MAX_ENTRIES", raising=False)
    assert schema_cache._default_max_entries() is None      # count cap is off by default
    monkeypatch.setenv("UEDCTL_SCHEMA_CACHE_MAX_ENTRIES", "0")
...
def test_cache_gc_verb_reclaims_orphans_and_evicts_to_the_given_cap(cache_on, capsys):
    """`uedctl cache gc` is the on-demand surface over the same `sweep()` the on-write auto-GC runs:
...
def test_auto_sweep_evicts_lru_under_env_byte_cap(cache_on, fire_copy, monkeypatch):
    ...
    monkeypatch.setenv("UEDCTL_SCHEMA_CACHE_MAX_BYTES", "512000")
