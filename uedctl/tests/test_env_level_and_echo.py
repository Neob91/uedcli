"""Regressions for the ambient-`$UEDCTL_LEVEL` model (decisions 2026-07-20; spec
...
def test_mutation_from_env_echoes_the_level_to_stderr(tmp_path, monkeypatch, capsys):
    proj, _ = _project(tmp_path, monkeypatch)              # $UEDCTL_LEVEL=lvl, no --tree
    assert dispatch.dispatch(_move(proj)) == 0
    assert "editing level 'lvl' (from $UEDCTL_LEVEL)" in capsys.readouterr().err
...
def test_mutation_with_explicit_tree_is_silent(tmp_path, monkeypatch, capsys):
    ...
    assert "from $UEDCTL_LEVEL" not in capsys.readouterr().err   # explicit target → no echo
...
def test_read_verb_from_env_does_not_echo(tmp_path, monkeypatch, capsys):
    ...
    assert rc == 0
    assert "from $UEDCTL_LEVEL" not in capsys.readouterr().err   # a read never announces


def test_echo_fires_once_per_command(tmp_path, monkeypatch, capsys):
    proj, _ = _project(tmp_path, monkeypatch)
    assert dispatch.dispatch(_move(proj)) == 0
    assert capsys.readouterr().err.count("from $UEDCTL_LEVEL") == 1


# ── `level list` never crashes on a bad $UEDCTL_LEVEL (spec §4b/§8g) ─────────────────────────
...
def test_level_list_survives_a_bad_env(tmp_path, monkeypatch, capsys, bad):
    ...
    monkeypatch.setenv("UEDCTL_LEVEL", bad)               # set-but-bad (nonexistent / malformed)
    assert dispatch.dispatch(_ns(cmd="level", sub="list", project=str(proj), json=False)) == 0
    err = capsys.readouterr().err
    assert f"$UEDCTL_LEVEL={bad} (not listed)" in err


def test_level_list_marks_the_active_level(tmp_path, monkeypatch, capsys):
    proj, _ = _project(tmp_path, monkeypatch)             # $UEDCTL_LEVEL=lvl (a real level)
    assert dispatch.dispatch(_ns(cmd="level", sub="list", project=str(proj), json=False)) == 0
    out, err = capsys.readouterr()
    assert out.splitlines() == ["lvl"]
    assert "$UEDCTL_LEVEL=lvl" in err and "(not listed)" not in err
...
def test_empty_tree_string_is_an_error_not_a_silent_fallback(tmp_path, monkeypatch, capsys):
    # `--tree ""` (e.g. an unset shell var) must NOT silently read $UEDCTL_LEVEL — for a READ verb
    # that would be an invisible wrong-level read. It errors with the grammar message instead.
    proj, _ = _project(tmp_path, monkeypatch)             # $UEDCTL_LEVEL=lvl is set
