...
def _run(args, level, stdin=None):
    ...
    with contextlib.ExitStack() as stack:
        stack.enter_context(mock.patch("uedctl.dispatch.Driver"))
        stack.enter_context(mock.patch("uedctl.dispatch._resolve_level_source", return_value=src))
...
def test_engine_fact_uv_formula_is_base_relative_plus_pan():
    ...
    from uedctl.model import Polygon
    from uedctl.preview_native import _world_uv_frame
    p = Polygon()
