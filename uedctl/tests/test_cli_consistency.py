...
def test_stash_apply_summary_goes_to_stderr_not_stdout(tmp_path, monkeypatch, capsys):
    ...
    reg = stash_register.FileStashRegister(root / ".uedctl" / "stash")
...
def _prop_get_json(tokens):
    ...
    with mock.patch("uedctl.dispatch._resolve_level_source", return_value=src), \
            ...
            mock.patch("uedctl.dispatch._class_schema",
                       lambda cls, project=None: dict(_PROP_SCHEMA)), \
            mock.patch("uedctl.dispatch._class_defaults", lambda cls, project=None: {}), \
            mock.patch("uedctl.dispatch._struct_members", lambda p, project=None: []), \
            mock.patch("uedctl.dispatch._enum_names", lambda p, project=None: ()):
...
def test_brush_build_prop_bakes_props_onto_the_mover(capsys):
    ...
    with mock.patch("uedctl.dispatch._class_schema",
