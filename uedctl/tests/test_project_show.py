...
def _user_config(tmp_path, base, monkeypatch):
    """Point $UEDCTL_HOME at a per-user config declaring [games.deusex] over `base`."""
...
def _project(tmp_path, *, overlay=False):
    """A uedctl project ROOT (`<root>/uedctl.toml`). With overlay=True it declares a `paths` dir
    ...
    (root / "uedctl.toml").write_text(body)
    return root
...
def test_project_show_no_user_config_exits_2(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setenv("UEDCTL_HOME", str(tmp_path / "empty"))   # no config.toml there
