...
`// uedctl-folder:` carrier round-trip, and Group⊥folder independence.
...
from uedctl import cli, dispatch, trunk
from uedctl.model import Actor, Level
from uedctl.normalize import canonical_actor_t3d, canonical_level_hash
...
def test_folder_written_atomically_via_os_replace(tmp_path):
    ...
    with mock.patch("uedctl.t3dtree.os.replace", _spy):
        trunk.write_level(tmp_path, lvl, {"A_1": "m"})
...
def test_folder_never_appears_in_emitted_t3d():
    ...
    assert "castleXYZ" not in t and "uedctl-folder" not in t
...
def test_add_persists_incoming_folder_carrier(tmp_path, monkeypatch, capsys):
    ...
    shown = capsys.readouterr().out                      # carries `// uedctl-folder: castle.props`
...
def test_add_carrier_into_stash_target_is_rejected(tmp_path, monkeypatch, capsys):
    """A `// uedctl-folder:` carrier is a folder surface too — adding it into a stash box (which has
...
def test_show_emits_the_folder_carrier_by_default(tmp_path, monkeypatch, capsys):
    ...
    assert "// uedctl-folder: castle.tower" in capsys.readouterr().out
...
def test_show_t3d_only_suppresses_the_carrier(tmp_path, monkeypatch, capsys):
    ...
    assert "uedctl-folder" not in capsys.readouterr().out
...
def test_add_folder_carrier_wins_no_override(tmp_path, monkeypatch, capsys):
    # With --folder removed from `actor add`, the incoming `// uedctl-folder:` carrier always wins
