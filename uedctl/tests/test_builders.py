...
from uedctl.geometry import GeometryError, validate_brush
from uedctl.emit import emit_actor, snap
from uedctl.model import Brush, parse_t3d
...
def test_staircase_passes_doctor_clean():
    ...
    from uedctl.doctor import run_doctor, Severity
...
def test_make_brush_actor_props_and_csg():
    a = make_brush_actor("UedctlBrush0", cube(128, 128, 128),
    ...
    assert props["Brush"] == "Model'MyLevel.Model_UedctlBrush0'"
    assert a.brush.model_name == "Model_UedctlBrush0"
    assert a.location == (100.0, 200.0, 300.0)


def test_make_brush_actor_emits_brush_ref_after_block():
    a = make_brush_actor("UedctlBrush1", cube(64, 64, 64))
    t3d = emit_actor(a)
...
def test_built_cube_actor_roundtrips():
    a = make_brush_actor("UedctlBrush2", cube(256, 128, 64))
    back = next(iter(parse_t3d("Begin Map\n" + emit_actor(a) + "\nEnd Map\n").actors.values()))
    assert back.name == "UedctlBrush2"
