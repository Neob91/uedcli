#!/usr/bin/env bash
# build-image.sh — build the uedctl-game image (level preview --game).
...
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"             # Tools/uedctl/uedctl/game
...
exec 9>"/tmp/uedctl-game-imgbuild.lock"
flock 9
...
echo "== step 2: docker build uedctl-game =="
docker build -t uedctl-game -f "$HERE/Dockerfile" "$HERE"
echo "== done: uedctl-game =="
