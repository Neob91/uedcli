#!/usr/bin/env bash
# game-entrypoint.sh — baked into uedctl-game; runs as the container CMD after the base
...
# /resources/<n> mounts ARE the composed ~/.uedctl + project paths, project first), boots
...
echo "[uedctl-game] assembling game root (system=$UED_GAME_SYSTEM exe=$GEXE)"
...
# window to ~90% wedge; a small LOCAL symlink-farm dir (what uplayctl does, and what uedctl did
...
[ $? -eq 0 ] || { echo "[uedctl-game] FATAL: ini patch failed" >&2; exit 1; }
...
for _try in $(seq 1 "${UED_LAUNCH_TRIES:-8}"); do
  echo "[uedctl-game] launching (attempt $_try/${UED_LAUNCH_TRIES:-8})" >&2
  _launch
  sleep "${UED_WEDGE_S:-35}"
  if listening; then LINKED=1; break; fi
  if [ "$(wc -l < DeusEx.log 2>/dev/null || echo 0)" -le 22 ]; then
    echo "[uedctl-game] frozen at boot banner (pipe_read wedge) — fast relaunch" >&2
    pkill -9 -f "$GEXE" 2>/dev/null || true; DISPLAY=$D wineserver -k 2>/dev/null || true; sleep 2
    continue
  fi
  echo "[uedctl-game] boot progressing (log grew) — waiting for the link" >&2
  for _ in $(seq 1 "${UED_ATTEMPT_S:-160}"); do listening && { LINKED=1; break; }; sleep 1; done
  [ "$LINKED" = "1" ] && break
  echo "[uedctl-game] progressed but no link in ${UED_ATTEMPT_S:-160}s — relaunching" >&2
  pkill -9 -f "$GEXE" 2>/dev/null || true; DISPLAY=$D wineserver -k 2>/dev/null || true; sleep 2
done
if [ "$LINKED" != "1" ]; then
  echo "[uedctl-game] FATAL: link never bound" >&2
  exit 1                       # fail CLOSED (spec §4.3): no marker/watchdog zombie; host reports it
fi
echo "[uedctl-game] READY link-up"
...
: > /work/.last_use
echo "[uedctl-game] warm: idle self-death after ${IDLE}s (pin with /work/.pinned)"
while true; do
  sleep 60
  ...
  if [ $((now - last)) -ge "$IDLE" ]; then
    echo "[uedctl-game] idle ${IDLE}s — self-terminating" >&2
