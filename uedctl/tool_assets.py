...
The uedctl *installation* owns assets that belong to no project: the docker-compose dir + the
committed UED22 editor substrate (`Tools/uedctl/uned/`) and the umodel mesh extractor
(`Tools/umodel_win32/`). They resolve **package-relative** — from this installed `uedctl`
...
def tool_root() -> Path:
    """The tool's source root `Tools/uedctl/` — the dir HOLDING the `uedctl` package
    (`…/uedctl/tool_assets.py` → two levels up)."""
