...
COMPUTED_PROPS: frozenset[str] = frozenset({
    ...
    # so a uedctl-authored mover (which omits them) and its re-export (which adds them) must
    ...
    # carries the sentinel while a uedctl-authored mover (which omits both) does not; without this
...
# uedctl-allocated names always carry a `_<suffix>` and the engine's is `LevelInfo0`, so neither
...
def _levelinfo_rename(level: Level) -> dict[str, str]:
    ...
    name is ignored (`is_builder_brush`). uedctl auto-names it on capture (`LevelInfo_4dosan`);
...
def _stated_axes(a: Actor) -> str:
    ...
    which is correct, because every uedctl write path emits all three (`emit.emit_actor`). When the
