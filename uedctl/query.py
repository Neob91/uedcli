...
def list_actors(
    ...
    *,
    ...
    `folders` is a list of globstar folder patterns (uedctl-side folder dimension,
    §3); `no_folder` selects only ungrouped actors (folder is None) and is
    mutually exclusive with `folders` at the CLI.
    `labels` is a list of flat `*`-glob label patterns (uedctl-side label dimension,
...
def _csg_oper(actor) -> str:
    ...
    case-insensitive (an imported map may spell the prop key differently than uedctl authors it)."""
    for k, v in actor.props:
...
def actor_show_block(actor: Actor, with_sidecars: bool) -> str:
    ...
    `// uedctl-folder: <path>` and `// uedctl-labels: a,b` carrier lines inside the block (spec §4,
...
def show_actor(level: Level, name_or_glob: str, *, with_sidecars: bool = False) -> str:
    """Return canonical T3D for the actor(s) matching name_or_glob (with the `// uedctl-folder:` /
    `// uedctl-labels:` carriers per block when `with_sidecars`).
