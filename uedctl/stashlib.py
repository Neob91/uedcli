...
def with_folder(actor: Actor, folder: str | None) -> Actor:
    """Copy with the uedctl-side `folder` field set (or cleared if None). Independent of `with_group`
...
def _level_from_blobs(full_level: dict[str, str], order: list[str],
    ...
    identical to the trunk's. A blob carries no folder (folder is a uedctl-side sidecar, never in
