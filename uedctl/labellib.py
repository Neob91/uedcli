...
A **label** is a uedctl-side flat classification token on an actor (e.g. "lighting", "flammable",
...
pure grammar/validation + the flat matcher + the `// uedctl-labels:` carrier; it has NO I/O, no
...
# The `actor show` label interchange carrier: a bare `// uedctl-labels: a,b,c` line inside an actor
...
_LABELS_CARRIER = re.compile(r"^\s*//\s*uedctl-labels:\s*(.+?)\s*$")
...
def format_labels_carrier(labels) -> str:
    """The `// uedctl-labels:` line for an actor's label set: sorted, comma-joined, indented to sit
    inside an actor block (mirrors the folder carrier's indentation)."""
    return "    // uedctl-labels: " + ",".join(sorted(labels))
