...
across cold `uedctl` runs (spec `dev/docs/specs/2026-07-18-package-schema-cache.md`).

Every `uedctl` command is a fresh host-native process, so all schema decoding otherwise starts from
...
primitives to `~/.uedctl/cache/schema/v<N>/<key>.{disc,prop}`, so a warm hit skips `load_package`
...
(a same-size, same-mtime content spoof) is accepted; `UEDCTL_SCHEMA_CACHE=off` and `cache clear`
...
(an unwritable cache dir — e.g. a root-owned `~/.uedctl/cache`), is SURFACED as `CacheWriteError`,
...
**Footprint GC (automatic, best-effort).** Beyond the manual `uedctl cache clear` (nuke all), the
...
same sweep is exposed on demand as **`uedctl cache gc [--max-bytes N] [--max-entries N]`** (the flags
...
# (mainly for tests): UEDCTL_SCHEMA_CACHE_MAX_BYTES (size cap) and UEDCTL_SCHEMA_CACHE_MAX_ENTRIES
...
class CacheWriteError(Exception):
    ...
    root-owned `~/.uedctl/cache` left behind by a container run). Raised instead of silently swallowed
...
def _enabled() -> bool:
    """The cache is ON unless `UEDCTL_SCHEMA_CACHE=off` (any other value / unset = on) — read at call
    time so tests and CI can toggle it per-process (spec §7)."""
    return os.environ.get("UEDCTL_SCHEMA_CACHE", "").strip().lower() != "off"
...
def _write_blob(path: str, data: bytes) -> None:
    ...
    can't be written (classically a root-owned `~/.uedctl/cache` left by a container run) otherwise
    ...
    at exit 2. The escape hatch for a genuinely read-only cache location is `UEDCTL_SCHEMA_CACHE=off`
    ...
    try:
        ...
        raise CacheWriteError(
            ...
            f"or disable the cache with UEDCTL_SCHEMA_CACHE=off.") from e
    _maybe_auto_sweep()


# --------------------------------------------------------------------------- footprint GC
# The manual `uedctl cache clear` nukes everything; this GC is the best-effort footprint control that
...
# blob write, and on demand via `uedctl cache gc [--max-bytes N] [--max-entries N]`, whose flags
...
def _default_max_bytes() -> int | None:
    return _env_int("UEDCTL_SCHEMA_CACHE_MAX_BYTES", SCHEMA_CACHE_MAX_BYTES)


def _default_max_entries() -> int | None:
    return _env_int("UEDCTL_SCHEMA_CACHE_MAX_ENTRIES", SCHEMA_CACHE_MAX_ENTRIES)
...
def clear() -> bool:
    """Delete `cache/schema/` (backs `uedctl cache clear`). Returns True if a dir was removed, False
