...
#
# Deliberately NOT listed: COUNTS (`--steps`, `--sides`) and ANGLES (`--degrees-per-step`,
# `--angle-offset`). Their real constraint is tighter than "> 0" — >= 1 step, >= 3 sides,
# 0 < sweep < 180 — and already lives in `builders.py` next to the geometry reason for it, where it
# rejects negatives with a more specific message than this generic one could give. `--angle-offset`
# is legitimately negative (it rotates a cross-section either way).
...
def _build_brushes(builders, shape, args):
    ...
    Brush; spiral → a central column plus one wedge tread per step, a list of len > 1).
    The dispatch caller emits one actor per Brush, so the `len(list) > 1` branch stays
    live for the spiral."""
    ...
    if shape == "spiral":
        return builders.spiral_staircase(args.steps, args.inner_radius, args.step_width,
                                         args.rise, args.degrees_per_step, args.texture)
