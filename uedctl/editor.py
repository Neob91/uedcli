...
def ensure_editor(editor_id: str, *, state_dir: Path, mounts=None, ready_timeout: float = 90.0,
    ...
    `state_dir` (required): the resolved project's `.uedctl/` state dir (`config.state_dir`),
    ...
    def _spin_up() -> None:
        ...
        # UEDCTL_STUB_CACHE feeds the compose file's `/stubs` volume source
        # (`${UEDCTL_STUB_CACHE:-${HOME}/.uedctl/cache/stubs}`), so the mount follows
        # `config.stub_cache_root()` — i.e. honors `$UEDCTL_HOME` — instead of a hardcoded
