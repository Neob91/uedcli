...
def _install_atomic(*, staging_host: str, target_host: str) -> None:
    ...
    staging dir is `.uedctl/tmp/` (same filesystem as an in-repo target → os.replace is atomic,
    ...
    try:
        ...
        target_dir_tmp = os.path.join(os.path.dirname(target_host) or ".",
                                      f".{uuid.uuid4().hex}.uedctl-tmp.dx")
...
def _save_and_swap_verified(ed, dx_path: str, expected, *, defaults, state_dir,
    ...
    `state_dir` is the resolved project's `.uedctl/` state dir (`config.state_dir`), threaded from
...
def run_materialize(*, level, packages, out_path, overwrite, state_dir, schema_resolver,
    ...
    v69 stub shadows any same-named v68 `.u`). `state_dir` is the resolved project's `.uedctl/`
    ...
    try:
        ...
        # host-fs failure (unwritable .uedctl/tmp, missing base ini) — never a bare traceback.
