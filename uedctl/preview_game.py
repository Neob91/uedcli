"""`level preview --game` — the faithful in-game backend (spec
`dev/docs/specs/2026-07-13-ingame-preview-design.md`, gate-folded 2026-07-16; plan
`dev/docs/plans/2026-07-16-ingame-preview-plan.md`).

Drives a WARM per-user GAME container (`uedctl-game`, built on demand by
`uedctl/game/build-image.sh`), whose console-spawned TCP link (`UedPreview.u`) freezes and
...
# Every map delivered to the game is written under <root>/.uedctl/preview/ with a HASH-named,
...
def _preview_dir(project) -> Path:
    """`<root>/.uedctl/preview/` — the delivered-map cache in the self-ignoring state dir
...
def materialized_dx(project, level_name: str, level, *, rebuild: bool) -> Path:
    ...
    .uedctl/preview/. Reuse the hash-named file iff current AND not --rebuild (--rebuild
...
def copied_map(project, map_path: str) -> Path:
    """Copy a prebuilt `--map` file to `copied__<contenthash12>.<ext>` under .uedctl/preview/
...
def ensure_image(project, user_config, row: dict) -> None:
    """Build/refresh the uedctl-game image. FAST-PATH: if the image exists AND the game sources
    ...
    if not (game_dir / "inputs" / "edit" / "hUCC.exe").is_file():
        raise GamePreviewError(
            ...
            "see uedctl/game/inputs/README.md (user-supplied, gitignored)")

    src = _game_source_hash(game_dir)
    marker = config.user_cache_home() / "game-image.sha256"   # honors $UEDCTL_HOME (review fix)
    ...
    try:
        ...
        raise GamePreviewError("uedctl-game image build timed out (30 min)") from None
    if r.returncode != 0:
        tail = (r.stdout + r.stderr).strip().splitlines()[-8:]
        raise GamePreviewError("uedctl-game image build failed:\n  " + "\n  ".join(tail))
    try:
...
def _game_system_dir(dirs: list[str], row: dict) -> str:
    ...
    raise GamePreviewError(
        ...
        "~/.uedctl/config.toml [games.*].paths (the game's System dir must be listed)")
...
def _run_game_container(*, name: str, project, user_config, row: dict,
    ...
    (host .uedctl/preview dir) is bind-mounted read-only at PREVIEW_MOUNT — OUTSIDE the
...
# ONE reusable game container per Unix user (`uedctl-game-preview-<uid>`), serialized by a
...
def _warm_name() -> str:
    return f"uedctl-game-preview-{os.getuid()}"
...
def _acquire_lock() -> int:
    ...
    lives in the per-user home via `config._user_home()`, so `$UEDCTL_HOME` is honored like
    ...
    while True:
        try:
            ...
            if time.time() > deadline:
                os.close(fd)
                raise GamePreviewError(
                    f"another uedctl --game preview has held the container lock for "
...
def render_shots(*, shots: list[Shot], out_dir: Path, size: tuple[int, int],
    ...
    try:
        ...
        # Build the delivered map INTO <root>/.uedctl/preview/ (bind-mounted at PREVIEW_MOUNT, so it
