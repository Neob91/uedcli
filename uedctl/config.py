...
(colon-separated), NOT globs — uedctl owns the five package extensions (`.u .dx .utx .uax .umx`)
and scans the dirs itself:

- **Per-user** `~/.uedctl/config.toml` — one `[games.<name>]` block per game, each with a `paths`
  colon-separated list of ABSOLUTE directories. Nothing else (no `[defaults]`, no `image`, no
  `container`).
- **Per-project** `<root>/uedctl.toml` — a free-standing marker file at the repo root (à la
  ...
  a plain walk-up to the first ancestor containing an `uedctl.toml` file (nearest wins) — no
  child-dir scan, no schema sniffing.

Machine-local, never-tracked project state (stash, delivered preview maps, locks, staging)
lives in the **self-ignoring `<root>/.uedctl/`** (`state_dir`): on first
creation uedctl writes `.uedctl/.gitignore` containing `*`, so it can never be committed.
...
@dataclass(frozen=True, kw_only=True)
class Project:
    root: str                       # abspath of the dir holding uedctl.toml — the project root
    ...
    maps: str | None = None
    source: str = ""                # the uedctl.toml path
...
def resolve_dirs(paths_str: str, base: str, *, require_absolute: bool = False) -> list[str]:
    ...
    2026-07-14 03:30 — `paths` are bare dirs now, NOT globs). No file globbing: uedctl scans each
...
def _read_toml(path: str) -> dict:
    try:
        ...
        # An unreadable config (e.g. a permission-denied uedctl.toml found by walk-up) is a HARD,
...
def _user_home() -> Path:
    """The per-user uedctl home: `$UEDCTL_HOME` if set (expanded + resolved), else `~/.uedctl`.
    The single resolution point for everything that lives under the per-user home (config +
    the derivable cache)."""
    home = os.environ.get("UEDCTL_HOME")
    return Path(os.path.expanduser(home)).resolve() if home else Path.home() / ".uedctl"


def user_config_path(override: str | None = None) -> str:
    """The per-user config.toml path: --config override, else $UEDCTL_HOME/config.toml, else
    ~/.uedctl/config.toml."""
    if override:
...
def user_cache_home() -> Path:
    """The per-user, cross-project, DERIVABLE cache root: `<$UEDCTL_HOME or ~/.uedctl>/cache`.
...
def schema_cache_root(*, create: bool = False) -> Path:
    ...
    `dev/docs/specs/2026-07-18-package-schema-cache.md`). Holds one marshal blob per `(package
...
def _project_toml_path(dir_or_toml: str) -> str:
    ...
    if os.path.isdir(p):
        return os.path.join(p, "uedctl.toml")
    # A file path must BE an `uedctl.toml` — spec §4: "the root dir or the uedctl.toml itself".
    # Accepting any project-schema TOML under any name would silently bless mis-pointed
    # --project/UEDCTL_PROJECT values (review fix).
    if os.path.basename(p) != "uedctl.toml":
        raise ConfigError(f"not a project marker: {p} (pass the project ROOT dir or its uedctl.toml)")
    return p          # possibly nonexistent — caller's isfile check fires


def load_project(dir_or_toml: str) -> Project:
    """Load a project `uedctl.toml` (given the project ROOT dir or the file itself). Validates
    ...
    if not os.path.isfile(toml_path):
        raise ConfigError(f"no uedctl.toml at {toml_path}")
...
def state_dir(root: str | os.PathLike, *, create: bool = False) -> Path:
    """The project's machine-local state dir `<root>/.uedctl` — stash, delivered preview maps,
    ...
    return self_ignoring_dir(Path(root) / ".uedctl", create=create, what="state dir")
...
def self_ignoring_dir(d: Path, *, create: bool = False, what: str = "dir") -> Path:
    ...
    (`<root>/.uedctl/`) and the catalog-adjacent texture-lock dir (`<catalog>/.locks/`). Every
...
def walk_up_root(start: str) -> str | None:
    """Walk up from `start`; the FIRST ancestor directory containing an `uedctl.toml` FILE is the
...
def find_old_layout_project_dir(start: str) -> str | None:
    ...
    project schema under some ancestor (the pre-2026-07-17 layout, conventionally `uedctl/`).
...
def resolve_project(*, project_flag: str | None = None, env_project: str | None = None,
    ...
    """Resolve the current project by the spec §4 precedence — highest wins, no silent default:
      1. `project_flag` (a path to the root dir or the uedctl.toml → load; a bare name →
         `resolve_name` hook, else error: no registry)
      2. `env_project` (UEDCTL_PROJECT; same path/name handling)
      3. walk-up: the nearest ancestor directory containing an `uedctl.toml` file
      4. None (caller decides the error)"""
    cwd = cwd or os.getcwd()
    for source, token in (("--project", project_flag), ("UEDCTL_PROJECT", env_project)):
        if not token:
            continue
