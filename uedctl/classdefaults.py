...
2026-07-18). uedctl USED TO test against ZERO — this module is what replaced that. The two coincide
for almost every class, which is why the divergence hid for so long, and they parted company in two
ways:

* uedctl wrote a spelling the editor never writes -> the H3 post-verify aborted the build with
  nothing written (a *loud* failure);
* uedctl OMITTED a property whose class default is NOT zero -> the engine substitutes its default,
