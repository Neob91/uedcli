...
class _Assembler:
    ...
    def _resolve_prop_value(self, p: Prop) -> Prop:
        if p.ptype == AW.PT_OBJECT and isinstance(p.value, ObjRef):
            return Prop(p.name, p.ptype, self.eref(p.value.name),
                        array_index=p.array_index)
        if p.ptype == AW.PT_OBJECT and isinstance(p.value, ImportRef):
            return Prop(p.name, p.ptype,
                        self.resolver.object_ref(p.value.object_class, p.value.qualified),
                        array_index=p.array_index)
        return p
