"""Write primitives — add and _re_add (the paste/import seam used by materialize).
  ...
  D6 — uedctl owns a reserved Name namespace; add_actor never lets the editor
...
NAME_PREFIX = "Uedctl"   # uedctl owns this namespace (D6) — never let the editor auto-name
...
def _write_container_file(driver, content: str) -> str:
    ...
    path = f"{CONTAINER_TMP}/uedctl_{uuid.uuid4().hex}.t3d"
    subprocess.run(
...
def allocate_name(level: Level, cls: str) -> str:
    """Pick a fresh uedctl-owned Name for a new actor of `cls`, checked against
