"""uedctl CLI — the LLM-facing verb surface. Query and mutate verbs operate
model-side on $UEDCTL_LEVEL's git-native T3D trunk (`maps/<level>/`); the
...
uedctl reads it but never runs git for you, so history/recovery exist only once
...
_CATALOG_DIR_HELP = ("tracked manifest dir (default: the resolved project's catalog dir — "
                     "the uedctl.toml `catalog` key, or <root>/texture-catalog/)")
...
def _preview_opts(pp):
    ...
    pp.add_argument("--out", default=None, metavar="PATH",
                    help="host path to write the rendered PNG to (relative → resolved against the "
                         ...
                         "unique temp file is created (uedctl-preview-*.png). The absolute path "
...
def _tree_flag(parser, *, level_only=False):
    ...
    instead of the ambient `$UEDCTL_LEVEL` (decisions 2026-07-20 — the three are one tree format, so
    ...
    if level_only:
        parser.add_argument(
            ...
            help="build/preview this level instead of $UEDCTL_LEVEL: level/NAME (materialize and "
                 "preview operate on a level only — use `stash preview`/`prefab preview` for a "
                 "captured set). Default: the level named by $UEDCTL_LEVEL.")
        return
    parser.add_argument(
        "--tree", metavar="KIND/NAME", default=None,
        help="operate on this tree instead of $UEDCTL_LEVEL: KIND/NAME where KIND is "
             "level|stash|prefab and NAME is the level/stash/prefab name (may be nested, e.g. "
             "stash/hangar/arch). Default: the level named by $UEDCTL_LEVEL.")
...
def _apply_flags(ap):
    ...
    ap.add_argument("--folder", default=None, metavar="PATH",
                    help="also stamp this uedctl-side folder (dotted path, e.g. castle.props) on the "
...
def build_parser() -> argparse.ArgumentParser:
    p = _CoordArgumentParser(prog="uedctl", description=__doc__)
    p.add_argument("--project", default=None,
                   help="project root (or its uedctl.toml); else $UEDCTL_PROJECT, else the nearest "
                        "ancestor dir containing an uedctl.toml walking up from the cwd")
    ...
    fldg.add_argument(
        ...
        help="match actors by uedctl-side folder (globstar): a WILDCARD-FREE pattern like 'castle' "
    ...
    lblg.add_argument(
        ...
        help="match actors by uedctl-side label (flat `*`-glob): an actor matches if ANY of its "
    ...
    show.add_argument("--t3d-only", dest="t3d_only", action="store_true",
                      help="suppress the `// uedctl-folder:` comment for a byte-exact editor export "
    ...
    _tree_flag(show)      # read a named tree explicitly instead of $UEDCTL_LEVEL
    ...
    dup.add_argument("--folder", default=None, metavar="PATH",
                     help="place the copies in this uedctl-side folder path (overrides each "
                          "original's folder); omit to keep each original's folder")
    _tree_flag(dup)       # duplicate within a named tree (defaults to $UEDCTL_LEVEL)


    folder = asub.add_parser(
        "folder",
        help="manage an actor's uedctl-side FOLDER: a hierarchical dotted organization path "
    ...
    label = asub.add_parser(
        "label",
        help="manage an actor's uedctl-side LABELS: a flat, multi-valued set of classification "
    ...
    abuild.add_argument("--folder", default=None, metavar="PATH",
                        help="hierarchical organization PATH (dotted, e.g. castle.wall.north) for the "
                             "emitted actor — a uedctl-side sidecar, NEVER emitted to the built map, "
                             "rides the T3D as a `// uedctl-folder:` carrier that `actor add` persists.")
    abuild.add_argument("--label", action="append", default=[], type=_nonempty, metavar="L",
                        help="cross-cutting classification label for the emitted actor (repeat for "
                             "several) — a uedctl-side sidecar, NEVER emitted to the built map, rides "
                             "the T3D as a `// uedctl-labels:` carrier `actor add` persists.")

    add = asub.add_parser("add", help="add actors from a T3D file (or - for stdin) to the "
                                      "$UEDCTL_LEVEL's trunk")
    ...
    # emit the `// uedctl-folder:`/`// uedctl-labels:` carriers `actor add` reads back) or changed on the
    ...
    dele = asub.add_parser("delete", help="delete one or more actors from $UEDCTL_LEVEL")
    ...
    aprev = asub.add_parser(
        "preview",
        help="self-rendered orthographic wireframe of actors from $UEDCTL_LEVEL (brushes coloured "
    ...
    def _common_build_opts(bp):
        ...
        bp.add_argument("--folder", default=None, metavar="PATH",
                        help="hierarchical organization PATH (dotted, e.g. castle.tower.roof) for the "
                             "emitted actor(s) — a uedctl-side sidecar, INDEPENDENT of the engine Group "
                             "prop, NEVER emitted to the built map. Rides the T3D as a `// uedctl-folder:` "
        ...
        bp.add_argument("--label", action="append", default=[], type=_nonempty, metavar="L",
                        help="cross-cutting classification label for the emitted actor(s) (repeat to add "
                             "several). A uedctl-side sidecar (flat set), NEVER emitted to the built map; "
                             "rides the T3D as a `// uedctl-labels:` carrier `actor add` persists. "
    ...
    def _frame_group(parser):
        ...
        fg.add_argument("--from-world", dest="from_world", action="store_true",
                        help="with --to: the coords are an ABSOLUTE WORLD pose; uedctl subtracts "
    ...
    lmat.add_argument("--keep-build", action="store_true",
                      help="on a verify FAILURE, cp the built map out to the project's "
                           ".uedctl/tmp/ for inspection instead of discarding it "
                           "(default: discard on failure)")
    _tree_flag(lmat, level_only=True)   # build a named level explicitly instead of $UEDCTL_LEVEL

    lprev = lsub.add_parser(
        "preview",
        help="render freely-posed still shots of $UEDCTL_LEVEL. The default --game "
    ...
    lprev.add_argument("--map", default=None, metavar="PATH",
                       help="--game only: preview a prebuilt map file (.dx/.unr) instead of the "
                            "$UEDCTL_LEVEL trunk (skips the materialize cache). Actor-relative shots "
    ...
    _tree_flag(lprev, level_only=True)  # preview a named level explicitly instead of $UEDCTL_LEVEL

    lstat = lsub.add_parser("status",
                            help="thin read-only dashboard for $UEDCTL_LEVEL (or a --tree "
    ...
    _tree_flag(lstat)     # inspect a named tree explicitly instead of $UEDCTL_LEVEL
    ...
    _tree_flag(ldoc)      # lint a named tree explicitly instead of $UEDCTL_LEVEL
    ...
    egraph = evsub.add_parser(
        "graph",
        help="print $UEDCTL_LEVEL's trigger wiring — one edge per line "
    ...
    _tree_flag(egraph)    # analyse a named tree explicitly instead of $UEDCTL_LEVEL

    project = sub.add_parser("project", help="inspect the resolved uedctl project")
    ...
    cap.add_argument("--from-t3d", dest="from_t3d", nargs="+", default=None, metavar="FILE",
                     help="capture from these T3D file(s) instead of $UEDCTL_LEVEL, or - for a T3D "
    ...
    _tree_flag(cap)       # name the SOURCE tree explicitly instead of $UEDCTL_LEVEL;
    ...
    stapply = stsub.add_parser("apply", help="merge a register entry into $UEDCTL_LEVEL")
    ...
    pfapply = pfsub.add_parser("apply", help="merge a prefab into $UEDCTL_LEVEL")
    ...
    cache = sub.add_parser(
        "cache", help="manage the per-user derivable caches under ~/.uedctl/cache")
    cachesub = cache.add_subparsers(dest="sub", required=True)
    cachesub.add_parser(
        "clear",
        help="delete the persistent package-schema cache (~/.uedctl/cache/schema); it is pure "
    ...
    gc.add_argument("--max-bytes", type=int, metavar="N", default=None,
                    help="evict until the cache holds at most N bytes (default: the built-in cap, "
                         "256 MiB, or $UEDCTL_SCHEMA_CACHE_MAX_BYTES). 0 evicts everything.")
    gc.add_argument("--max-entries", type=int, metavar="N", default=None,
                    help="evict until the cache holds at most N blob files (default: no count cap "
                         "unless $UEDCTL_SCHEMA_CACHE_MAX_ENTRIES sets one). 0 evicts everything.")
