...
- a **stash** entry (`.uedctl/stash/<id>/`) — via `stash_register.py`,
...
never calls them). See specs/2026-07-05-uedctl-git-native-model-design.md +
...
def write_actor_tree(tree_dir: Path, level: Level, ranks: dict[str, str],
    ...
    for name, actor in level.actors.items():
        ...
        # The `folder` sidecar: a uedctl-side organization path, NOT in the T3D body/map (see
        ...
        # The `labels` sidecar: a uedctl-side FLAT set (sorted-set analog of `folder`), NOT in the
