...
A **folder** is a uedctl-side hierarchical dotted organization path on an actor (`castle.tower.roof`),
...
def format_folder_carrier(folder: str) -> str:
    """The `// uedctl-folder:` line for an actor's folder path, indented to sit inside an actor block
    (mirrors `labellib.format_labels_carrier`). Consumed by `emit.inject_carriers` / `actor show`; the
    T3D importer strips bare `//` lines, so it round-trips uedctl-side only."""
    return "    // uedctl-folder: " + folder
