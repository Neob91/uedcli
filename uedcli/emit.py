...
def snap(value) -> int:
    ...
    **No production caller today** — `grep -rn '\bsnap(' uedcli/` finds only this definition and
...
def inject_carriers(block: str, actor: Actor) -> str:
    """Insert the `// uedcli-folder:` / `// uedcli-labels:` carrier lines inside an actor T3D `block`
