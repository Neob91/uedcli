...
# another. Owner ruling 2026-09-07 (mesh skins); extended to surfaces 2026-09-10.
PROCEDURAL_RED = DecodedTexture(
    ref="<procedural>", width=1, height=1, rgb=b"\xff\x00\x00", mask=b"\x01",
    layout="linear1", layout_source="synthetic", format_code=-1, array="mips",
    b_masked=False, b_alpha_texture=False,
)


def resolve_or_procedural_red(resolver: "TextureResolver", ref: str) -> TextureResult:
    """`resolver.resolve(ref)`, substituting `PROCEDURAL_RED` for a `no-mip-data` result (a real
    texture the engine generates at runtime -- the draft rasterizer has no bitmap to sample)
    instead of surfacing it as an error. Any OTHER `TextureError` (unknown package/texture,
    corrupt body, an unsupported layout...) still comes back unchanged, for the caller to raise
    on -- this only swallows the ONE case that means "a real texture, just not a static one"."""
    got = resolver.resolve(ref)
    if isinstance(got, TextureError) and got.case == "no-mip-data":
