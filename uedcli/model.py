...
# parse_t3d reads the `// uedcli-labels:` carrier via `labellib._LABELS_CARRIER`.
...
# The `actor show` folder interchange carrier: a bare `// uedcli-folder: <path>` line inside an
...
_FOLDER_CARRIER = re.compile(r"^\s*//\s*uedcli-folder:\s*(\S+)\s*$")
...
class Actor:
    ...
    # A uedcli-side hierarchical dotted organization path (e.g. "castle.tower.roof"), stored in a
    ...
    # falls back to "every axis is stated" — which is exactly right, because every uedcli write
