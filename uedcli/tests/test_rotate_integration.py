...
Deselected by default (pytest.ini: addopts = -m "not integration"). Run with:
    pytest uedcli/tests/test_rotate_integration.py -m integration -v
...
editor reproduces the SAME world geometry uedcli's `world_vertices` predicts. The editor applies
the stored `Rotation` field to the local PolyList via its GMath integer sine table; uedcli's
...
def test_non_cardinal_brush_world_geometry_matches_table_driven_world_vertices(driver):
    ...
    matches uedcli's table-driven `world_vertices` to ~1e-4uu. This is the test that actually
    ...
    box = make_brush_actor("UedcliRotIT", cube(512, 64, 64, texture="DefaultTexture"),
    ...
    builder = make_brush_actor("UedcliBigBuilder",
