...
from uedcli.builders import PF_NOTSOLID, PF_SEMISOLID, make_brush_actor
from uedcli.doctor import Severity
...
def test_it_pins_line_key_epsilon_to_weld():
    ...
    from uedcli.doctor import _line_key, WATERTIGHT_LINE_EPS
    from uedcli.builders import WELD
...
def test_it_does_not_flag_nonsolid_walkable_floor():
    ...
    # decoration, a deliberate trap) — doctor must not flag it (board item
    # `doctor-fallthrough-warns-on-every-upward-facing`).
...
def test_subclass_mover_is_treated_as_closed_solid():
    from uedcli.doctor import _is_closed_solid_brush
