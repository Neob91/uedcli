"""Pins for the built-model located BSP check (invisible walls).

The committed golden `endgame4_model1.bin` is the built `Model1` body of retail `99_Endgame4.dx`
(a clean map) — it must locate NOTHING. The defect case is a constructed `Model` so the located
`Finding` can be asserted exactly.
"""
