...
Deselected by default (`pytest.ini`: `addopts = -m "not integration"`).  Run with:
    ...
    bin/test uedcli/tests/test_integration_intersect_oracle.py -m integration -v
...
from uedcli.tests import editor_oracle, intersect_cases
from uedcli.tests.merge_compare import FIXTURES, native_faces, oracle_faces, assert_same_faces
...
def test_native_matches_the_editor(case_id, state_dir):
    ...
    **Writing the golden is OPT-IN** (`UEDCLI_REGEN_GOLDENS=1`).  Rewriting the tracked fixture on
    ...
    if os.environ.get("UEDCLI_REGEN_GOLDENS"):
