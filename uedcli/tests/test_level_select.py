"""Unit tests for `level_sources.resolve_level` — the ambient-level resolver. Since 2026-07-20 the
...
from uedcli.cli.errors import LevelSelectionError
...
def test_resolve_returns_the_env_level(tmp_path):
    ...
    assert level_sources.resolve_level(env_level="20_AireGardens", maps_dir=maps) == "20_AireGardens"


def test_resolve_errors_when_env_unset(tmp_path):
    (tmp_path / "maps").mkdir()
    with pytest.raises(LevelSelectionError, match="no level"):
        level_sources.resolve_level(env_level=None, maps_dir=tmp_path / "maps")
...
def test_resolve_hints_the_bare_name_grammar_on_a_slash(tmp_path, value):
    ...
    with pytest.raises(LevelSelectionError, match="bare level name, not KIND/NAME"):
        level_sources.resolve_level(env_level=value, maps_dir=tmp_path / "maps")
...
def test_list_levels_still_enumerates_trunk_dirs(tmp_path):
    ...
    assert level_sources.list_levels(maps) == ["A", "b", "c"]
