...
from uedcli.native import codec, umodel, materialize, assemble
from uedcli.native.actor_write import Prop, PT_STRUCT, struct_vector
from uedcli.native.materialize import BuildError, self_check, build_carved_box_package
...
def test_native_ext_ffi_smoke():
    ...
    un = pytest.importorskip("uedcli_native")
...
def test_gate5_populated_hulls_rust_matches_python():
    ...
    from uedcli.native.csg_golden import _case_single_subtract
...
def _load_line_check():
    ...
    edits, so whether it imports at all depends on the spike tree's layout rather than on uedcli. When
    it does not import, SKIP with a reason naming the spike env — a permanently-red suite trains
    everyone to ignore red. A failure to import `uedcli` itself is NOT skipped: that is a real
    ...
    try:
        ...
        if (e.name or "").split(".")[0] == "uedcli":      # the code under test — a real failure
            raise
        pytest.skip(f"line_check oracle needs the spike env ({_SPIKE}): {type(e).__name__}: {e}")


def test_line_check_loader_skips_on_a_spike_dep_but_not_on_a_uedcli_one(monkeypatch):
    ...
    ImportError from `uedcli` itself must PROPAGATE: that is a regression in the code under test."""
    import importlib
    import os
    ...
    _raise.missing = "uedcli.native.umodel"              # the code under test → surface it
...
def _real_game_class_index():
    ...
    from the user's `~/.uedcli/config.toml` `[games.deusex].paths` — bypassing the autouse
    ...
    import os
    import tomllib
    from uedcli.native.pkgref import build_class_package_index
    override = os.environ.get("UEDCLI_GAME_SYSTEM")
    if override:
        return build_class_package_index([d for d in override.split(os.pathsep) if d])
    cfg = Path.home() / ".uedcli" / "config.toml"
    if not cfg.exists():
        return {}
    try:
