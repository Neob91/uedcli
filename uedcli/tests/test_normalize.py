...
from uedcli.model import parse_t3d as _parse_t3d
from uedcli.normalize import (normalize_actor as _normalize_actor,
...
def test_it_strips_editor_derived_basepos_baserot_but_keeps_keyframes():
    from uedcli.normalize import normalize_actor
    t3d = (
...
# uedcli-authored mover (which omits both) and the map it materializes into (which comes back
...
def _mover_level(*, stamped: bool):
    ...
    from uedcli.normalize import level_order
...
# default and omits a struct member equal to the default member (`unrealed/t3d.md`). uedcli's
...
def test_a_property_equal_to_the_class_default_compares_equal_to_an_omitted_line():
    """The five default-equal properties measured in this repo's own trunks (`uedcli/maps/*`). The
...
def test_the_editor_tag_default_stamp_is_dropped_from_the_compare():
    ...
    leaving it in would fail post-verify for every uedcli-created actor."""
