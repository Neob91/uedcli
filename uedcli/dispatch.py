...
current level ($UEDCLI_LEVEL) from its on-disk trunk (`maps/<level>/`) via the `TrunkLevelSource`
...
`git commit` = one change; the user's own `git`, not uedcli). The editor is
...
def _doc_bytes(doc) -> bytes:
    ...
    try:
        ...
        raise _SelectionExit(f"cannot read doc {doc.key}: {e.strerror or e}") from None


def _dispatch_docs(args) -> int:
    ...
    if args.sub == "show":
        if args.topic == "-":
            ...
            #
            # Reuses `_resolve_target_names`, the CLI's one newline-list-from-stdin reader, rather
            ...
            if missing:
                raise _SelectionExit("Docs not found: " + ", ".join(missing))
            chunks: list[bytes] = []
            for doc in resolved:
        ...
        if (doc := userdocs.find_doc(docs, args.topic)) is None:
            raise _SelectionExit(userdocs.not_found_message(docs, args.topic))
    ...
    if args.sub == "search":
        if not args.query.strip():
            ...
            raise _SelectionExit("docs search: the query must not be empty")
    ...
    raise _SelectionExit(f"unimplemented docs sub-verb: {args.sub}")
...
def _resolve_level_source(args):
    ...
    project = _resolve_project(args)                     # no project → _ProjectError → exit 2
    maps_dir = Path(config.project_maps_dir(project))
    name = level_select.resolve_level(env_level=os.environ.get("UEDCLI_LEVEL"), maps_dir=maps_dir)
        ...
        for a in incoming_actors:
            if a.folder is not None:                  # a carrier (possibly hand-edited) → validate
                try:
                    ...
                    raise _SelectionExit(f"in `// uedcli-folder:` carrier: {e}")
        ...
        for a in incoming_actors:
            for lbl in a.labels:                      # a carrier (possibly hand-edited) → validate
                try:
                    ...
                    raise _SelectionExit(f"in `// uedcli-labels:` carrier: {e}")
...
_NO_GAMES_CONFIG = ("no per-user games config (~/.uedcli/config.toml): needed to resolve the game's "
                    "base package paths; create it with a [games.<name>] paths dir list")
...
def _dispatch(args) -> int:
    ...
    # $UEDCLI_LEVEL, so it must run BEFORE the eager level-source resolution below.
    if args.cmd == "actor" and args.sub == "preview" and getattr(args, "from_t3d", None):
        return _preview_from_t3d(args)

    # Every remaining verb (reads included) resolves the trunk level source — the ambient $UEDCLI_LEVEL in a
    # uedcli project. No project ⇒ a clean `_ProjectError` (exit 2), not a traceback.
    ...
    if args.cmd == "actor" and args.sub == "add":
        ...
        # `// uedcli-folder:`/`// uedcli-labels:` carriers in the T3D (set by the generator) win as-is;
