...
def _ref_facts(resolver, ref: str) -> tuple[str | None, dict, "utexture.TextureError | None"]:
    ...
    + full facts. A procedural → its NAME identity + facts with no bitmap. Any other error →
    `identity=None` (reads unclassified; a per-ref request exits 2). `group` is a Layer-2 fact read
    live from the export.

    A procedural texture is NAME-keyed whether or not a frame can be painted for it: the painted
    pixels are generated by uedcli, not stored content, so hashing them would key the
    classification shard to the renderer's output instead of to the texture
    (`direction/asset-catalog.md` — name where content does not exist). `dimensions()` decides
    that, not `resolve()`: it reports `no-mip-data` for exactly the textures that store no pixels,
    and it stops before layout detection — so enumerating a whole path does not pay to paint
    hundreds of frames whose every fact here is `None`."""
    pkgi = resolver.package_for_ref(ref)
    group = utexture.group_of_export(*pkgi) if pkgi else None
    dims = resolver.dimensions(ref)
    if isinstance(dims, utexture.TextureError) and dims.case == "no-mip-data":
        pkg, name = _ref_pkg_name(ref)
        identity = utexture.procedural_identity(pkg, name)
        return identity, {"ref": ref, "width": None, "height": None, "format": None,
                          "group": group, "masked": None, "identity": identity,
                          "procedural": True}, None
...
def _run_preview(args, resolver, catalog_dir) -> int:
    ...
    for ref in refs:
        ...
        if args.skeleton:                                # the ready-to-fill classification row
            # A painted procedural frame's colours are this renderer's, not the texture's, and
            # this row is meant to be piped into `classify set -` — so it pre-fills nothing,
            # matching what `show` and `classify` already report for one (`_colors_for`).
            colors = ([] if utexture.is_procedural(res)
                      else texture_colors.derive_colors(res.width, res.height, res.rgb))
