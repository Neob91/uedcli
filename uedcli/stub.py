...
class StubBuildError(RuntimeError):
    ...
    clean exit rather than a bare traceback (uedcli house rule)."""
...
def ephemeral_build_container(*, state_dir, mounts=None, ready_timeout: float = 60.0):
    ...
    try:
        # Same compose file/volume as the editor: UEDCLI_STUB_CACHE feeds the `/stubs` mount
        # source (`${UEDCLI_STUB_CACHE:-…}`), so the BUILD container's stub cache also honors
        # `$UEDCLI_HOME` — /stubs is FIRST on its Paths precisely so previously-built stubs
