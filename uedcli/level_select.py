...
19:07). The level is an **ambient environment variable `$UEDCLI_LEVEL`** (a bare level name), read by
`resolve_level` as the default when a verb has no explicit `--tree` (decisions 2026-07-20 — this
replaces the old machine-local `.uedcli/current-level` pointer file, which was a live cross-session
