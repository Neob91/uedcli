...
reverse-engineered spec at `dev/docs/board/inbox/unrealed-geometry-build-map-rebuild-bsp-rebuild/spec.md`.
...
- **Never cite `uedcli-native`'s own code, or any board item, as evidence for what UnrealEd
  itself does.** Only fresh disassembly or direct live observation of the real editor counts.
  The spec (`dev/docs/board/inbox/unrealed-geometry-build-map-rebuild-bsp-rebuild/spec.md`) states
  this rule at its own top and should keep being the durable record of confirmed UnrealEd facts.
...
- Board items are agent-operated territory (`dev/docs/board/`) — log findings freely via
  `bin/board new inbox '<title>'` or by writing `overview.md` directly, no owner approval needed
  there, unlike the rest of `dev/docs/`.

## Where everything lives

- Full history of today's investigation: `dev/docs/board/inbox/front-2-re-characterized-diffuse-repartition/overview.md`,
  `dev/docs/board/inbox/editor-unatco-repartition-soup-size-unknown/overview.md`,
  `dev/docs/board/inbox/findbestsplit-divergence-forensic-dive-17-real/overview.md`,
  `dev/docs/board/inbox/bspmergecoplanars-8-case-merge-gap-live-traced/overview.md` (read in this
  order — each is the exact resume pointer the next session's task was built from).
  Also `dev/docs/board/done/unatco-detail-brush-pass-staging-generalized/overview.md` for the
...
- The reverse-engineered spec of UnrealEd's own behavior (the ground truth every fix here is
  checked against): `dev/docs/board/inbox/unrealed-geometry-build-map-rebuild-bsp-rebuild/spec.md`.
