...
container, driven by `Tools/uedctl/uned/wine_ctl.py`) into a **queryable, scriptable, auditable**
...
uedctl is **pure Python 3.12** (one dep, `Pillow`). It runs **host-native** through the `bin/uedctl`
launcher, which self-creates and reuses a dev venv (`bin/_venv.sh`, `.venv/`) — the host needs
`python3.12` on `PATH`, nothing else. Only the editor/build containers uedctl *drives* use Docker.
...
export PATH="$PWD/Tools/uedctl/bin:$PATH"   # or: ln -s "$PWD/Tools/uedctl/bin/uedctl" ~/.local/bin/

uedctl level status
uedctl actor find --exact-class Brush
uedctl brush poly list Brush41
uedctl actor preview Brush41 --out /tmp/b41.png   # color quad wireframe PNG, poly-index labels
uedctl brush clip Brush41 --axis z --coord 128 --keep below
...
Prefer a native interpreter? Everything above also works as `python3 -m uedctl …` /
`python -m pytest uedctl -q` from a Python **3.12** environment with `Pillow>=11`.
